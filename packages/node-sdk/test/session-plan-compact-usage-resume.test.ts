...
import { createMultiAIHarness, type Event, type MultiAIError } from '#/index';
...
describe('Session plan, compact, usage, and resume APIs', () => {
  ...
  it.todo('marks resumed plan mode active when the restored plan has no plan data', async () => {
    ...
    await writeTestConfig(homeDir);
    const createdHarness = createMultiAIHarness({ homeDir, identity: TEST_IDENTITY });
    let sessionId = '';
    let sessionDir = '';
    ...
    const resumedHarness = createMultiAIHarness({ homeDir, identity: TEST_IDENTITY });
    try {
