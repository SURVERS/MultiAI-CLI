 ...
 * Wiring: real MultiAIHarness/Core/filesystem and stdio transport; only the OAuth
...
describe('global MCP configuration (persisted user entries)', () => {
  it('lists only user-global servers when project config also exists', async () => {
    ...
    const harness = createMultiAIHarness({ homeDir });
...
function oauthHarness(rpc: OAuthRpc): MultiAIHarness {
