 ...
 * Responsibilities: list workspace/session skills and activate a session skill through MultiAIHarness.
...
import type * as KosongModule from '@multiai/kosong';
...
vi.mock('@multiai/kosong', async (importOriginal) => {
...
describe('Session skills', () => {
  it('lists session skills without exposing content', async () => {
    ...
    try {
      ...
      expect(listed?.path.endsWith('/.multiai/skills/review/SKILL.md')).toBe(true);
  ...
  it('activates a skill through core and emits the public skill event', async () => {
    ...
    try {
      ...
      const skillDir = normalizeWorkDir(await realpath(join(workDir, '.multiai', 'skills', 'review')));
      await expect(
        waitForAgentWireEvent(
          homeDir,
          session.id,
        ...
        input: [
          {
            ...
            text: [
              ...
              '',
              `<multiai-skill-loaded name="review" trigger="user-slash" source="project" dir="${skillDir}" args="src/app.ts">`,
              'Review the requested file.',
              '',
  ...
  it('resolves user brand skills from MULTIAI_HOME, not the OS home', async () => {
    const homeDir = await makeTempDir(tempDirs, 'kimi-sdk-skills-home-');
    ...
    const harness = createMultiAIHarness({ identity: TEST_IDENTITY });
...
describe('MultiAIHarness workspace skills', () => {
