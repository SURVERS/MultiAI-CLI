...
const USAGE = `Usage:
  pnpm exec tsx --import ./build/register-raw-text-loader.mjs packages/node-sdk/examples/multiai-harness-log-marker.ts --session <session-id>

Options:
  -s, --session <id>   Existing session id to resume and mark
      --home <dir>     MultiAI home dir; defaults to MULTIAI_HOME or ~/.multiai
...
async function main(): Promise<void> {
  ...
  const resolvedHome = resolveMultiAIHome(options.homeDir);
  const harness = createMultiAIHarness({
    identity: { userAgentProduct: 'multiai-cli', version: 'log-marker' },
  ...
  try {
    ...
    sessionLogPath = join(sessionDir, 'logs', 'multiai.log');
    const payload = {
  ...
  if (!sessionMatched) {
    process.stderr.write(
      [
        ...
        'Check that MULTIAI_LOG_LEVEL is not "off" and that the session id exists in this MULTIAI_HOME.',
        '',
      ].join('\n'),
    );
    process.exitCode = 1;
