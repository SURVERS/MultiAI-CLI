...
import { createMultiAIHarness, log } from '@multiai/sdk';

const SESSION_LOG = 'logs/multiai.log';
const GLOBAL_LOG = 'logs/global/multiai.log';
const MAIN_WIRE = 'agents/main/wire.jsonl';
const TEST_HOME = join(homedir(), '.multiai-test');
...
async function readSessionLogs(sessionDir: string): Promise<{
  ...
  const files = (await readdir(logsDir))
    .filter((file) => file === 'multiai.log' || file.startsWith('multiai.log.'))
    .toSorted();
...
async function assertRotatedFilesStayBounded(sessionDir: string, files: readonly string[]): Promise<void> {
  assert(files.includes('multiai.log.1'), 'session log did not rotate after exceeding max bytes');
  assert(!files.includes('multiai.log.2'), 'session log kept more archives than configured');
...
async function main(): Promise<void> {
  process.env['MULTIAI_HOME'] = TEST_HOME;
  process.env['MULTIAI_LOG_LEVEL'] = 'warn';
  process.env['MULTIAI_LOG_SESSION_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['MULTIAI_LOG_SESSION_FILES'] = '2';
  process.env['MULTIAI_LOG_GLOBAL_MAX_BYTES'] = String(MAX_LOG_BYTES);
  process.env['MULTIAI_LOG_GLOBAL_FILES'] = '2';
  ...
  const harness = createMultiAIHarness({
    identity: { userAgentProduct: 'multiai-cli', version: '0.1.1' },
    homeDir: TEST_HOME,
  });
  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        '  - logs/multiai.log',
        '  - logs/multiai.log.1',
        `  - ${summary.sessionDir}/logs/multiai.log`,
        `  - ${summary.sessionDir}/logs/multiai.log.1`,
        '',
        'Rotation evidence:',
        `  - should NOT appear in session logs: ${evictedEntry}`,
        `  - should appear in session logs: ${finalEntry}`,
        '  - multiai.log.1 exists',
        '  - multiai.log.2 does not exist',
        '',
        'Export evidence:',
        '  - logging-session.zip includes session logs and wire.jsonl',
        '  - logging-session.zip does not include logs/global/multiai.log',
        '  - logging-with-global.zip includes only the active global log at logs/global/multiai.log',
        '  - global rotated logs such as ~/.multiai-test/logs/multiai.log.1 are intentionally not bundled',
        '',
        'Redaction evidence:',
