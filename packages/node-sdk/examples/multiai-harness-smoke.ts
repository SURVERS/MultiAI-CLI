...
async function main(): Promise<void> {
  const workDir = await mkdtemp(join(tmpdir(), 'multiai-harness-smoke-'));
  const homeDir = await mkdtemp(join(tmpdir(), 'multiai-harness-home-'));
  ...
  try {
    ...
    const model = config.defaultModel ?? 'multiai/kimi-for-coding';
