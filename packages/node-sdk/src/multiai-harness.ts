...
import type { MultiAIAuthFacade } from '#/auth';
...
export interface MultiAIHarnessRuntimeOptions {
...
export class MultiAIHarness {
  readonly homeDir: string;
  readonly configPath: string;
  readonly auth: MultiAIAuthFacade;

  private readonly identity: MultiAIHostIdentity | undefined;
  ...
  constructor(
    ...
    options: MultiAIHarnessRuntimeOptions,
  ) {
  ...
  async getConfig(options: GetConfigOptions = {}): Promise<MultiAIConfig> {
  ...
  async setConfig(patch: MultiAIConfigPatch): Promise<MultiAIConfig> {
  ...
  /** User-global MCP entries from `<MULTIAI_HOME>/mcp.json` only. */
  ...
  async authenticateMcpServer(
    ...
    try {
      ...
      if (opened === false) {
        throw new MultiAIError(ErrorCodes.REQUEST_INVALID, 'MCP OAuth authorization was cancelled');
      }
...
function normalizeSessionId(value: string): string {
  if (typeof value !== 'string') {
    throw new MultiAIError(ErrorCodes.SESSION_ID_REQUIRED, 'Session id is required.');
  }
  const normalized = value.trim();
  if (normalized.length === 0) {
    throw new MultiAIError(ErrorCodes.SESSION_ID_EMPTY, 'Session id cannot be empty.');
  }
  return normalized;
}
