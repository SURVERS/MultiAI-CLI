 ...
 *
 * Usage:
 *   pnpm --filter @multiai/agent-core-v2 gen:config-manifest          # write the file
 *   pnpm --filter @multiai/agent-core-v2 gen:config-manifest --check  # freshness check (CI-style)
 *
...
function packageRelativePath(file: string): string {
  return relative(PKG, file).replaceAll('\\', '/');
}
...
function scanSectionOwners(): Map<string, string> {
  ...
  for (const file of walk(SRC)) {
    ...
    for (const match of source.matchAll(/registerConfigSection\(\s*(?:'([^']+)'|([A-Za-z0-9_$]+))/g)) {
      ...
      if (domain !== undefined) owners.set(domain, packageRelativePath(file));
    }
  }
  return owners;
}
...
function scanOverlayOwners(): Map<string, string> {
  ...
  for (const file of walk(SRC)) {
    ...
    for (const match of source.matchAll(/registerConfigOverlay\(\s*([A-Za-z0-9_$]+)/g)) {
      const ident = match[1];
      if (ident !== undefined) owners.set(ident, packageRelativePath(file));
    }
  }
  return owners;
}
...
export async function buildConfigManifest(): Promise<string> {
  ...
  const out: string[] = [
    ...
    '# Regenerate with: pnpm --filter @multiai/agent-core-v2 gen:config-manifest',
    '#',
