 ...
 *
 * Usage:
 *   pnpm --filter @multiai/agent-core-v2 gen:wire-manifest          # write the file
 *   pnpm --filter @multiai/agent-core-v2 gen:wire-manifest --check  # freshness check (CI-style)
 *
...
function findImportSource(file: string, name: string): string | undefined {
  ...
  for (const m of source.matchAll(re)) {
    for (const part of (m[1] ?? '').split(',')) {
      ...
      const source = m[2];
      if (source !== undefined && (named[2] ?? named[1]) === name) return source;
    }
  }
  return undefined;
}
...
function summarizeTsUnion(
  ...
  if (bodies.length > 0 && bodies.every((b) => b !== undefined)) {
    ...
    const firstFields = fieldMaps[0] ?? new Map<string, { type: string }>();
    for (const [name, info] of firstFields) {
      if (
        /^'[^']*'$/.test(info.type) &&
  ...
  if (record?.[1] !== undefined) {
    ...
    if (parts.length === 2) {
      ...
      if (valuePart !== undefined) {
        return `record<string, ${stringifySketch(friendlyZodExpr(valuePart, ownerFile, depth + 1))}>`;
      }
    }
  }
...
export async function buildWireManifest(): Promise<string> {
  ...
  const out: string[] = [
    ...
    '// Regenerate with: pnpm --filter @multiai/agent-core-v2 gen:wire-manifest',
    '//',
