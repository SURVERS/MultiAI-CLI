/**
 * Scenario: executing built-in and registered tools through the agent tool contract.
 * Responsibilities: enforce permissions/hooks, persist oversized output, and emit stable wire facts.
 * Wiring: a real scoped agent uses deterministic model, process, hook, and RPC boundaries.
 * Run: pnpm exec vitest run packages/agent-core-v2/test/tool/tool.test.ts
 */
...
describe('Agent tool execution contract', () => {
  ...
  it('points at the secondary model config when the configured alias is invalid', async () => {
    ...
    expect(result.output).toContain('comes from [secondary_model].model / MULTIAI_SECONDARY_MODEL');
    expect(lifecycle.create).not.toHaveBeenCalled();
  });
  ...
  it('does not rewrite spawn failures unrelated to the model config', async () => {
    ...
    expect(result.output).not.toContain('MULTIAI_SECONDARY_MODEL');
  });
        ...
        [
          {
            event: 'PreToolUse',
            matcher: 'Bash',
            command: nodeEvalCommand("console.error('blocked by PreToolUse'); process.exit(2);"),
          },
          {
