 ...
 * REST consumers can render it: an internal `multiai-file://<id>?path=…`
...
import { parseMultiAIFileUrl } from '#/agent/media/multiaiFileUrl';
...
function mapContentPart(part: ContextMessage['content'][number]): MessageContent {
  switch (part.type) {
    ...
    case 'video_url': {
      const ref = parseMultiAIFileUrl(part.videoUrl.url);
