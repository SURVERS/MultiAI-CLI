...
description: Import Claude Code and Codex instructions, skills, and MCP settings into MultiAI CLI.
...
Help them migrate selected local Claude Code and Codex assets into MultiAI CLI.
...
- Do **not** migrate `.agents` content. MultiAI CLI already supports `.agents`
...
- Only write under MultiAI CLI targets:
  - User-global: `$MULTIAI_HOME` if set, otherwise `~/.multiai`.
  - Project instructions/skills: `<project root>/.multiai`, where the project
    root is the nearest parent directory containing `.git`; if no `.git` exists,
    use the current working directory.
  - Project-local MCP: `<cwd>/.multiai/mcp.json`, because MultiAI reads the
    current working directory's MultiAI-specific MCP file, not every project-root
    `.multiai/mcp.json` from subdirectories.
- Preserve existing MultiAI files. Never overwrite existing skills or replace an
...
Resolve paths explicitly; `~` is the real OS home, and MultiAI home follows
`$MULTIAI_HOME` before `~/.multiai`.
...
MultiAI-readable, and project-root `.mcp.json` is already read by MultiAI as a
...
- `$MULTIAI_HOME/AGENTS.md`, or `~/.multiai/AGENTS.md` if the env var is not
  set.

Map project-level instruction sources to:

- `<project root>/.multiai/AGENTS.md`
...
- `$MULTIAI_HOME/skills/`, or `~/.multiai/skills/` if the env var is not set.

Map project-level skill sources to:

- `<project root>/.multiai/skills/`
...
- Read a bundle's `SKILL.md` enough to verify that directory skills have
  frontmatter with non-empty `name` and `description`, because MultiAI requires
...
- If two source entries would write the same target path, keep the first one in
  ...
  3. user Claude
  4. user Codex
- Warn when a source skill uses Claude/Codex-specific fields or syntax that MultiAI
...
For the preview, collect MCP candidates and normalize them into MultiAI's MCP shape
when possible:
...
- Map Codex fields to MultiAI fields:
  - `command` -> `command`
  - `args` -> `args`
  - `env` -> `env`
  - `cwd` -> `cwd`
...
- Do not import project-root `.mcp.json`; MultiAI already reads it.

For each MCP candidate, choose the target scope in the preview:

- User-level source -> user-global MCP target (`$MULTIAI_HOME/mcp.json` or
  `~/.multiai/mcp.json`).
- Project-level source -> project-local MultiAI MCP target (`<cwd>/.multiai/mcp.json`). If `<cwd>` is not the project root, call this out in the preview so the user understands when MultiAI will load it.
...
- collision policy: keep existing MultiAI entries on name conflict,
...
- Tell the user to start a new session (for example `/new`) or restart MultiAI CLI
