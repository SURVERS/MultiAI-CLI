 ...
 *
 * Declares the `MULTIAI_CRON_*` environment bindings for the cron operational
...
export const cronEnvBindings: EnvBindings<CronConfig> = envBindings(cronConfigSchema, {
  debug: { env: 'MULTIAI_CRON_DEBUG', parse: on },
  noJitter: { env: 'MULTIAI_CRON_NO_JITTER', parse: on },
  noStale: { env: 'MULTIAI_CRON_NO_STALE', parse: on },
  disabled: { env: 'MULTIAI_DISABLE_CRON', parse: on },
  manualTick: { env: 'MULTIAI_CRON_MANUAL_TICK', parse: on },
  clock: 'MULTIAI_CRON_CLOCK',
  pollIntervalMs: { env: 'MULTIAI_CRON_POLL_INTERVAL_MS', parse: parsePollIntervalMs },
});
