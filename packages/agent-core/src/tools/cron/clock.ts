 ...
 *
 * `wallNow` resolution is driven by the `MULTIAI_CRON_CLOCK` env var; see
...
export interface ClockSources {
   ...
   * via `MULTIAI_CRON_CLOCK`. Used for cron matching, `createdAt`, stale
   * judgment.
   */
  wallNow(): number;
 ...
 * whenever `MULTIAI_CRON_CLOCK` is unset, set to `"system"`, or set to a
 ...
 * `process.env.MULTIAI_CRON_CLOCK`).
 *
...
function debugInvalidSpec(spec: string, reason: string): void {
  ...
  // gated on MULTIAI_CRON_DEBUG is enough — production is silent.
  if (process.env['MULTIAI_CRON_DEBUG'] === '1') {
    process.stderr.write(
      `[cron/clock] invalid MULTIAI_CRON_CLOCK spec ${JSON.stringify(spec)}: ${reason} — falling back to system clock\n`,
    );
  }
}
