...
const ROOT_SYMBOL = Symbol.for('multiai.logger.root');
const SESSION_LOG_ID = Symbol('multiai.logger.sessionLogId');
...
class RootLoggerImpl implements RootLogger {
  ...
  attachSession(input: SessionAttachInput): SessionLogHandle {
    ...
    const sink = new RotatingFileSink({
      path: join(input.sessionDir, 'logs', 'multiai.log'),
 ...
 * importing `log` at module load (before `MultiAIHarness` configures the root)
