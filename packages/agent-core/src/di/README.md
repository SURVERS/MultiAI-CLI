# `@multiai/agent-core` Dependency Injection Container
...
- **Testing** — `TestInstantiationService` (subpath
  `@multiai/agent-core/di/test`) exposes direct `.get` / `.stub` so
...
1. **Zero runtime dependencies** — the container is ~600 LoC of plain
   ...
   subsystem.
2. **Familiar shape** — most MultiAI contributors have seen VS Code's
...
Test files import from the subpath `@multiai/agent-core/di/test`
...
├── test.ts                      ← subpath barrel (`@multiai/agent-core/di/test`)
