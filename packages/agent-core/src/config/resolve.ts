...
/**
 * Parse a floating-point environment value (e.g. `MULTIAI_MODEL_TEMPERATURE`).
 * Returns `undefined` when unset/blank; throws `MultiAIError(CONFIG_INVALID)` on a
 * non-numeric value so a typo fails fast like the other `MULTIAI_MODEL_*` vars.
...
export function parseFloatEnv(value: string | undefined, varName: string): number | undefined {
  ...
  if (!Number.isFinite(parsed)) {
    throw new MultiAIError(ErrorCodes.CONFIG_INVALID, `${varName} must be a number, got "${value}".`);
  }
  return parsed;
}
