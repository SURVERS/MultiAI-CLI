 ...
 * `<MULTIAI_HOME>/credentials/mcp/` (default
 * `~/.multiai/credentials/mcp/`).
 *
...
export function mcpCredentialsDir(multiaiHomeDir: string): string {
  return join(multiaiHomeDir, 'credentials', 'mcp');
}

export function defaultMcpCredentialsDir(): string {
  return mcpCredentialsDir(join(homedir(), '.multiai'));
}
