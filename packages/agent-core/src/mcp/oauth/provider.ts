 ...
 *    state under `<MULTIAI_HOME>/credentials/mcp/<key>-*.json`
 *    (mode 0600; default home is `~/.multiai`).
...
export interface McpOAuthProviderOptions {
  ...
  /** Identifier embedded in DCR `client_name` ("multiai-cli (server)"). */
...
export class McpOAuthClientProvider implements OAuthClientProvider {
  ...
  constructor(options: McpOAuthProviderOptions) {
    ...
    this.clientLabel = options.clientLabel ?? `multiai-cli (${options.serverName})`;
  }
