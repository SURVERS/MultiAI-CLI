...
// Relative to brandHomeDir, which already IS the brand data dir (~/.multiai or
// $MULTIAI_HOME) — no '.multiai' segment here, or it would nest twice.
const USER_BRAND_DIRS = ['skills'] as const;
const USER_GENERIC_DIRS = ['.agents/skills'] as const;
const PROJECT_BRAND_DIRS = ['.multiai/skills'] as const;
...
export interface SkillPathContext {
  ...
  /**
   * Brand data dir — `MULTIAI_HOME`, or `<userHomeDir>/.multiai` by default.
   * User brand skills live directly under here as `skills/`, so this path
   * carries no `.multiai` segment of its own (that would double the prefix).
   */
...
export async function resolveSkillRoots(
  ...
  const brandHomeDir = options.paths.brandHomeDir ?? path.join(userHomeDir, '.multiai');
