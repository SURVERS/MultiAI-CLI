 ...
 * `MultiAICore` (and synthetic events from daemon-side services) to all
 ...
 *
 * - `Event` (from `@multiai/agent-core/base/common/event`) — the generic
 *   VSCode-style emitter accessor type. `Event<T>` is the listener-tuple
 *   type used to declare `readonly onDidXxx: Event<T>`.
 * - `ProtocolEvent` (alias of `@multiai/protocol`'s `Event`) — the
