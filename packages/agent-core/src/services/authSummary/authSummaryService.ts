...
/** Wire name of the OAuth-managed MultiAI provider. */
const MANAGED_PROVIDER_NAME = 'managed:multiai';
...
export class AuthSummaryService
  ...
  private async _readConfig(): Promise<MultiAIConfig> {
    // `reload: true` forces MultiAICore to re-read `config.toml` from disk
    ...
    // MultiAICore's `this.config` only refreshes when something explicitly
