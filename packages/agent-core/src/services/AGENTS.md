...
In-process service layer for the MultiAI server, now hosted inside
`@multiai/agent-core` (merged from the former `@multiai/services`
...
from `index.ts` so importing `@multiai/agent-core` runs the registration
...
2. **Consumers seed from `getSingletonServiceDescriptors()` directly**.
   Importing `@multiai/agent-core` loads the package barrel, whose impl
