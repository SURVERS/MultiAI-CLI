 ...
 * role — routes `QuestionRequest`s coming out of `MultiAICore` to a waiter
 ...
 *      `BridgeClientAPI` sees from `MultiAICore.requestQuestion(...)`. See
...
export interface IQuestionService {
  ...
  /**
   * Called by the adapter when MultiAICore needs the user to answer a question.
