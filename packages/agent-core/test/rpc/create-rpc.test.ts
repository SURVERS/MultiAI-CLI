...
describe('createRPC', () => {
  ...
  it('binds prototype methods and rehydrates plain remote errors as MultiAIError(internal)', async () => {
    ...
    await expect(hostProxy.fail({ code: 'boom' })).rejects.toBeInstanceOf(MultiAIError);
    expect(hostImpl.approvals).toEqual(['approval-2']);
  });

  it('passes a thrown MultiAIError across the wire preserving code and details', async () => {
    ...
    expect(received).toBeInstanceOf(MultiAIError);
    const error = received as MultiAIError;
  ...
  it('serializes remote error details through JSON semantics before crossing the wire', async () => {
    ...
    await connectRemote({
      async fail() {
        ...
        throw new MultiAIError(ErrorCodes.INTERNAL, 'Remote failed', {
          details,
        });
      },
    });
  ...
  it('rehydrates provider.* codes as MultiAIError', async () => {
    interface RemoteSide {
    ...
    await connectRemote({
      async callProvider() {
        throw new MultiAIError(ErrorCodes.PROVIDER_RATE_LIMIT, 'Upstream rate limit', {
  ...
  it('collapses non-Error throws to MultiAIError(internal)', async () => {
    interface RemoteSide {
    ...
    await expect(remoteProxy.misbehave({})).rejects.toBeInstanceOf(MultiAIError);
  });
});
