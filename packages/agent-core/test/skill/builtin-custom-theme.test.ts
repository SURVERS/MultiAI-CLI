...
describe('builtin skill: custom-theme', () => {
  ...
  it('pins the docs token reference and points users at MULTIAI_HOME/themes and /theme', () => {
    ...
    expect(content).toContain('<MULTIAI_HOME>/themes');
  ...
  it('registers through registerBuiltinSkills but stays out of the model skill listing', () => {
    ...
    expect(registry.getMultiAISkillsDescription()).toContain('custom-theme');
