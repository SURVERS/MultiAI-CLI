/**
 * Scenario: assembling, buffering, disabling, and recovering MultiAI telemetry events.
 * Responsibilities: preserve payload contracts while keeping network delivery disabled by default.
 * Wiring: real clients/transports use isolated files, fake HTTP, and child processes at boundaries.
 * Run: pnpm exec vitest run packages/telemetry/test/telemetry.test.ts
 */
...
async function tempHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'multiai-telemetry-'));
  tempDirs.push(dir);
  return dir;
}
...
describe('payload assembly', () => {
  it('adds server event prefix, payload user id, and flattened fields', () => {
    ...
    expect(payload.user_id).toBe('multiai_device_id_device-1');
  ...
  it('does not double-prefix already-prefixed events', () => {
    const payload = buildPayload([sampleEvent('multiai_started')], 'device-1');

    expect(payload.events[0]?.['event']).toBe('multiai_started');
  });
...
describe('server prefix application', () => {
  it('locks the outbound telemetry prefixes', () => {
    expect(SERVER_EVENT_PREFIX).toBe('multiai_');
    expect(USER_ID_PREFIX).toBe('multiai_device_id_');
  });
...
describe('AsyncTransport', () => {
  ...
  it('retries disk events through the outbound pipeline and deletes the file on success', async () => {
    ...
    expect(payload.events[0]?.['event']).toBe('multiai_from_disk');
  ...
  it('writes one JSONL line per event and keeps raw event names on disk fallback', async () => {
    ...
    expect(file).not.toContain('multiai_first');
  });
...
describe('telemetry bootstrap', () => {
  ...
  it('keeps singleton network delivery disabled without a configured endpoint', async () => {
    const fetchImpl = vi.fn(async () => new Response('', { status: 200 }));
    vi.stubGlobal('fetch', fetchImpl);
