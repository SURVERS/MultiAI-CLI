...
import { MultiAIError, ErrorCodes, type Event, type MultiAIHarness, type Session } from '@multiai/sdk';
...
describe('AcpServer session/load replay', () => {
  ...
  it('maps the SDK session.not_found error to ACP invalid_params (-32602)', async () => {
    const harness = makeHarness({
      hasUsableToken: true,
      resumeError: new MultiAIError(ErrorCodes.SESSION_NOT_FOUND, 'Session "ghost" was not found'),
    });
    const { agentStream, clientStream } = makeInMemoryStreamPair();
    new AgentSideConnection((c) => new AcpServer(harness, c), agentStream);
    const clientConn = new ClientSideConnection((_a) => new CapturingClient(), clientStream);
