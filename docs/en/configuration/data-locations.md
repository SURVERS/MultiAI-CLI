...
MultiAI CLI stores all runtime data — the config file, session history, login credentials, and diagnostic logs — under `~/.multiai/`. This page helps you understand where each type of data lives, what it is for, and how to clean up or relocate it when needed.

## Data root directory

The default data root is `~/.multiai/`. The actual path varies by platform:

- macOS: `/Users/<name>/.multiai`
- Linux: `/home/<name>/.multiai`
- Windows: `C:\Users\<name>\.multiai`

If you need to move the data directory elsewhere (for example, to isolate configs for different projects with independent environments), set `MULTIAI_HOME`:

```sh
export MULTIAI_HOME="$HOME/.config/multiai"
```

Once set, **all** MultiAI CLI data — config, sessions, logs, OAuth credentials, MultiAI-specific user Skills, global `AGENTS.md`, and more — lands under the new path. For the full reference on `MULTIAI_HOME`, see [Environment variables](./env-vars.md).

::: tip Note

**Generic `.agents` resources** stay under the real OS home so they can be shared across tools. For example, user-level generic Skills remain at `~/.agents/skills/`, while MultiAI-specific user Skills move with `MULTIAI_HOME` as `$MULTIAI_HOME/skills/`.
:::
...
├── AGENTS.md               # Global MultiAI-specific agent instructions (optional)
├── mcp.json                # User-level MCP server declarations (optional)
├── skills/                 # MultiAI-specific user-level Skills (optional)
├── plugins/
...
├── logs/
│   └── multiai.log       # Global diagnostic log
├── updates/
│   ├── latest.json
│   ├── install.json
│   ├── install.lock
...
- **`AGENTS.md`**: global MultiAI-specific agent instructions. This file moves with `MULTIAI_HOME`; generic cross-tool instructions can still live under `~/.agents/AGENTS.md`.
- **`mcp.json`**: user-level MCP server declarations, merged with the project-local `.multiai/mcp.json` on startup. See [MCP](../customization/mcp.md).
- **`skills/`**: MultiAI-specific user-level Skills. This directory moves with `MULTIAI_HOME`; generic cross-tool Skills can still live under `~/.agents/skills/`. See [Agent Skills](../customization/skills.md).
...
- **`logs/multiai.log`**: diagnostic log for this session; only present when a diagnostic event occurs.
- **`tasks/`**: background task persistence — `tasks/<task_id>.json` stores status/pid/exit code; `tasks/<task_id>/output.log` stores output.
- **`cron/`**: scheduled task persistence; reloaded into the scheduler when the session is resumed with `multiai --session`. See [Scheduled tasks](../reference/tools.md#scheduled-tasks).
...
- **`logs/multiai.log`** (global): records startup, login, export, and other cross-session events.
- **`<sessionDir>/logs/multiai.log`** (session-level): records diagnostic events within a single session.

When reporting a bug, prefer exporting the relevant session with `multiai export` (see [multiai command](../reference/multiai-command.md)); the session log is included in the export by default. Add `--no-include-global-log` if you do not want to share the global log.
...
Deleting the data root directory (`~/.multiai/` or the path set by `MULTIAI_HOME`) removes all runtime data. To clear only part of the data:

| Goal | Action |
| --- | --- |
| Reset configuration | Delete `~/.multiai/config.toml` |
| Reset terminal UI preferences | Delete `~/.multiai/tui.toml` |
| Clear all sessions | Delete `~/.multiai/sessions/` and `session_index.jsonl` |
| Clear diagnostic logs | Delete `~/.multiai/logs/` |
| Clear input history | Delete `~/.multiai/user-history/` |
| Reset update state | Delete `~/.multiai/updates/latest.json` |
| Force re-download of managed `rg` and `fd` | Delete `~/.multiai/bin/` |
...
| Remove user-level MCP declarations | Delete `$MULTIAI_HOME/mcp.json` (default `~/.multiai/mcp.json`) |
| Clear global MultiAI-specific agent instructions | Delete `$MULTIAI_HOME/AGENTS.md` (default `~/.multiai/AGENTS.md`) |
| Clear plugin install records | Delete `$MULTIAI_HOME/plugins/` (local plugin source directories are not affected) |
| Clear MultiAI-specific user-level Skills | Delete `$MULTIAI_HOME/skills/` (default `~/.multiai/skills/`) |

## Next steps

- [Configuration files](./config-files.md) — full reference for `config.toml` fields
- [Environment variables](./env-vars.md) — detailed usage of `MULTIAI_HOME` and related path variables
