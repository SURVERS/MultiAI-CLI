# `multiai acp` subcommand

`multiai acp` switches MultiAI CLI to **ACP (Agent Client Protocol)** mode: it communicates with an ACP client (such as Zed, JetBrains AI Chat, etc.) via JSON-RPC over stdin/stdout, letting the IDE directly drive MultiAI CLI's sessions, prompts, and tool calls.

```sh
multiai acp
```

Once started, the command prints no banner and immediately waits for the ACP client to send an `initialize` request on stdin. Logs are written to stderr (as well as the diagnostic log under `~/.multiai/logs/`), so the ACP channel itself stays clean.

::: tip Who calls this?
You typically do not need to run `multiai acp` manually — this command is the subprocess entry point for IDEs. For IDE-side configuration, see [Using in IDEs](../guides/ides.md).
:::
...
| --- | --- | --- |
| `initialize` | Yes | Version negotiation; returns `agentInfo: { name: 'MultiAI CLI', version }`, capability matrix, and `authMethods` |
...
- `http` → MultiAI CLI's `transport: 'http'` configuration
- `stdio` → MultiAI CLI's `transport: 'stdio'` configuration
- `sse` → MultiAI CLI's `transport: 'sse'` configuration
- `acp` → discarded with a warn log entry

## Next steps

- [Using in IDEs](../guides/ides.md) — Zed / JetBrains configuration steps and troubleshooting
- [`multiai` Command Reference](./multiai-command.md) — Complete subcommand list
