...
`multiai` provides `login`, `logout`, `account` (alias `profile`), `acp`, `web`, `doctor`, `export`,
...
Show the current profile, wallet balance and breakdown, subscription state, usage quotas,
granted scopes, connection details, and masked API keys. `multiai profile` is an alias;
use `--json` for the raw account snapshot:
