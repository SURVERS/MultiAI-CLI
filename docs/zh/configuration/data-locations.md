...
MultiAI CLI 把所有运行时数据——配置文件、会话历史、登录凭据、诊断日志——集中存放在 `~/.multiai/` 下。本页帮你搞清楚每类数据在哪里、用来做什么，以及需要时怎么清理或搬迁。

## 数据根目录

默认数据根是 `~/.multiai/`，在不同平台的实际路径：

- macOS：`/Users/<name>/.multiai`
- Linux：`/home/<name>/.multiai`
- Windows：`C:\Users\<name>\.multiai`

如果你需要把数据目录挪到别处（比如用多个独立环境隔离不同项目的配置），设置 `MULTIAI_HOME` 即可：

```sh
export MULTIAI_HOME="$HOME/.config/multiai"
```

设置后，配置、会话、日志、OAuth 凭据、MultiAI 专属用户级 Skills、全局 `AGENTS.md` 等 **MultiAI CLI 数据**都会落到新路径下。`MULTIAI_HOME` 的完整说明见[环境变量](./env-vars.md)。

::: tip 提示

**通用 `.agents` 资源**仍放在真实 OS home 下，以便跨工具共享。例如，用户级通用 Skills 仍位于 `~/.agents/skills/`，而 MultiAI 专属用户级 Skills 会随 `MULTIAI_HOME` 移动到 `$MULTIAI_HOME/skills/`。
:::

## 目录结构

```
$MULTIAI_HOME  （默认 ~/.multiai）
├── config.toml             # 用户配置
├── tui.toml                # 终端界面偏好（含自动更新开关）
├── AGENTS.md               # 全局 MultiAI 专属 Agent 指令（可选）
├── mcp.json                # 用户级 MCP server 声明（可选）
├── skills/                 # MultiAI 专属用户级 Skills（可选）
├── plugins/
...
- **`AGENTS.md`**：全局 MultiAI 专属 Agent 指令。该文件会随 `MULTIAI_HOME` 移动；跨工具通用指令仍可放在 `~/.agents/AGENTS.md`。
- **`mcp.json`**：用户级 MCP server 声明，启动时与项目内的 `.multiai/mcp.json` 合并加载。详见 [MCP](../customization/mcp.md)。
- **`skills/`**：MultiAI 专属用户级 Skills。该目录会随 `MULTIAI_HOME` 移动；跨工具通用 Skills 仍可放在 `~/.agents/skills/`。详见 [Agent Skills](../customization/skills.md)。
...
- **`logs/multiai.log`**：该会话的诊断日志，只有发生诊断事件时才存在。
- **`tasks/`**：后台任务持久化——`tasks/<task_id>.json` 保存状态/pid/退出码，`tasks/<task_id>/output.log` 保存输出。
- **`cron/`**：定时任务持久化，用 `multiai --session` 恢复会话时重新加载到调度器。详见[定时任务](../reference/tools.md#定时任务)。
...
- **`logs/multiai.log`**（全局）：记录启动、登录、导出等跨会话事件。
- **`<sessionDir>/logs/multiai.log`**（会话级）：记录单个会话内的诊断事件。

报 bug 时，优先用 `multiai export` 导出相关会话（详见 [multiai 命令](../reference/multiai-command.md)）；会话日志默认包含在导出包里。不想分享全局日志时加 `--no-include-global-log`。
...
删除数据根目录（`~/.multiai/` 或 `MULTIAI_HOME` 指定路径）可清除所有运行时数据。只需清理部分内容时：

| 需求 | 操作 |
| --- | --- |
| 重置配置 | 删除 `~/.multiai/config.toml` |
| 重置终端界面偏好 | 删除 `~/.multiai/tui.toml` |
| 清理所有会话 | 删除 `~/.multiai/sessions/` 和 `session_index.jsonl` |
| 清理诊断日志 | 删除 `~/.multiai/logs/` |
| 清理输入历史 | 删除 `~/.multiai/user-history/` |
| 重置更新状态 | 删除 `~/.multiai/updates/latest.json` |
| 强制重新下载托管 `rg` 和 `fd` | 删除 `~/.multiai/bin/` |
| 清除供应商 OAuth 登录态 | 运行 `/logout`，或删除对应的 `credentials/<name>.json` |
| 清除 MCP server OAuth 登录态 | 删除 `credentials/mcp/`（`/logout` 不会清理 MCP 凭据） |
| 移除用户级 MCP 声明 | 删除 `$MULTIAI_HOME/mcp.json`（默认为 `~/.multiai/mcp.json`） |
| 清理全局 MultiAI 专属 Agent 指令 | 删除 `$MULTIAI_HOME/AGENTS.md`（默认为 `~/.multiai/AGENTS.md`） |
| 清理 plugin 安装记录 | 删除 `$MULTIAI_HOME/plugins/`（本地 plugin 源码不受影响） |
| 清空 MultiAI 专属用户级 Skills | 删除 `$MULTIAI_HOME/skills/`（默认为 `~/.multiai/skills/`） |

## 下一步

- [配置文件](./config-files.md) — `config.toml` 各字段的完整说明
- [环境变量](./env-vars.md) — `MULTIAI_HOME` 等路径变量的详细用法
