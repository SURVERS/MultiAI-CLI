...
`multiai` 是 MultiAI CLI 的主命令，用于在终端中启动一次交互式会话。不带任何参数运行时，它会在当前工作目录下开启一个新会话；配合不同的 flag，可以续上历史会话、跳过审批、从 Plan 模式开始，或者指定自定义的 Skills 目录。
...
所有 flag 都是可选的，直接运行 `multiai` 即可进入交互式会话：
...
| `--agent <name>` | | 以指定 Agent 作为主 Agent 启动会话（仅实验性 `multiai -p`） |
| `--agent-file <path>` | | 从 Markdown 文件加载自定义 Agent（仅本次启动、仅实验性 `multiai -p`）并选中它。不可重复传入，也不能与 `--agent` 同时使用 |
...
恢复会话时，可以通过 `--auto`、`--yolo` 或 `--plan` 覆盖原会话保存的权限或计划模式。例如，`multiai --continue --auto` 会恢复最近会话并切换到 auto 权限模式。
...
`--agent` 和 `--agent-file` 用于选择驱动会话的 Agent。目前二者仅在 `MULTIAI_EXPERIMENTAL_FLAG=1` 时的 `multiai -p` 下可用，其他启动方式会以明确错误拒绝：

```sh
MULTIAI_EXPERIMENTAL_FLAG=1 multiai -p --agent reviewer "审查这个分支上的改动"
```
...
`multiai` 提供 `login`、`logout`、`account`、`acp`、`web`、`doctor`、`export`、
`upgrade`、`vis` 与 `provider` 子命令。旧版迁移与以前的 `server` 命令树不再提供。

### `multiai login`

无需进入 TUI 即可登录 MultiAI 账户。默认使用浏览器 Authorization Code + PKCE；
`--device` 改用 Device Authorization + PKCE。Refresh token 默认存入系统 keyring，
`--session-only` 则明确选择仅限当前进程的会话。
...
| 选项 | 说明 |
| --- | --- |
| `--device` | 显示 device code，不打开 loopback 浏览器回调 |
| `--session-only` | 会话只保留到进程退出；keyring 不可用时，非交互式运行必须明确传入 |

OAuth 应用设置、token 行为与账户特殊情况见 [OAuth 与账户](../guides/account-and-oauth.md)。

### `multiai logout`

尽可能撤销 refresh token，然后始终清除本地会话与托管 MultiAI 模型：
...
显示当前资料、wallet、订阅限额、已授予 scopes 与脱敏 API 密钥。使用 `--json`
输出原始账户快照：
...
把 MultiAI CLI 切换到 ACP（Agent Client Protocol）模式，在标准输入/输出上以 JSON-RPC 形式与 IDE 对话，让编辑器直接驱动 multiai 的会话和工具调用。通常不需要手动运行——IDE 会把它作为子进程入口启动。配置方式见[在 IDE 中使用](../guides/ides.md)，技术细节见 [multiai acp 参考](./multiai-acp.md)。
...
在当前终端前台运行本地 MultiAI 服务 —— 同一个进程同时挂载 REST + WebSocket API 与 web UI —— 并在服务就绪后用默认浏览器打开 web UI。命令会一直挂在终端，直到收到 `SIGINT` / `SIGTERM`（如 `Ctrl-C`）时干净退出。

服务运行时，`GET /openapi.json` 会返回 REST OpenAPI 文档，`GET /asyncapi.json` 会返回本地 WebSocket 协议的 AsyncAPI 文档。

```sh
multiai web                 # 前台运行服务并打开浏览器
multiai web --no-open       # 不打开浏览器
multiai web --port 58628    # 指定绑定端口
```

同一 home 目录下可以同时运行多个实例：每个实例注册到 `~/.multiai/server/instances/`，端口被占用时自动 +1 重试（58628、58629……）。
...
`multiai web` 默认只绑定本机 loopback 地址，并在启动横幅中打印 bearer token；web UI 通过 URL 的 `#token=` 片段自动完成鉴权。
...
生成新的持久化 bearer token（写入 `~/.multiai/server.token`），旧 token 立即失效。token 是整个 home 目录共享的，所有运行中的实例会在下一次鉴权校验时自动换用新 token，无需重启。

### `multiai doctor`

校验 `config.toml` 和 `tui.toml`，不会启动 TUI，也不会修改任一文件。默认检查 `MULTIAI_HOME` 下的文件；未设置该环境变量时检查 `~/.multiai`。默认路径缺失时会显示为跳过，因为内置默认值仍可生效。
...
| 命令 | 说明 |
| --- | --- |
| `multiai doctor` | 校验默认 `config.toml` 和 `tui.toml` |
| `multiai doctor config [path]` | 只校验 `config.toml`；传入 `path` 时使用该文件而不是默认文件 |
| `multiai doctor tui [path]` | 只校验 `tui.toml`；传入 `path` 时使用该文件而不是默认文件 |
...
导出包含目标会话目录内的所有文件。全局诊断日志（`~/.multiai/logs/multiai.log`）默认包含，因为它可能含有其他会话或项目的事件；不想分享时加 `--no-include-global-log`。
...
立即检查最新版本并展示更新提示，选择操作后退出。也可以使用别名 `multiai update`。

```sh
multiai upgrade
```

对全局 npm、pnpm、yarn、bun 以及 macOS / Linux native 安装，`multiai upgrade` 会展示更新选项；选择 `Install update now` 后运行对应的前台安装命令。当前安装方式无法自动升级时（如 Windows native 安装），改为打印手动更新命令。
...
| --- | --- |
| `<url>` | Registry 地址 |
| `--api-key <key>` | 访问 registry 时携带的 Bearer token。未传时回退到环境变量 `MULTIAI_REGISTRY_API_KEY`，必填 |
...
| `--api-key <key>` | 供应商 API key。未传时回退到 `MULTIAI_REGISTRY_API_KEY`，必填 |
...
```sh
multiai provider catalog list anthropic          # 先看可选的模型
