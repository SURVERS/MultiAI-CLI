...
import MultiAILogo from './MultiAILogo.vue'
...
const copy = computed(() => isZh.value
  ? {
      ...
      secondaryHref: 'https://github.com/SURVERS/MultiAI-CLI',
    }
  : {
      titleLead: 'Kimi',
      titleAccent: 'Code',
...
<template>
  <section class="MultiAIHero">
    <div class="MultiAIHero__halo" aria-hidden="true" />
    <div class="MultiAIHero__inner">
      <div class="MultiAIHero__logo">
        <MultiAILogo :size="64" />
      </div>
      <h1 class="MultiAIHero__title">
        {{ copy.titleLead }}&nbsp;<span class="MultiAIHero__accent">{{ copy.titleAccent }}</span>{{ copy.titleTail }}
      </h1>
      <p class="MultiAIHero__tagline">{{ copy.tagline }}</p>
      <div class="MultiAIHero__actions">
        <a class="MultiAIBtn MultiAIBtn--primary" :href="withBase(copy.primaryHref)">
          {{ copy.primaryText }}
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
            <path d="M6 3l5 5-5 5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </a>
        <a class="MultiAIBtn MultiAIBtn--ghost" :href="copy.secondaryHref" target="_blank" rel="noopener">
...
.MultiAIHero__accent {
  background: var(--multiai-brand-gradient);
...
@media (max-width: 480px) {
  ...
  .MultiAIHero__actions .MultiAIBtn {
    width: 100%;
  }
}
</style>
