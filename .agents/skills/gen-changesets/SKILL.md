---
name: gen-changesets
description: Use when generating changesets in the MultiAI CLI repository, including package bump selection, internal package and CLI bundle handling, bump levels, major confirmation, and English changelog wording.
---

# Generate Changesets

`MultiAI CLI` uses changesets to manage versions and changelogs. The current user-facing published package is:

- `multiai-cli`: the CLI

All other `@multiai/*` packages are treated as internal packages, including `@multiai/sdk`, `agent-core`, `kosong`, `kaos`, `oauth`, and `telemetry`.

`@multiai/pi-tui` is a special internal package: it is a private fork (`private: true`) that is never published, but it keeps its own changelog through changesets. It is an exception to Core Rule 4 — see the dedicated section below.
...
3. **Map ignored internal changes to the affected released package.** If an ignored internal package changes CLI output or behavior, list `multiai-cli` and describe the actual user-visible or release-artifact change in the changelog text.
4. **Internal package source changes that enter the CLI bundle must manually list the CLI — when they get a changeset at all.** `multiai-cli` inline-bundles `@multiai/*` source, but those internal packages are devDependencies from the CLI's perspective, so changesets will not automatically propagate bumps. If a change enters the CLI output and is user-perceivable, list `multiai-cli`. See rule 6 for when to skip the changeset entirely.
   - **Web app (`@multiai/web`) changes always enter the CLI bundle.** `@multiai/web` is ignored by changesets (see `.changeset/config.json`) and cannot be mixed with `multiai-cli` in one changeset frontmatter. Describe the web change in the changelog text, but list `multiai-cli` so the CLI release carries the bundled `dist-web` output.
...
6. **Skip changes users cannot perceive — write no changeset at all.** The CLI changelog is user-facing; a changeset is a changelog entry, not a shipping gate. Internal changes merged to `main` still ship in the next release triggered by any user-facing changeset, so skipping the changeset loses nothing. Do not write changesets for:
   ...
   - `kap-server` WebSocket / REST protocol changes consumed only by the bundled web UI, multiai-inspect, or other dev tooling (new endpoints, subscribe protocols, stream baselines). A web-facing feature they back gets its own `web:` entry instead.
   - Behavior that only takes effect on the experimental engine (e.g. experimental `multiai -p`), unless it exposes documented user configuration such as a `config.toml` section or env vars that also work on a shipped surface (TUI or `multiai web`).
   - When unsure whether users can perceive a change, ask before writing.
7. `@multiai/vis` / `vis-server` / `vis-web` are ignored by changesets and should not be handled. `@multiai/inspect` (a private dev app that never ships) is likewise ignored and must never appear in a changeset frontmatter.
...
4. If an ignored internal package change enters the CLI bundle, put `multiai-cli` in frontmatter instead of mixing the ignored package into the same changeset.
...
- **For new user-facing features, append a brief usage hint** so users know how to try it. Keep it to a single short line — a command name, a subcommand, a flag, or a one-line "how to use". Do not explain design rationale or list edge cases. Skip the hint for bug fixes, internal changes, and refactors.
  ...
  - CLI subcommand: `Add the multiai web subcommand to open the web UI. Run multiai web to launch it.`
...
Add the multiai web subcommand to open the web UI. Run multiai web to launch it.
```
...
`@multiai/web` is ignored by changesets and must **never** appear in a changeset frontmatter. Because the web app is bundled into the CLI release artifact, any web change that ships must list `multiai-cli` instead and describe the actual web-facing change in the text.

- Prefix the changelog entry text with `web: ` (for example `web: Fix the chat not scrolling to the bottom after sending a message.`) so the synced docs changelog can mark web UI entries. Apply this whenever the change is to the web project (`@multiai/web`).
...
## `@multiai/pi-tui` changes

`@multiai/pi-tui` is a vendored fork that lives in `packages/pi-tui`. It is `private: true` and is never published, but it is **not** ignored by changesets: changesets versions it and writes `packages/pi-tui/CHANGELOG.md` so the fork keeps its own history. Because it is bundled into the CLI like other internal packages, it is an exception to Core Rule 4 — do **not** list `multiai-cli` for a change that only touches pi-tui.

- Changes that only affect pi-tui (build, package, strict-mode cleanup, renderer fixes): list `@multiai/pi-tui` only. No CLI changeset.
- If the same change is also user-visible in the CLI (for example a terminal rendering fix that CLI users can see), add a **separate** changeset that lists `multiai-cli` with CLI-focused wording, in addition to the pi-tui changeset. Do not mix both packages in one frontmatter — the two changelogs need different wording.

pi-tui-only change:

```markdown
---
"@multiai/pi-tui": patch
---
...
- Internal package source enters the CLI bundle, but `multiai-cli` is missing.
- A changeset frontmatter mixes ignored internal packages with non-ignored packages.
- `packages/node-sdk` was not changed, but `@multiai/sdk` was listed for "internal package sync".
...
- A change that only touches `@multiai/pi-tui` lists `multiai-cli` instead of `@multiai/pi-tui`, or mixes both packages in one frontmatter.
