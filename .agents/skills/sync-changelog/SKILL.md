---
name: sync-changelog
description: Use after a release succeeds, when maintainers need to sync apps/multiai-cli/CHANGELOG.md into docs/en/release-notes/changelog.md and docs/zh/release-notes/changelog.md, then open a PR on a dedicated branch.
---
...
`MultiAI CLI` uses changesets for versioning. Each package gets its own `CHANGELOG.md`. The user-facing CLI package, `@multiai/cli`, writes its changelog here:

```text
apps/multiai-cli/CHANGELOG.md
```
...
- The top of `apps/multiai-cli/CHANGELOG.md` contains version blocks that are not yet in `docs/en/release-notes/changelog.md`.
- The `gen-docs` flow does not run this automatically; maintainers must explicitly do it after release.

Do **not** run this before the Release PR is merged. At that point, changesets has not yet written the new version into `apps/multiai-cli/CHANGELOG.md`.

## Source And Targets

| File | Role | Edited by |
|---|---|---|
| `apps/multiai-cli/CHANGELOG.md` | **Only upstream source**, generated by changesets | Never edit manually |
...
- The released version exists on npm (`npm view @multiai/cli versions --json`) or has a matching GitHub Release tag.
- The top of `apps/multiai-cli/CHANGELOG.md` is that new version.
...
```bash
rg '^## ' apps/multiai-cli/CHANGELOG.md | head -5
...
- The upstream H1 `# @multiai/cli` because the docs page already has `# Changelog`.
...
Drop SDK-only and provider-internal detail. This changelog serves `@multiai/cli` CLI and web users. Within an entry, keep only what CLI/web users can perceive, and remove sentences that document internals instead of user-visible behavior. Apply this on both the English and Chinese pages:
...
This page documents the changes in each MultiAI CLI release.
```
...
```bash
git log -1 --format=%cs "@multiai/cli@<version>"
```
...
- **Keep usage hints to one short clause**.
  ...
  - Better: `例如 multiai web --allowed-host example.com。`
...
```markdown
- Add a --allowed-host flag to multiai web that lets extra Host header values pass the DNS-rebinding check, and include allow guidance in the 403 error message. Pass --allowed-host <host> to allow an extra host.
```

Before (literal, wordy):

```markdown
- 为 `multiai web` 新增 `--allowed-host` 标志，允许额外的 Host 请求头值通过 DNS 重绑定检查，并在 403 错误消息中包含允许指引。传入 `--allowed-host <host>` 以允许额外的 host。例如 `multiai web --allowed-host example.com`。
```

After (concise, idiomatic):

```markdown
- `multiai web` 新增 `--allowed-host <host>` 选项，可将指定 Host 加入 DNS 重绑定白名单；403 错误会提示如何通过 `--allowed-host` 或 `MULTIAI_ALLOWED_HOSTS` 放行，例如 `multiai web --allowed-host example.com`。
```
...
```text
docs(changelog): sync <version range> from apps/multiai-cli/CHANGELOG.md
```
...
- Synced `<version range>` from `apps/multiai-cli/CHANGELOG.md` into `docs/en/release-notes/changelog.md`
...
- Never edit upstream `apps/multiai-cli/CHANGELOG.md`.
- Do not backfill unreleased `.changeset/*.md` drafts into the docs site.
- Prefix web UI entries with `web: ` (when the upstream commit touches `apps/multiai-web/`), and keep the prefix on both the English and Chinese pages.
...
| Mistake | Fix |
|---|---|
| Adding entries directly to the English docs page without reading upstream | Use `apps/multiai-cli/CHANGELOG.md` as the source |
...
| Forgetting or translating the `web:` prefix on web UI entries | Prefix web UI entries (commit touches `apps/multiai-web/`) with `web: ` on both pages; keep the prefix as-is when translating |

## Stop Signals

- The top version in `apps/multiai-cli/CHANGELOG.md` is not published on npm or GitHub Releases.
- You are about to edit `apps/multiai-cli/CHANGELOG.md`.
