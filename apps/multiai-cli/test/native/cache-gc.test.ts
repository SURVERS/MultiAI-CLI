/**
 * Scenario: pruning stale native cache roots.
 * Responsibilities: retain the active and newest cache roots while removing older siblings.
 * Wiring: the real cache cleanup runs against isolated temporary directories.
 * Run: pnpm --filter @multiai/cli exec vitest run test/native/cache-gc.test.ts
 */
...
describe('cleanupStaleNativeCache', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'multiai-cache-gc-'));
  });
