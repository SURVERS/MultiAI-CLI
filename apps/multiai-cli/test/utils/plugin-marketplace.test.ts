...
import { MULTIAI_PLUGIN_MARKETPLACE_URL_ENV } from '#/constant/app';
...
describe('loadPluginMarketplace', () => {
  ...
  it('loads an explicitly configured remote marketplace', async () => {
    const source = 'https://plugins.example.test/marketplace.json';
  ...
  it('requires explicit marketplace configuration', async () => {
    const previous = process.env[MULTIAI_PLUGIN_MARKETPLACE_URL_ENV];
    delete process.env[MULTIAI_PLUGIN_MARKETPLACE_URL_ENV];
    ...
    try {
      ...
      ).rejects.toThrow(/No plugin marketplace is configured/);
      expect(fetchImpl).not.toHaveBeenCalled();
    } finally {
      if (previous === undefined) {
        delete process.env[MULTIAI_PLUGIN_MARKETPLACE_URL_ENV];
      } else {
        process.env[MULTIAI_PLUGIN_MARKETPLACE_URL_ENV] = previous;
      }
    }
  });

  it('does not fall back when an explicit marketplace fetch fails', async () => {
    const fetchImpl = vi.fn(async () => {
  ...
  describe('version derivation from a GitHub source', () => {
    ...
    it('does not derive a version from a non-GitHub URL', async () => {
      const entry = await loadEntry('https://plugins.example.test/curated/superpowers.zip');
      expect(entry.version).toBeUndefined();
    });
  ...
  it('accepts legacy marketplace type aliases as normal plugins', async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        plugins: [
          {
            id: 'multiai-webbridge',
            type: 'guide',
            displayName: 'MultiAI WebBridge',
            source: './multiai-webbridge',
    ...
    expect(marketplace.plugins).toContainEqual(
      expect.objectContaining({
        id: 'multiai-webbridge',
        source: join(dir, 'multiai-webbridge'),
      }),
    );
    expect(marketplace.plugins).toContainEqual(
      expect.objectContaining({
