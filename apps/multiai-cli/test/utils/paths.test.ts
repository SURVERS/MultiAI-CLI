...
describe('getDataDir', () => {
  it('returns ~/.multiai when MULTIAI_HOME is not set', () => {
    expect(getDataDir()).toBe(join(homedir(), '.multiai'));
  });

  it('returns MULTIAI_HOME when set', () => {
    process.env['MULTIAI_HOME'] = '/tmp/kimi-test-data';
    expect(getDataDir()).toBe('/tmp/kimi-test-data');
  });

  it('returns MULTIAI_HOME even if it is a relative path', () => {
    process.env['MULTIAI_HOME'] = 'relative/path';
...
describe('getLogDir', () => {
  it('returns <dataDir>/logs', () => {
    expect(getLogDir()).toBe(join(homedir(), '.multiai', 'logs'));
  });

  it('respects MULTIAI_HOME', () => {
    process.env['MULTIAI_HOME'] = '/z';
...
describe('getBinDir', () => {
  it('returns <dataDir>/bin', () => {
    expect(getBinDir()).toBe(join(homedir(), '.multiai', 'bin'));
  });

  it('respects MULTIAI_HOME', () => {
    process.env['MULTIAI_HOME'] = '/custom-bin-home';
...
describe('getUpdateStateFile', () => {
  it('returns <dataDir>/updates/latest.json', () => {
    expect(getUpdateStateFile()).toBe(join(homedir(), '.multiai', 'updates', 'latest.json'));
  });

  it('respects MULTIAI_HOME', () => {
    process.env['MULTIAI_HOME'] = '/updates-home';
...
describe('getUpdateInstallStateFile', () => {
  it('returns <dataDir>/updates/install.json', () => {
    expect(getUpdateInstallStateFile()).toBe(
      join(homedir(), '.multiai', 'updates', 'install.json'),
    );
  });
...
describe('getInputHistoryFile', () => {
  it('returns <dataDir>/user-history/<md5(workDir)>.jsonl', () => {
    ...
    expect(getInputHistoryFile(workDir)).toBe(
      join(homedir(), '.multiai', 'user-history', `${hash}.jsonl`),
    );
  });

  it('respects MULTIAI_HOME', () => {
    process.env['MULTIAI_HOME'] = '/custom/data';
