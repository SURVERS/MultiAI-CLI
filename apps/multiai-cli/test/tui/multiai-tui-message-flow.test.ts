...
import { MultiAITUI, type MultiAITUIStartupInput, type TUIState } from '#/tui/multiai-tui';
...
function makeStartupInput(): MultiAITUIStartupInput {
  return {
    cliOptions: {
      session: undefined,
      continue: false,
...
async function makeDriver(
  ...
  const driver = new MultiAITUI(harness as never, makeStartupInput()) as unknown as MessageDriver;
...
const originalKimiCodeHome = process.env['MULTIAI_HOME'];
const originalPluginMarketplaceUrl = process.env['MULTIAI_PLUGIN_MARKETPLACE_URL'];
...
afterEach(async () => {
  ...
  if (originalKimiCodeHome === undefined) {
    ...
    process.env['MULTIAI_HOME'] = originalKimiCodeHome;
  }
  ...
  if (originalPluginMarketplaceUrl === undefined) {
    delete process.env['MULTIAI_PLUGIN_MARKETPLACE_URL'];
  } else {
    process.env['MULTIAI_PLUGIN_MARKETPLACE_URL'] = originalPluginMarketplaceUrl;
  }
...
describe('MultiAITUI message flow', () => {
  ...
  it('tracks theme changes from slash commands', async () => {
    process.env['MULTIAI_HOME'] = await makeTempHome();
    const { driver, harness } = await makeDriver();
    harness.track.mockClear();
  ...
  it('dispatches /reload-tui without reloading the active session', async () => {
    ...
    process.env['MULTIAI_HOME'] = homeDir;
    await writeFile(
...
`,
  ...
  it('keeps in-product feedback disabled and points users to GitHub Issues', async () => {
    const { driver, harness } = await makeDriver(
      makeSession(),
      {
        getConfig: vi.fn(async () => ({
    ...
    expect(promptFeedbackAttachment).not.toHaveBeenCalled();
    ...
    const transcript = stripSgr(renderTranscript(driver));
    expect(transcript).toContain('In-product feedback is disabled.');
    expect(transcript).toContain('GitHub Issues');
  });

  it.skip('submits text feedback before preparing requested attachments', async () => {
    const { driver, harness } = await makeDriver(
      makeSession(),
      {
        getConfig: vi.fn(async () => ({
  ...
  it.skip('waits for the codebase upload to finish before returning', async () => {
    const { driver, harness } = await makeDriver(
      makeSession(),
      {
        getConfig: vi.fn(async () => ({
  ...
  it.skip('uploads session logs when codebase scanning fails but the session directory is available', async () => {
    const { driver, harness } = await makeDriver(
      makeSession(),
      {
        getConfig: vi.fn(async () => ({
  ...
  it.skip('tells the user when feedback is sent but codebase packaging fails', async () => {
    const { driver, harness } = await makeDriver(
      makeSession(),
      {
        getConfig: vi.fn(async () => ({
  ...
  it.skip('tells the user when the codebase upload fails', async () => {
    const { driver, harness } = await makeDriver(
      makeSession(),
      {
        getConfig: vi.fn(async () => ({
  ...
  it('removes debug timing status from undone turns', async () => {
    const { driver, session } = await makeDriver();
    const previousDebug = process.env['MULTIAI_DEBUG'];
    process.env['MULTIAI_DEBUG'] = '1';
    try {
      driver.handleUserInput('hello');
      driver.sessionEventHandler.handleEvent(
        {
      ...
      if (previousDebug === undefined) {
        delete process.env['MULTIAI_DEBUG'];
      } else {
        process.env['MULTIAI_DEBUG'] = previousDebug;
      }
    }
  });
  ...
  it('does not append a legacy feedback-upload hint beneath session errors', async () => {
    const { driver } = await makeDriver();
    ...
    expect(transcript).not.toContain('/export-debug-zip');
    expect(transcript).not.toContain("Please don't share it publicly");
  });
  ...
  it('renders /status using the active session runtime status', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(output).toContain('>_ MultiAI CLI');
  ...
  it('requires trust for remote plugins and does not apply legacy quota messaging', async () => {
    const session = makeSession({
      installPlugin: vi.fn(async () => ({
        id: 'example-plugin',
        ...
        hasErrors: false,
        source: 'zip-url',
        originalSource: 'https://plugins.example.test/example-plugin.zip',
      })),
    });
    const { driver } = await makeDriver(session);

    driver.handleUserInput('/plugins install https://plugins.example.test/example-plugin.zip');
  ...
  it('loads an explicitly configured local marketplace and confirms remote sources', async () => {
        ...
        plugins: [
          {
            ...
            source: 'https://plugins.example.test/example-plugin.zip',
          },
        ],
      }),
      'utf8',
    );
    process.env['MULTIAI_PLUGIN_MARKETPLACE_URL'] = marketplacePath;
    const session = makeSession();
    const { driver } = await makeDriver(session);
    ...
    await vi.waitFor(() => {
      expect(session.installPlugin).toHaveBeenCalledWith(
        'https://plugins.example.test/example-plugin.zip',
      );
    });
    await vi.waitFor(() => {
      const transcript = stripSgr(renderTranscript(driver));
  ...
  it('returns to the plugin list when a confirmed marketplace install fails', async () => {
    const marketplaceDir = await makeTempHome();
    const marketplacePath = join(marketplaceDir, 'marketplace.json');
    await writeFile(
      marketplacePath,
  ...
  it('removes a plugin record without auto-running any cleanup skill', async () => {
    ...
    driver.handleUserInput('/plugins remove multiai-webbridge');
    ...
    await vi.waitFor(() => {
      expect(session.removePlugin).toHaveBeenCalledWith('multiai-webbridge');
    });
    expect(session.activateSkill).not.toHaveBeenCalled();
  });

  it('does not load a default marketplace when none is configured', async () => {
    ...
    try {
      ...
      await vi.waitFor(() => {
        expect(stripSgr(panel.render(120).join('\n'))).toContain(
          'No plugin marketplace is configured',
        );
      });
  ...
  it('shows an inline Official error when the marketplace is unreachable, keeping the panel open', async () => {
    const originalFetch = globalThis.fetch;
    process.env['MULTIAI_PLUGIN_MARKETPLACE_URL'] = 'https://example.test/marketplace.json';
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
  ...
  it('applies /model selection with inline thinking state', async () => {
    ...
    expect(pickerOutput).toMatch(/Kimi K2\s+MultiAI CLI ← current/);
    expect(pickerOutput).toMatch(/❯ Kimi Turbo\s+MultiAI CLI/);
  ...
  it('forks the active session and switches to the returned session', async () => {
    ...
    try {
      ...
      expect(driver.state.transcriptContainer.render(120).join('\n')).toContain(
        'Session forked (ses-fork). To return to the original session: multiai -r ses-source',
      );
    } finally {
      process.title = originalTitle;
    }
...
describe('transcript step and assistant folding', () => {
  // Each cycle is one agent step: the model answers, calls a tool, and the
  // result comes back. The step boundary matters — text within a step merges
  // into one block, so without it every cycle would land in the same block.
  ...
  function mountedAssistants(driver: MessageDriver): AssistantMessageComponent[] {
    return driver.state.transcriptContainer.children.filter(
      (child): child is AssistantMessageComponent => child instanceof AssistantMessageComponent,
    );
  }

  function assistantTexts(driver: MessageDriver): string[] {
    driver.streamingUI.flushNow();
    return mountedAssistants(driver).map((assistant) =>
      stripSgr(assistant.render(120).join('\n')).trim(),
    );
  }

  function sendDelta(driver: MessageDriver, delta: string, step: number): void {
    driver.streamingUI.setStep(step);
    driver.sessionEventHandler.handleEvent(
      { type: 'assistant.delta', agentId: 'main', sessionId: 'ses-1', turnId: 1, delta } as Event,
      vi.fn(),
    );
  }

  function sendToolCall(driver: MessageDriver, toolCallId: string): void {
    driver.sessionEventHandler.handleEvent(
      {
        type: 'tool.call.started',
        agentId: 'main',
  ...
  it('merges text emitted around a tool call in the same step into one block', async () => {
    const { driver } = await makeDriver();
    driver.handleUserInput('прочитай воркспейс');

    // One assistant message that interleaves text and tool calls: the model
    // stops mid-sentence, streams the tool arguments, then finishes the
    // sentence. Both halves belong to the same message.
    sendDelta(driver, 'Сейчас осмотрю структуру воркспейса и', 1);
    sendToolCall(driver, 'call_read_1');
    sendDelta(driver, ' ключевые файлы.', 1);

    const texts = assistantTexts(driver);
    expect(texts).toHaveLength(1);
    expect(texts[0]).toBe(`${STATUS_BULLET}Сейчас осмотрю структуру воркспейса и ключевые файлы.`);
  });

  it('does not inject a separator when a tool call interrupts the stream mid-word', async () => {
    const { driver } = await makeDriver();
    driver.handleUserInput('прочитай воркспейс');

    // Deltas are token-sized, so the interruption can land inside a word. The
    // halves must be concatenated verbatim — the stream already carries every
    // space the model emitted.
    sendDelta(driver, 'Читаю ключевые ф', 1);
    sendToolCall(driver, 'call_read_2');
    sendDelta(driver, 'айлы воркспейса.', 1);

    const texts = assistantTexts(driver);
    expect(texts).toHaveLength(1);
    expect(texts[0]).toBe(`${STATUS_BULLET}Читаю ключевые файлы воркспейса.`);
    expect(texts[0]).not.toContain('ф айлы');
  });

  it('opens a new block for the next step instead of merging across messages', async () => {
    const { driver } = await makeDriver();
    driver.handleUserInput('two steps');

    sendDelta(driver, 'шаг один', 1);
    sendToolCall(driver, 'call_read_4');
    sendDelta(driver, 'шаг два', 2);

    const texts = assistantTexts(driver);
    expect(texts).toHaveLength(2);
    expect(texts[0]).toContain('шаг один');
    expect(texts[1]).toContain('шаг два');
  });
});
