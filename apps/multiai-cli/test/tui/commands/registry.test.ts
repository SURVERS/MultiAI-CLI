...
describe('built-in slash command registry', () => {
  it('finds built-ins by name or alias', () => {
    ...
    expect(findBuiltInSlashCommand('profile')?.name).toBe('account');
    expect(findBuiltInSlashCommand('lang')?.name).toBe('language');
  ...
  it('keeps swarm mode changes and swarm tasks idle-only', () => {
    ...
    expect((swarm as MultiAISlashCommand).experimentalFlag).toBeUndefined();
  ...
  it('defaults commands without explicit availability to idle-only', () => {
    const command: MultiAISlashCommand = {
      name: 'example',
      aliases: [],
  ...
  it('sorts commands by priority descending and name ascending', () => {
    const commands: MultiAISlashCommand[] = [
  ...
  it('registers goal with subcommand-aware availability', () => {
    ...
    expect((goal as MultiAISlashCommand).experimentalFlag).toBeUndefined();
