...
describe('plugins selector dialogs', () => {
  it('does not implicitly trust remote plugin paths', () => {
    expect(pluginTrustLabel({
      id: 'example-official',
      displayName: 'Example Official',
      enabled: true,
      state: 'ok',
      skillCount: 0,
      mcpServerCount: 0,
      ...
      hasErrors: false,
      source: 'zip-url',
      originalSource: 'https://plugins.example.test/official/example.zip',
    })).toBe('third-party');
    expect(pluginTrustLabel({
      id: 'superpowers',
      displayName: 'Superpowers',
      ...
      hasErrors: false,
      source: 'zip-url',
      originalSource: 'https://plugins.example.test/curated/superpowers.zip',
    })).toBe('third-party');
    expect(pluginTrustLabel({
      id: 'demo',
      displayName: 'Demo',
    ...
    expect(pluginTrustLabel({
      ...
      hasErrors: false,
      source: 'local-path',
      originalSource: 'https://plugins.example.test/official/local',
    })).toBe('third-party');
  });

  it('does not recognize legacy CDN provenance as official', () => {
    const base = {
      id: 'kimi-datasource',
    ...
    expect(isOfficialPluginInstall({
      ...base,
      source: 'zip-url',
      originalSource: 'https://plugins.example.test/official/example-plugin.zip',
    })).toBe(false);
  ...
  it('does not trust legacy Kimi CDN sources', () => {
    expect(isOfficialPluginSource('https://plugins.example.test/official/example-plugin.zip')).toBe(false);
    // Curated and other Kimi CDN paths are not "official" for the install gate.
    expect(isOfficialPluginSource('https://plugins.example.test/curated/superpowers.zip')).toBe(false);
    expect(isOfficialPluginSource('https://plugins.example.test/foo.zip')).toBe(false);
    ...
    expect(isOfficialPluginSource('http://plugins.example.test/official/x.zip')).toBe(false);
  ...
  it('does not render a built-in promotion while the marketplace is loading', () => {
    const { panel } = makePanel({ initialTab: 'official' });
    const out = strip(renderRaw(panel));
    expect(out).not.toContain('WebBridge');
    expect(out).toContain('Loading marketplace');
  });

  it('does not render a built-in promotion when the marketplace errors', () => {
    const { panel } = makePanel({ initialTab: 'official' });
  ...
  it('installs the first catalog official entry on Enter', () => {
  ...
  it('keeps the only official entry selected when navigating down', () => {
    const { panel, onSelect } = makePanel({ initialTab: 'official' });
    panel.setMarketplace(marketplaceEntries, '/tmp/marketplace.json');
    panel.handleInput('\u001B[B');
    panel.handleInput('\r');
  ...
  it('renders catalog entries without built-in substitutions', () => {
    const entries = [
      {
        id: 'example-browser-plugin',
        tier: 'official' as const,
        displayName: 'Example Browser Plugin',
        source: 'https://plugins.example.test/browser.zip',
      },
      ...officialEntries,
    ];
    const { panel } = makePanel({ initialTab: 'official' });
    panel.setMarketplace(entries, '/tmp/marketplace.json');
    const out = strip(renderRaw(panel));
    expect(out.split('Example Browser Plugin').length - 1).toBe(1);
  });

  it('installs a third-party entry without special-casing its id', () => {
    const entries = [
      {
        id: 'example-browser-plugin',
        tier: 'curated' as const,
    ...
    panel.setMarketplace(entries, '/tmp/marketplace.json');
    const out = strip(renderRaw(panel));
    expect(out).toContain('Example Browser Plugin  install');
    panel.handleInput('\r');
    expect(onSelect).toHaveBeenCalledWith({
      kind: 'install',
  ...
  it('defaults the third-party install trust prompt to exit', () => {
    ...
    expect(out.some((line) => line.includes('MultiAI has not reviewed'))).toBe(true);
