...
describe('buildWeightedTips — weighted rotation', () => {
  it('repeats higher-priority tips more often (length = sum of weights)', () => {
    const seq = buildWeightedTips([
      { en: 'a', ru: 'а' }, // weight 1 (default)
      { en: 'b', ru: 'б', priority: 3 },
      { en: 'c', ru: 'в', priority: 2 },
    ]);

    const count = (t: string) => seq.filter((x) => x.en === t).length;
  ...
  it('keeps duplicates spread out — no tip sits next to itself', () => {
    ...
    for (let i = 1; i < seq.length; i++) {
      expect(seq[i]!.en).not.toBe(seq[i - 1]!.en);
    }
  });

  it('preserves array order when all weights are the default (1)', () => {
    const seq = buildWeightedTips([{ en: 'x', ru: 'х' }, { en: 'y', ru: 'у' }, { en: 'z', ru: 'з' }]);
    expect(seq.map((t) => t.en)).toEqual(['x', 'y', 'z']);
  });

  it('clamps non-positive / fractional priorities to a weight of at least 1', () => {
    const seq = buildWeightedTips([
      { en: 'a', ru: 'а', priority: 0 },
      { en: 'b', ru: 'б', priority: -5 },
      { en: 'c', ru: 'в', priority: 1.9 },
    ]);
    expect(seq).toHaveLength(3);
    expect(seq.map((t) => t.en).toSorted()).toEqual(['a', 'b', 'c']);
  });
});
