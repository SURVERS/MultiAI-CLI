import { visibleWidth, type TUI } from '@multiai/pi-tui';
...
describe('ToolCallComponent', () => {
  ...
  it('truncates a long file path from the head so the filename stays visible', () => {
    const longPath =
      'apps/multiai-cli/src/tui/components/messages/tool-renderers/long-path/example/final-file.ts';
    const component = new ToolCallComponent(
      {
        id: 'call_long_path',
        name: 'Read',
    ...
    expect(out).not.toContain('apps/multiai-cli/src/tui/components/messages/tool-renderers/long-pa…');
  });
    ...
    const component = new ToolCallComponent(
      {
        ...
        name: 'Read',
        args: { path: '/tmp/proj-a/apps/multiai-cli/src/main.ts' },
      },
      {
  ...
  it('renders a single foreground subagent without the generic Agent tool header', () => {
    ...
    component.appendSubToolCall({
      ...
      name: 'Read',
      args: { path: 'apps/multiai-cli/src/tui/utils/background-agent-status.ts' },
    });

    out = strip(component.render(120).join('\n'));
    expect(out).toContain('Explore Agent Running (explore project xxx) · 1 tool · 10s');
    expect(out).toContain('Using Read (apps/multiai-cli/src/tui/utils/background-agent-status.ts)');
