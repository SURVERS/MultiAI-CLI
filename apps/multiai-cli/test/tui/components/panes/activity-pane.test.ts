import { Text, visibleWidth } from '@multiai/pi-tui';
import { afterEach, describe, expect, it } from 'vitest';

import { ActivityPaneComponent } from '#/tui/components/panes/activity-pane';
import { setTuiLanguage } from '#/tui/i18n';
...
describe('ActivityPaneComponent', () => {
  ...
  it('renders the tip prefix in Russian', () => {
    setTuiLanguage('ru');
    const { spinner } = createMockSpinner('работаю');
    const component = new ActivityPaneComponent({
      mode: 'composing',
      spinner,
      tip: 'используйте /help',
    });

    expect(component.render(80).map((line) => line.trimEnd())).toEqual([
      '',
      'работаю · Совет: используйте /help',
    ]);
  });
