...
import { log, type GoalSnapshot } from '@multiai/sdk';
...
interface StartupInternalsDriver extends StartupDriver {
  start(): Promise<void>;
...
function makeStartupInput(
  cliOptions: Partial<MultiAITUIStartupInput['cliOptions']> = {},
  tuiConfig: Partial<MultiAITUIStartupInput['tuiConfig']> = {},
): MultiAITUIStartupInput {
  return {
    cliOptions: {
      session: undefined,
      continue: false,
...
function createResumeState(overrides: { permissionMode?: string; planMode?: boolean } = {}) {
  return {
    id: 'ses-latest',
    workDir: '/tmp/proj-a',
    sessionDir: '/tmp/proj-a/.multiai/sessions/ses-latest',
    createdAt: Date.now(),
    updatedAt: Date.now(),
    sessionMetadata: {},
    agents: {
...
function loginRequiredError(): Error & { readonly code: string } {
  return Object.assign(new Error('OAuth provider "managed:multiai" requires login.'), {
...
function makeDriver(harness: ReturnType<typeof makeHarness>, input: MultiAITUIStartupInput) {
  const driver = new MultiAITUI(harness as never, input) as unknown as StartupDriver;
...
describe('MultiAITUI startup', () => {
  ...
  it('does not resume a session from a different cwd and shows a cd hint', async () => {
    ...
    expect(driver.state.activeDialog).toBeNull();
    const expectedResumeCmd = `cd ${quoteShellArg('/tmp/proj-b')} && multiai --resume ${quoteShellArg('ses-other-cwd')}`;
  ...
  it('copies a shell-safe resume command for another cwd with metacharacters', async () => {
    ...
    expect(resumeSession).not.toHaveBeenCalled();
    const expectedResumeCmd = `cd ${quoteShellArg('/tmp/proj$(touch /tmp/pwned)')} && multiai --resume ${quoteShellArg('ses-other-cwd')}`;
    expect(copyTextToClipboardMock).toHaveBeenCalledWith(expectedResumeCmd);
    const transcript = driver.state.transcriptContainer.render(160).join('\n');
    expect(transcript).toContain(`To resume, run: ${expectedResumeCmd}`);
  });
  ...
  it('exits after picking another cwd from the startup picker', async () => {
    ...
    await expect((driver as unknown as StartupInternalsDriver).initMainTui()).resolves.toBe(false);
    await (driver as unknown as { bootstrapFromPicker(): Promise<void> }).bootstrapFromPicker();
  ...
  it('does not override active session thinking when configured thinking is enabled after OAuth login', async () => {
    ...
    expect(harness.track).not.toHaveBeenCalledWith('login', expect.anything());
  });
  ...
  it('logs login failures with session context', async () => {
    ...
    const session = makeSession();
    const loginError = new Error('Failed to list MultiAI CLI models (HTTP 402).');
    const harness = makeHarness(session, {
      auth: {
        status: vi.fn(async () => ({
          loggedIn: false,
    ...
    try {
      ...
      expect(warn).toHaveBeenCalledWith(
        'MultiAI login failed',
        expect.objectContaining({
          providerName: 'managed:multiai',
          sessionId: 'ses-1',
          error: expect.objectContaining({
            message: 'Failed to list MultiAI CLI models (HTTP 402).',
          }),
        }),
      );
    } finally {
  ...
  it('tracks logout after managed credentials and session state are cleared', async () => {
    ...
    const harness = makeHarness(session, {
      getConfig: vi.fn(async () => ({
        models: {
          k2: { provider: 'managed:multiai', model: 'moonshot-v1', maxContextSize: 100 },
        },
        providers: { 'managed:multiai': { type: 'multiai' } },
      })),
      auth: {
        status: vi.fn(async () => ({
          loggedIn: true,
    ...
    expect(harness.auth.logout).toHaveBeenCalledWith();
    ...
    expect(harness.track).not.toHaveBeenCalledWith('logout', expect.anything());
  });
  ...
  it('can log out a stale managed entry even after the OAuth token is gone', async () => {
    ...
    const harness = makeHarness(session, {
      ...
      auth: {
        // Token gone (e.g. keyring entry deleted) but the managed entry
  ...
  it('does not mount the footer when resuming a missing session fails', async () => {
    ...
    ) as unknown as StartupInternalsDriver;
