...
const ENABLED = process.env['MULTIAI_E2E'] === '1';
...
beforeEach(async () => {
  ...
  oldHome = process.env['MULTIAI_HOME'];
  oldLogLevel = process.env['MULTIAI_LOG_LEVEL'];
...
afterEach(async () => {
  ...
  if (oldHome === undefined) {
    ...
    process.env['MULTIAI_HOME'] = oldHome;
  }
  if (oldLogLevel === undefined) {
    delete process.env['MULTIAI_LOG_LEVEL'];
  } else {
    process.env['MULTIAI_LOG_LEVEL'] = oldLogLevel;
  }
...
describe.skipIf(!ENABLED)('local logging export e2e', () => {
  it('exports session log and global log by default, and allows skipping global log', async () => {
    const harness = createMultiAIHarness({
      homeDir,
      identity: createMultiAIHostIdentity('0.1.1'),
    });
    try {
...
async function runKimiExport(args: string[]): Promise<void> {
  ...
  registerExportCommand(program, {
    ...
    exit: (code: number): never => {
      throw new Error(`multiai export exited ${code}: ${stderr.join('')}`);
    },
  });
