/**
 * Real-LLM smoke. Opt-in only — runs when `MULTIAI_E2E_REAL=1` is set, so
 * `make test` / CI never touches the network. Goes through the SDK's
 * `MultiAIHarness` entry and round-trips one real prompt. Requires provider
 * credentials on disk.
 *
 * Env knobs:
 *   MULTIAI_E2E_REAL     — set to "1" to enable this suite
 *   MULTIAI_E2E_MODEL    — model alias override (default: config's default)
 *   MULTIAI_E2E_PROMPT   — prompt text (default: "Reply with a single word: hi")
 *   MULTIAI_E2E_WORKDIR  — workspace directory (default: /tmp/kimi-e2e)
 */

import { mkdirSync } from 'node:fs';
import process from 'node:process';

import { createMultiAIHarness, type Event } from '@multiai/sdk';
...
const ENABLED = process.env['MULTIAI_E2E_REAL'] === '1';
type TurnEndedEvent = Extract<Event, { readonly type: 'turn.ended' }>;

describe.skipIf(!ENABLED)('SDK e2e — real LLM smoke', () => {
  test(
    'round-trips a single prompt through MultiAIHarness',
    async () => {
      const workDir = process.env['MULTIAI_E2E_WORKDIR'] ?? DEFAULT_WORKDIR;
      const prompt = process.env['MULTIAI_E2E_PROMPT'] ?? DEFAULT_PROMPT;
      const modelAlias = process.env['MULTIAI_E2E_MODEL'];
