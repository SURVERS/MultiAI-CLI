/**
 * Tests for the `multiai web` Commander wiring and its subcommands.
 *
 ...
 * output, browser opening, and the rotate-token
...
describe('kimi web', () => {
  ...
  it('does not register the removed legacy server command', () => {
    const program = makeProgram();
    expect(program.commands.find((command) => command.name() === 'server')).toBeUndefined();
  });
});
...
describe('`kimi web` ready banner', () => {
  it('prints the TUI-style ready panel once listening', async () => {
    ...
    expect(plain).toContain('MultiAI server ready');
    ...
    // the MultiAI mark stays next to the title.
    expect(plain).not.toContain('╭');
    expect(plain).not.toContain('╰');
    expect(plain).toContain('▐█▛█▛█▌');
    expect(plain).toContain('▐█████▌');
    expect(plain).not.toContain('MultiAI server:');

    // Title is above the URLs; Logs/Stop are at the bottom.
    expect(plain.indexOf('MultiAI server ready')).toBeLessThan(plain.indexOf('Local:'));
  ...
  it('uses the TUI dark palette for the ready banner', async () => {
    ...
    expect(out).toContain(color.bold.hex(darkColors.primary)('MultiAI server ready'));
...
describe('ready banner reflects the bind class', () => {
  it('lists Local + Network addresses for a 0.0.0.0 bind (Vite-style)', async () => {
    ...
    expect(raw).toContain('MultiAI server ready');
...
describe('`kimi web` option threading', () => {
  ...
  it('prints the one-line ready line instead of the full banner with a non-default --log-level', async () => {
    ...
    const plain = stripAnsi(readStdout());
    expect(plain).toContain('MultiAI server: http://127.0.0.1:58627/#token=tok');
    expect(plain).not.toContain('MultiAI server ready');
