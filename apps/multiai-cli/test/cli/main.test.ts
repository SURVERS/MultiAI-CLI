...
describe('main entry command handling', () => {
  ...
  it('sets the process title during startup', () => {
    ...
    try {
      ...
      expect(process.title).toBe('multiai-cli');
    } finally {
  ...
  it('initializes and flushes telemetry around the upgrade command', async () => {
    ...
    expect(mocks.createMultiAIHarness).toHaveBeenCalledWith(expect.objectContaining({
      homeDir: '/tmp/multiai-home',
      telemetry: {
        track: mocks.track,
