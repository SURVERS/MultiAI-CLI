/**
 * `multiai provider` CLI unit tests. The handlers receive an injected `getHarness`
...
import type { MultiAIConfig } from '@multiai/sdk';
...
interface FakeHarness {
  ...
  setConfig: (patch: Partial<MultiAIConfig>) => Promise<MultiAIConfig>;
  removeProvider: (providerId: string) => Promise<MultiAIConfig>;
}

function makeHarness(initial: MultiAIConfig): {
  harness: FakeHarness;
  current: () => MultiAIConfig;
  setConfigCalls: Array<Partial<MultiAIConfig>>;
  removeCalls: string[];
} {
  ...
  // `removeProvider`.
  let persisted: MultiAIConfig = structuredClone(initial);
  const setConfigCalls: Array<Partial<MultiAIConfig>> = [];
  ...
  const harness: FakeHarness = {
    ...
    setConfig: async (patch) => {
      ...
      // Mirror the real `setMultiAIConfig`: deep-merge with undefined keys
      ...
      persisted = next as MultiAIConfig;
...
describe('multiai provider add', () => {
  it('imports providers and models from a custom registry, persisting source on each provider', async () => {
    ...
    const { harness, current, setConfigCalls } = makeHarness({ providers: {} } as MultiAIConfig);
  ...
  it('reads the api key from MULTIAI_REGISTRY_API_KEY when --api-key is omitted', async () => {
    const fetchMock = mockRegistryFetch();
    const { harness } = makeHarness({ providers: {} } as MultiAIConfig);
    const { deps, exitCodes } = makeDeps(harness, {
      env: { MULTIAI_REGISTRY_API_KEY: 'sk-env-token' },
    });
...
describe('multiai provider remove', () => {
...
describe('multiai provider list', () => {
  ...
  it('renders one row per provider with counts and source labels', async () => {
    ...
    expect(out).toMatch(/managed:multiai\s+type=kimi\s+models=0\s+source=oauth/);
...
describe('registerProviderCommand', () => {
  it('describes the user-facing subcommand and routes flags through commander', async () => {
    const fetchMock = mockRegistryFetch();
    const { harness, current } = makeHarness({ providers: {} } as MultiAIConfig);
...
describe('multiai provider catalog list', () => {
...
describe('multiai provider catalog add', () => {
  ...
  it('rejects an unknown --default-model with a helpful hint', async () => {
    ...
    expect(err).toContain('multiai provider catalog list anthropic');
  });
  ...
  it('does not persist thinking.enabled=false for first-time setup with --default-model', async () => {
    ...
    // explicit "off" request. A fresh `multiai provider catalog add
  ...
  it('falls back to MULTIAI_REGISTRY_API_KEY when --api-key is omitted', async () => {
    mockRegistryFetch(CATALOG_BODY);
    const { harness, current } = makeHarness({ providers: {} } as MultiAIConfig);
    const { deps, exitCodes } = makeDeps(harness, {
      env: { MULTIAI_REGISTRY_API_KEY: 'sk-env' },
    });
