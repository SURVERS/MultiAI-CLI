...
describe('CLI options parsing', () => {
  ...
  describe('MULTIAI_MODEL_OUTPUT_FORMAT', () => {
    ...
    it('rejects an invalid env value', () => {
      ...
      ).toThrow('Invalid MULTIAI_MODEL_OUTPUT_FORMAT value "json"');
    });
  ...
  describe('--agent / --agent-file', () => {
    ...
    it('accepts the flags in prompt mode with the v2 engine flag', () => {
      const opts = parse(['-p', 'hi', '--agent', 'reviewer']);
      expect(validateOptions(opts, { MULTIAI_EXPERIMENTAL_FLAG: '1' }).uiMode).toBe('print');
    });
  });
  ...
  describe('sub-commands', () => {
    ...
    it('registers the visible sub-commands', () => {
      ...
      expect(program.commands.find((command) => command.name() === 'account')?.aliases()).toContain(
        'profile',
      );
    });
  });
