...
describe('multiai export', () => {
  ...
  it('exports without initializing disabled telemetry', async () => {
    const program = new Command('multiai');
    ...
    await program.parseAsync(['node', 'multiai', 'export', 'ses_telemetry', '--output', output], {
      from: 'node',
    });
  ...
  it('keeps telemetry disabled when the config also disables it', async () => {
    const program = new Command('multiai');
    ...
    await program.parseAsync(['node', 'multiai', 'export', 'ses_disabled', '--output', output], {
      from: 'node',
    });
  ...
  it('does not emit first-launch telemetry when the harness creates a device id', async () => {
    const program = new Command('multiai');
    ...
    await program.parseAsync(['node', 'multiai', 'export', 'ses_first_launch', '--output', output], {
      from: 'node',
    });
