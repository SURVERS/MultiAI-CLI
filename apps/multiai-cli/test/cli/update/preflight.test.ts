...
vi.mock('../../../src/tui/config', () => ({
  ...
  TuiConfigParseError: class TuiConfigParseError extends Error {
    ...
    constructor(fallback: TuiConfig) {
      super('Invalid client preferences in ~/.multiai/tui.toml; using defaults.');
...
describe('runUpdatePreflight', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('MULTIAI_EXPERIMENTAL_FLAG', '');
  ...
  it('skips all update work when MULTIAI_NO_AUTO_UPDATE is set', async () => {
    vi.stubEnv('MULTIAI_NO_AUTO_UPDATE', '1');
  ...
  it('does not retain the removed MULTIAI_CLI_NO_AUTO_UPDATE alias', async () => {
    vi.stubEnv('MULTIAI_CLI_NO_AUTO_UPDATE', 'true');
    mocks.readUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    const { options } = captureOutput();

    await expect(runUpdatePreflight('0.4.0', options)).resolves.toBe('continue');

    expect(readUpdateCache).toHaveBeenCalledOnce();
  });
  ...
  it('npm-global: prompts and spawns npm install -g when automatic updates are disabled', async () => {
    ...
    expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
      expect.objectContaining({
        installCommand: 'npm install -g multiai-cli@0.5.0',
    ...
    expect(stdout.join('')).toContain('Updated multiai-cli to 0.5.0');
  });
  ...
  it('refreshes a stale cached target before showing the foreground install prompt', async () => {
    ...
    expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        installCommand: 'npm install -g multiai-cli@0.7.0',
      }),
    );
    expect(mocks.spawn).toHaveBeenCalledWith(
      expect.stringMatching(/^npm(\.cmd)?$/),
      ['install', '-g', 'multiai-cli@0.7.0'],
      expect.objectContaining({ stdio: 'inherit' }),
    );
    expect(stdout.join('')).toContain('Updated multiai-cli to 0.7.0');
  });
    ...
    try {
      ...
      expect(mocks.promptForInstallChoice).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          installCommand: 'npm install -g multiai-cli@0.6.0',
        }),
      );
    } finally {
      vi.useRealTimers();
  ...
  it('homebrew: points to the npm package and GitHub releases without spawning', async () => {
    mocks.readUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    mocks.detectInstallSource.mockResolvedValue('homebrew');
    const { stdout, options } = captureOutput();
    await expect(runUpdatePreflight('0.4.0', options)).resolves.toBe('continue');
    expect(stdout.join('')).toContain('npm install -g multiai-cli@0.5.0');
    expect(stdout.join('')).toContain('Third-party sources may lag behind the official release.');
    expect(stdout.join('')).toContain('https://github.com/SURVERS/MultiAI-CLI/releases');
    expect(promptForInstallChoice).not.toHaveBeenCalled();
    expect(mocks.spawn).not.toHaveBeenCalled();
  });

  it('native on darwin: points to GitHub releases and does not run a remote script', async () => {
    disableAutoInstall();
    mocks.readUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
    mocks.refreshUpdateCache.mockResolvedValue(cacheWith('0.5.0'));
  ...
  it('warns and continues when spawn exits non-zero, without claiming success', async () => {
    ...
    expect(stdout.join('')).not.toContain('Updated multiai-cli');
  });
  ...
  it('shows a one-shot notice after a background update succeeds and the new version starts', async () => {
    ...
    expect(rendered).toContain('MultiAI CLI updated to v0.5.0');
    expect(rendered).toContain(
      'https://survers.github.io/MultiAI-CLI/en/release-notes/changelog.html',
    );
  ...
  it('infers a background update success notice when the active install version is now running', async () => {
    ...
    expect(stdout.join('')).toContain('MultiAI CLI updated to v0.5.0');
    expect(writeUpdateInstallState).toHaveBeenCalledWith(expect.objectContaining({
      active: null,
      lastFailure: null,
      lastSuccess: expect.objectContaining({
  ...
  describe('rollout gating', () => {
    ...
    it('MULTIAI_EXPERIMENTAL_FLAG bypasses the rollout: held devices still update', async () => {
    ...
    it('MULTIAI_NO_AUTO_UPDATE still wins over the experimental flag', async () => {
      vi.stubEnv('MULTIAI_EXPERIMENTAL_FLAG', '1');
      vi.stubEnv('MULTIAI_NO_AUTO_UPDATE', '1');
