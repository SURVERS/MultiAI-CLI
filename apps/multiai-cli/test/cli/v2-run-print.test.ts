...
const mocks = vi.hoisted(() => ({
  ...
  createMultiAIDefaultHeaders: vi.fn(() => ({})),
  resolveMultiAIHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/kimi-code-test-home'),
  createMultiAIDeviceId: vi.fn(() => 'device-1'),
}));

vi.mock('@multiai/agent-core-v2', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@multiai/agent-core-v2')>();
  return {
    ...actual,
...
vi.mock('@multiai/oauth', async () => {
  ...
  return {
    ...actual,
    createMultiAIDefaultHeaders: mocks.createMultiAIDefaultHeaders,
