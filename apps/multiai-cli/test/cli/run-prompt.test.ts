        ...
        if (outputFormat === 'stream-json') {
          ...
          stdout.write(
            `${JSON.stringify({
              ...
              command: 'multiai -r ses_prompt',
              content: 'To resume this session: multiai -r ses_prompt',
            })}\n`,
          );
          return;
        }
        stderr.write(`multiai version ${version}\n`);
        stdout.write('• hello world\n\n');
        stderr.write('To resume this session: multiai -r ses_prompt\n');
      },
    ),
...
// when MULTIAI_EXPERIMENTAL_FLAG is set. Mock the native v2 runner so tests
...
describe('runPrompt', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('MULTIAI_MODEL_OUTPUT_FORMAT', '');
  });
  ...
  it('creates a fresh auto-permission session and streams assistant output to stdout', async () => {
    ...
    expect(stderr.text()).toBe('To resume this session: multiai -r ses_prompt\n');
  ...
  it('does not attach telemetry first-launch callbacks to the device identity', async () => {
  ...
  it('formats thinking and assistant output as transcript blocks', async () => {
    ...
    expect(stderr.text()).toBe(
      '• The user wants an exact reply.\n  No tools are needed.\n\nTo resume this session: multiai -r ses_prompt\n',
    );
  ...
  it('writes stream-json output as assistant JSONL with resume meta without transcript bullets', async () => {
    ...
    expect(stdout.text()).toBe(
      [
        ...
        '{"role":"meta","type":"session.resume_hint","session_id":"ses_prompt","command":"multiai -r ses_prompt","content":"To resume this session: multiai -r ses_prompt"}',
        '',
      ].join('\n'),
    );
    expect(stderr.text()).toBe('');
  ...
  it('throws when no default model is configured', async () => {
    ...
    await expect(
      ...
      'No model configured. Run `multiai` and use /login to sign in, then retry; or set default_model in config.toml.',
    );
