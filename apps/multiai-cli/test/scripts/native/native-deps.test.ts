/**
 * Scenario: enumerating native dependency descriptors.
 * Responsibilities: map each supported target to the binaries required by the bundle.
 * Wiring: dependency metadata is exercised as pure data.
 * Run: pnpm --filter @multiai/cli exec vitest run test/scripts/native/native-deps.test.ts
 */
...
describe('resolveTargetDeps', () => {
  it('returns one descriptor per package for darwin-arm64', () => {
    ...
    expect(names).toContain('@multiai/pi-tui');
  });
  ...
  it('encodes pi-tui native file path per target', () => {
    const linuxPiTui = resolveTargetDeps('linux-arm64').find(
      (d) => d.resolvedName === '@multiai/pi-tui',
    );
