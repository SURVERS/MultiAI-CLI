/**
 * Scenario: collecting web UI files for native packaging.
 * Responsibilities: copy only runtime assets and serialize a deterministic manifest.
 * Wiring: the real collector runs against isolated temporary directories.
 * Run: pnpm --filter multiai-cli exec vitest run test/scripts/native/web-assets.test.ts
 */
...
describe('collectWebAssets', () => {
  it('collects dist-web files into deterministic SEA asset keys', async () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'multiai-web-assets-build-'));
    try {
  ...
  it('fails clearly when dist-web has not been built', async () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'multiai-web-assets-missing-'));
    try {
      await expect(collectWebAssets({ appRoot, target: 'test-target' })).rejects.toThrow(
        /MultiAI web build output was not found/,
      );
    } finally {
      rmSync(appRoot, { recursive: true, force: true });
    }
  ...
  it('keeps manifest JSON parseable and stable', async () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'multiai-web-assets-json-'));
    try {
