/**
 * Scenario: resolving native build and release paths.
 * Responsibilities: produce deterministic target-specific locations and filenames.
 * Wiring: path helpers are exercised as pure functions.
 * Run: pnpm --filter @multiai/cli exec vitest run test/scripts/native/paths.test.ts
 */
...
describe('targetTriple', () => {
  ...
  it('honors MULTIAI_BUILD_TARGET override', () => {
    expect(
      targetTriple({
        platform: 'darwin',
        arch: 'arm64',
        env: { MULTIAI_BUILD_TARGET: 'linux-arm64' },
      }),
    ).toBe('linux-arm64');
  });
});
...
describe('executableName', () => {
  it('returns multiai.exe on win32', () => {
    expect(executableName('win32')).toBe('multiai.exe');
  });

  it('returns multiai on other platforms', () => {
    expect(executableName('darwin')).toBe('multiai');
    expect(executableName('linux')).toBe('multiai');
  });
});
