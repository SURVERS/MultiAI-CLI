/**
 * Scenario: producing native release artifacts.
 * Responsibilities: package target files, checksums, and release metadata consumable by CI.
 * Wiring: real packaging scripts run against isolated temporary directories.
 * Run: pnpm --filter @multiai/cli exec vitest run test/scripts/native/release-artifacts.test.ts
 */
...
const executableName = process.platform === 'win32' ? 'multiai.exe' : 'multiai';
...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    rmSync(resolve(artifactsDir, `multiai-cli-${target}.zip`), { force: true });
    rmSync(resolve(artifactsDir, `multiai-cli-${target}.zip.sha256`), { force: true });
  });
  ...
  it('packages the native binary as a zip archive and checksums the archive', async () => {
    ...
    await execFileAsync(process.execPath, [packageScript], {
      cwd: appRoot,
      env: { ...process.env, MULTIAI_BUILD_TARGET: target },
    });

    const archivePath = resolve(artifactsDir, `multiai-cli-${target}.zip`);
    ...
    expect(readFileSync(checksumPath, 'utf-8')).toBe(
      `${sha256(readFileSync(archivePath))}  multiai-cli-${target}.zip\n`,
    );
  });

  it('produces a manifest from zip archive checksums', async () => {
    const releaseDir = await mkdtemp(join(tmpdir(), 'multiai-manifest-zip-'));
    const archiveBytes = Buffer.from('fake zip bytes');
    const checksum = sha256(archiveBytes);
    await writeFile(join(releaseDir, 'multiai-cli-darwin-arm64.zip'), archiveBytes);
    await writeFile(
      join(releaseDir, 'multiai-cli-darwin-arm64.zip.sha256'),
      `${checksum}  multiai-cli-darwin-arm64.zip\n`,
    );

    await execFileAsync(process.execPath, [manifestScript, releaseDir, '@multiai/cli@1.0.0']);
    ...
    expect(manifest).toEqual({
      version: '1.0.0',
      tag: '@multiai/cli@1.0.0',
      platforms: {
        'darwin-arm64': {
          filename: 'multiai-cli-darwin-arm64.zip',
          checksum,
        },
      },
    });
