# apps/multiai-cli Development Guide

This file only contains rules local to `apps/multiai-cli`. For cross-repo rules, see the root `AGENTS.md`.
...
`apps/multiai-cli` is the terminal UI / CLI app. The entry chain is:

`src/main.ts` -> `src/cli/commands.ts` -> `src/cli/run-shell.ts` -> SDK `MultiAIHarness` -> `src/tui/multiai-tui.ts`
...
- `src/tui/multiai-tui.ts`: the `MultiAITUI` coordinator — wires state, layout, editor, session, SDK events, and dialogs together, and dispatches slash-command handlers. Heavy logic is delegated to `controllers/`, not accumulated here.
...
- `MultiAITUI` coordinates; it does not accumulate complex business rules. New logic that can be tested independently should be split into `controllers`, `commands`, `components`, `reverse-rpc`, or `utils` first.
- `controllers` own the heavy, independently-testable slices (event routing, streaming render, session replay, tasks browser, editor keyboard, auth). Event-routing and rendering logic belong here, not on the `MultiAITUI` class.
- `commands` only owns slash-command declaration, parsing, and the parsed-result types. The actual execution can be dispatched from `MultiAITUI`, but complex logic should continue to sink downward.
...
- `apps/multiai-cli` may only use core capabilities through `@multiai/sdk`. Do not import `@multiai/agent-core` directly in app code.
...
- Functions with no state / UI side effects do not belong as private methods on the `MultiAITUI` class; put them in external utils.
