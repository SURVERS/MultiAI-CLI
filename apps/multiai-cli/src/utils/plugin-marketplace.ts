...
import {
  MULTIAI_PLUGIN_MARKETPLACE_URL_ENV,
} from '#/constant/app';
...
export async function loadPluginMarketplace(
  ...
  const configuredSource = options.source ?? process.env[MULTIAI_PLUGIN_MARKETPLACE_URL_ENV];
  if (configuredSource === undefined) {
    throw new Error(
      `No plugin marketplace is configured. Set ${MULTIAI_PLUGIN_MARKETPLACE_URL_ENV} to opt in.`,
    );
  }
...
function resolveMarketplaceLocation(source: string, workDir: string): MarketplaceLocation {
  ...
  if (trimmed.length === 0) {
    throw new Error(`${MULTIAI_PLUGIN_MARKETPLACE_URL_ENV} cannot be empty.`);
  }
