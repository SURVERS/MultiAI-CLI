...
export const OUTPUT_FORMAT_ENV = 'MULTIAI_MODEL_OUTPUT_FORMAT';
 ...
 *
 * Precedence: explicit `--output-format` flag → `MULTIAI_MODEL_OUTPUT_FORMAT` env
 * (prompt mode only) → `text`. The env var is ignored outside prompt mode so an
 * ambient value never affects interactive `multiai`. An invalid env value fails
...
export function validateOptions(
  ...
  if (
    ...
    (!promptMode || !isMultiAIV2Enabled(env))
  ) {
    throw new OptionConflictError(
      '--agent/--agent-file are only available with the v2 engine (multiai -p with MULTIAI_EXPERIMENTAL_FLAG=1).',
    );
  }
  ...
  // Validate `MULTIAI_MODEL_OUTPUT_FORMAT` eagerly in prompt mode so a typo fails
