/**
 * Native v2 `multiai -p` (print mode) runner.
 *
 ...
 *
 * Selected by `runPrompt` when `MULTIAI_EXPERIMENTAL_FLAG` is set.
 */
...
import { createMultiAIDefaultHeaders } from '@multiai/oauth';
...
export async function runV2Print(
  ...
  const homeDir = resolveMultiAIHome();
  const logging = resolveLoggingConfig({ homeDir, env: process.env });
  const identity = createMultiAIHostIdentity(version);
  const hostHeaders = createMultiAIDefaultHeaders({ homeDir, ...identity });
    ...
    if (target.cwd !== undefined && resolve(target.cwd) !== resolve(workDir)) {
      stderr.write(
        `Session "${opts.session}" was created under a different directory.\n` +
          `  cd "${target.cwd}" && multiai -r ${opts.session}\n\n`,
      );
...
/**
 * Apply the print-mode (`multiai -p`) background-resource policy after the main
