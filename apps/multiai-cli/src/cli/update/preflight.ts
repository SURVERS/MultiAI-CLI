...
import { log, type Logger } from '@multiai/sdk';
import type { TelemetryProperties } from '@multiai/telemetry';

import {
  MULTIAI_OFFICIAL_INSTALL_URL,
} from '#/constant/app';
...
export function installCommandFor(
  ...
  switch (source) {
    ...
    case 'homebrew':
      return `npm install -g ${NPM_PACKAGE_NAME}@${version}`;
    case 'native':
      return MULTIAI_OFFICIAL_INSTALL_URL;
    case 'unsupported':
      return `npm install -g ${NPM_PACKAGE_NAME}@${version}`;
  }
}
...
export function spawnForSource(
  ...
  switch (source) {
    ...
    case 'homebrew':
      throw new Error('homebrew install source cannot be auto-installed');
    case 'native':
      throw new Error('native install source cannot be auto-installed');
    case 'unsupported':
...
const THIRD_PARTY_SOURCE_NOTE =
  ...
  `For the latest updates, use the official installer: ${MULTIAI_OFFICIAL_INSTALL_URL}\n`;
...
function renderBackgroundInstallSuccessNotice(version: string): string {
  ...
  return `MultiAI CLI updated to ${displayVersion}\nChangelog: ${CHANGELOG_URL}\n`;
}
...
function isAutoUpdateDisabledByEnv(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  return truthy(env['MULTIAI_NO_AUTO_UPDATE']);
}
