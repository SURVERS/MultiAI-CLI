...
export const CHANGELOG_URL = 'https://survers.github.io/MultiAI-CLI/en/release-notes/changelog.html';
...
const INSTALL_HINT = () => t('Install update now', 'Установить обновление сейчас');
const SKIP_HINT = () => t('Continue with current version', 'Продолжить с текущей версией');
...
function renderInstallPrompt(
  ...
  const changelogText = chalk.hex(UPDATE_PROMPT_PRIMARY).underline(t(`View changelog: ${CHANGELOG_URL}`, `Список изменений: ${CHANGELOG_URL}`));
  const lines = [
    chalk.hex(UPDATE_PROMPT_PRIMARY).bold(t('MultiAI CLI Update Available', 'Доступно обновление MultiAI CLI')),
    chalk.hex(UPDATE_PROMPT_MUTED)(t(`${PRODUCT_NAME} has a newer release ready.`, `${PRODUCT_NAME}: доступна новая версия.`)),
    `]8;;${CHANGELOG_URL}\\${changelogText}]8;;\\`,
    '',
    `${label(t('Current', 'Текущая'))}  ${currentVersion}`,
    `${label(t('Target ', 'Целевая '))}  ${targetVersion}`,
    `${label(t('Source ', 'Источник '))}  ${sourceLabel}`,
    `${label(t('Command', 'Команда'))}  ${command}`,
    '',
    chalk.hex(UPDATE_PROMPT_MUTED)(t('↑↓ choose · Enter confirm · Esc continue', '↑↓ выбрать · Enter подтвердить · Esc продолжить')),
    '',
  ];
