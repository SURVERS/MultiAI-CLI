/**
 * `multiai web` — run the local server in the foreground and open the web UI.
 *
...
import { hostRequestHeadersSeed } from '@multiai/agent-core-v2';
import { createServerLogger, startServer, type ServerLogger } from '@multiai/kap-server';
import { shutdownTelemetry, track } from '@multiai/telemetry';
...
function formatReadyLine(
  ...
  const notice = dangerousBypassAuth
    ...
    : '';
  return `${notice}MultiAI server: ${buildOpenableUrl(origin, token)}\n`;
}
...
function formatDangerNoticeLines(): string[] {
  ...
  return [
    `  ${dangerBold(t('⚠ DANGER: authentication is DISABLED (--dangerous-bypass-auth).', '⚠ ОПАСНО: аутентификация ОТКЛЮЧЕНА (--dangerous-bypass-auth).'))}`,
    `  ${danger(t('Anyone who can reach this port gets full access. Only continue if you understand the risk.', 'Любой, кто имеет доступ к этому порту, получит полный доступ. Продолжайте, только если понимаете риск.'))}`,
    `  ${danger(t('If you are unsure, stop this process now with ', 'Если не уверены, остановите процесс сейчас с помощью '))}${dangerBold('Ctrl+C')}${danger('.')}`,
  ];
}

/**
 * `multiai web` — runs the local server in-process, attached to the current
...
async function runServerInProcess(
  ...
  const v2 = await startServer({
    ...
    telemetry: false,
    // Seed the CLI's MultiAI identity headers so the engine's outbound
    // requests (model, WebSearch, FetchURL) carry the same User-Agent +
    // X-Msh-* identity as direct CLI runs.
    seeds: hostRequestHeadersSeed(buildMultiAIDefaultHeaders(version)),
...
export function formatReadyBanner(
  ...
  // Borderless header: the MultiAI mark (the little mascot with eyes) sits next
  // to the title, keeping the brand without the enclosing box.
  const logo = ['▐█▛█▛█▌', '▐█████▌'] as const;
  const lines: string[] = [
    '',
    `  ${primary(logo[0])}  ${title(t('MultiAI server ready', 'Сервер MultiAI готов'))}  ${dim(getVersion())}`,
    `  ${primary(logo[1])}  ${dim(t('Local web UI is available from this machine.', 'Локальный веб-интерфейс доступен на этой машине.'))}`,
    '',
  ];
  ...
  if (isLoopbackHost(host)) {
    lines.push(`  ${label(t('Network:  ', 'Сеть:     '))}${muted(t('off', 'выкл'))}${dim(t('  use --host to enable', '  используйте --host для включения'))}`);
  }
  if (opts.token !== undefined) {
    // Set the token off with surrounding whitespace rather than color, so it is
    // easy to spot without being highlighted.
    lines.push('');
    lines.push(`  ${label(t('Token:    ', 'Токен:    '))}${opts.token}`);
    lines.push('');
  }

  // Auxiliary controls last.
  lines.push(`  ${label(t('Logs:     ', 'Логи:     '))}${muted(t('off', 'выкл'))}${dim(t('  use --log-level info to enable', '  используйте --log-level info для включения'))}`);
  // The server always runs in the foreground attached to this terminal.
  lines.push(`  ${label(t('Stop:     ', 'Стоп:     '))}${muted('Ctrl+C')}`);
  lines.push('');
  return lines.join('\n');
}
