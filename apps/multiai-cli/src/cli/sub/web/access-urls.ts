 ...
 *
 * Shared by the `multiai web` ready banner and `multiai web rotate-token` so both
...
export function accessUrlLines(
  ...
  if (isWildcard(host)) {
    const lines: AccessUrlLine[] = [
      { label: t('Local:    ', 'Локально: '), url: buildOpenableUrl(`http://localhost:${port}`, token) },
    ];
    const addrs = networkAddresses ?? listNetworkAddresses();
    for (const addr of addrs) {
      lines.push({
        label: t('Network:  ', 'Сеть:     '),
  ...
  if (isLoopbackHost(host)) {
    return [{ label: t('Local:    ', 'Локально: '), url: buildOpenableUrl(hostOrigin(host, port), token) }];
  }
  return [{ label: t('URL:      ', 'URL:      '), url: buildOpenableUrl(hostOrigin(host, port), token) }];
}
