...
export async function handleProviderAdd(
  ...
  if (apiKey === undefined) {
    deps.stderr.write(
      'Missing API key. Pass --api-key <key> or set MULTIAI_REGISTRY_API_KEY.\n',
    );
    deps.exit(1);
  }
...
export function registerProviderCommand(parent: Command, deps?: Partial<ProviderDeps>): void {
  ...
  provider
    ...
    .option('--api-key <key>', 'Registry API key. Falls back to MULTIAI_REGISTRY_API_KEY.')
  ...
  catalog
    ...
    .option('--api-key <key>', 'API key for the provider. Falls back to MULTIAI_REGISTRY_API_KEY.')
