...
import { track as trackTelemetry, type TelemetryProperties } from '@multiai/telemetry';
...
export async function handleUpgrade(
  ...
  try {
    ...
    deps.stderr.write(`${t(`error: failed to check for updates: ${reason}`, `ошибка: не удалось проверить обновления: ${reason}`)}\n`);
    return 1;
  }
  ...
  if (target === null) {
    ...
    deps.stdout.write(`${t(`MultiAI CLI is already up to date (${formatDisplayVersion(currentVersion)}).`, `MultiAI CLI уже обновлён до последней версии (${formatDisplayVersion(currentVersion)}).`)}\n`);
    return 0;
  }
