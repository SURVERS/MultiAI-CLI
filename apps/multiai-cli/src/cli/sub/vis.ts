/**
 * `multiai vis` sub-command.
 *
 * CLI glue only: resolves the MultiAI home, starts the in-process session
...
export async function handleVis(deps: VisDeps, opts: VisOptions): Promise<void> {
  ...
  // Lazily load the embedded single-file SPA so normal `multiai` startup never
  ...
  try {
    ...
    deps.stderr.write(`${t(`Failed to start multiai vis: ${msg}`, `Не удалось запустить multiai vis: ${msg}`)}\n`);
    return deps.exit(1);
  }
  ...
  deps.stdout.write(`${t(`multiai vis is running at ${server.url}`, `multiai vis запущен по адресу ${server.url}`)}\n`);
  deps.stdout.write(`${t('Press Ctrl-C to stop.', 'Нажмите Ctrl-C для остановки.')}\n`);

  if (opts.open) {
    try {
      await deps.openUrl(target);
    } catch {
      deps.stderr.write(`${t(`Could not open a browser; visit ${target} manually.`, `Не удалось открыть браузер; перейдите на ${target} вручную.`)}\n`);
    }
  }
...
export function registerVisCommand(parent: Command, overrides?: Partial<VisDeps>): void {
  parent
    .command('vis')
    .description(t('Launch the session visualizer in your browser.', 'Запустить визуализатор сессий в браузере.'))
    .option('--port <number>', t('Port to bind. Default: auto-pick a free port.', 'Порт для прослушивания. По умолчанию свободный порт выбирается автоматически.'))
    .option('--host <host>', t('Host to bind. Default: 127.0.0.1.', 'Хост для прослушивания. По умолчанию: 127.0.0.1.'))
    .option('--no-open', t('Do not open the browser automatically.', 'Не открывать браузер автоматически.'))
    .argument('[sessionId]', t('Open directly to this session.', 'Сразу открыть указанную сессию.'))
    .action(
      async (
  ...
  return {
    ...
    startVisServer:
      ...
      (async (opts) => {
        ...
        const { startVisServer } = await import('@multiai/vis-server/start');
