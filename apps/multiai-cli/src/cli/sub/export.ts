...
export async function handleExport(
  ...
  if (requestedId !== undefined) {
    ...
    if (previousSummary === undefined) {
      deps.stderr.write(`${t('No previous session found to export.', 'Не найдена предыдущая сессия для экспорта.')}\n`);
      deps.exit(1);
    }
    ...
    if (!opts.yes) {
      ...
      if (!confirmed) {
        deps.stdout.write(`${t('Export cancelled.', 'Экспорт отменён.')}\n`);
        return;
      }
    }
  }
...
export function registerExportCommand(parent: Command, deps?: Partial<ExportDeps>): void {
  parent
    .command('export')
    .description(t('Export a session as a ZIP archive.', 'Экспортировать сессию в ZIP-архив.'))
    .option('-o, --output <path>', t('Output ZIP path.', 'Путь к выходному ZIP-архиву.'))
    .option('-y, --yes', t('Skip previous-session confirmation.', 'Не запрашивать подтверждение выбора предыдущей сессии.'))
    .option(
      '--no-include-global-log',
      t('Skip bundling the active global diagnostic log (~/.multiai/logs/multiai.log, not rotated .1 files). By default the global log is included.', 'Не включать активный глобальный журнал диагностики (~/.multiai/logs/multiai.log, без архивных файлов .1). По умолчанию журнал включается.'),
    )
    .argument('[sessionId]', t('Session id to export. Defaults to the most recent session.', 'ID экспортируемой сессии. По умолчанию используется последняя сессия.'))
    .action(
      async (
        sessionId: string | undefined,
...
function createDefaultExportDeps(overrides: Partial<ExportDeps> = {}): ExportDeps {
  ...
  const getHarness = (): MultiAIHarness => {
    const currentTelemetryBootstrap = getTelemetryBootstrap();
    harness ??= createMultiAIHarness({
