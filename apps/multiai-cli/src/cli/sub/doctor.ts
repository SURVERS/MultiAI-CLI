...
export interface DoctorDeps {
  ...
  readonly configRpc?: MultiAIConfigRpc;
...
export function registerDoctorCommand(parent: Command, deps?: Partial<DoctorDeps>): void {
  const doctor = parent
    .command('doctor')
    .description(t('Validate MultiAI CLI configuration files.', 'Проверить файлы конфигурации MultiAI CLI.'))
    .action(async () => {
  ...
  doctor
    .command('config')
    .description(t('Validate config.toml.', 'Проверить config.toml.'))
    .argument('[path]', t('Validate this file as config.toml instead of the default path.', 'Проверить этот файл как config.toml вместо файла по умолчанию.'))
  ...
  doctor
    .command('tui')
    .description(t('Validate tui.toml.', 'Проверить tui.toml.'))
    .argument('[path]', t('Validate this file as tui.toml instead of the default path.', 'Проверить этот файл как tui.toml вместо файла по умолчанию.'))
    .action(async (path: string | undefined) => {
...
function resolveDeps(deps: Partial<DoctorDeps> | DoctorDeps | undefined): ResolvedDoctorDeps {
  ...
  const getConfigRpc = (): MultiAIConfigRpc => {
    configRpc ??= createMultiAIConfigRpc();
    return configRpc;
  };
  ...
  if (!deps.fileExists(spec.path)) {
    return {
      ...
      message: spec.explicit
        ? t('File does not exist.', 'Файл не существует.')
        : t('File does not exist; built-in defaults will apply.', 'Файл не существует; будут применены встроенные значения по умолчанию.'),
    };
  }
...
function formatSuccess(results: readonly CheckResult[]): string {
  return [
    ...
    '',
    t('All checked config files are valid.', 'Все проверенные файлы конфигурации корректны.'),
    '',
  ].join('\n');
}
...
function formatFailure(results: readonly CheckResult[], issueCount: number): string {
  return [
    t(
      `MultiAI doctor found ${String(issueCount)} ${issueCount === 1 ? 'issue' : 'issues'}.`,
      `MultiAI doctor обнаружил проблем: ${String(issueCount)}.`,
    ),
    '',
    ...formatResults(results),
    '',
...
function formatErrorMessage(error: unknown, filePath: string): string {
  ...
  if (validationIssues !== undefined) {
    return [
      t(`Invalid configuration in ${filePath}.`, `Некорректная конфигурация в ${filePath}.`),
      t('Validation issues:', 'Ошибки проверки:'),
...
function findValidationIssues(error: unknown): readonly MultiAIConfigValidationIssue[] | undefined {
...
function isValidationIssueArray(value: unknown): value is readonly MultiAIConfigValidationIssue[] {
  return Array.isArray(value) && value.every(isValidationIssue);
}

function isValidationIssue(value: unknown): value is MultiAIConfigValidationIssue {
