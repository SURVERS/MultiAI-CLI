...
import { MultiAITUI } from '#/tui/index';
import { setTuiLanguage, t } from '#/tui/i18n';
...
import { isMultiAIV2Enabled } from './experimental-v2';
import { createCliTelemetryBootstrap, initializeCliTelemetry } from './telemetry';
import { createMultiAIHostIdentity } from './version';
...
export async function runShell(
  ...
  setTuiLanguage(tuiConfig.language ?? 'en');
  ...
  const harnessOptions: MultiAIHarnessOptions = {
    homeDir: telemetryBootstrap.homeDir,
    identity: createMultiAIHostIdentity(version),
    skillDirs: opts.skillsDirs,
    telemetry: telemetryClient,
    sessionStartedProperties: { yolo: opts.yolo, auto: opts.auto, plan: opts.plan, afk: false },
  };
  // Experimental agent-core-v2 route (same master switch as `multiai -p`): the
  // harness is the SDK's v2-backed client, so the whole TUI runs on the
  // agent-core-v2 engine.
  const harness = isMultiAIV2Enabled()
    ? createMultiAIHarnessV2(harnessOptions)
    : createMultiAIHarness(harnessOptions);
  log.info('multiai-cli starting', {
    version,
    uiMode: CLI_UI_MODE,
  ...
  const tui = new MultiAITUI(harness, {
    cliOptions: opts,
  ...
  const trackLifecycleForSession = (
    sessionId: string,
    event: string,
    properties?: Parameters<MultiAIHarness['track']>[1],
  ) => {
  ...
  const trackLifecycle = (event: string, properties?: Parameters<MultiAIHarness['track']>[1]) => {
  ...
  // If we crash without going through MultiAITUI.stop(), the terminal is left in
  ...
  tui.onExit = async (exitCode = 0) => {
    ...
    process.stdout.write(`${gutter}${t('Bye!', 'До встречи!')}\n`);
    const hints: string[] = [];
    if (sessionId !== '' && hasContent) {
      hints.push(`${gutter}${t('To resume this session:', 'Чтобы продолжить эту сессию:')} multiai -r ${sessionId}`);
    }
