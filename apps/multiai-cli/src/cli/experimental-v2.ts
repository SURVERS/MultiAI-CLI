 ...
 *
 * When the master switch `MULTIAI_EXPERIMENTAL_FLAG` is truthy, `multiai -p`
 ...
 *
 * Note: `multiai web` always boots kap-server (the agent-core-v2 engine
 * server) — it does not consult this switch.
 */

export const MULTIAI_V2_ENV = 'MULTIAI_EXPERIMENTAL_FLAG';
...
export function isMultiAIV2Enabled(
  env: Readonly<Record<string, string | undefined>> = process.env,
): boolean {
  return isTruthyEnv(MULTIAI_V2_ENV, env);
}
