 ...
 * MultiAI CLI client preferences such as terminal UI and update behavior.
 */
...
import { TuiLanguageValues, type TuiLanguage } from './i18n';

export const INVALID_TUI_CONFIG_MESSAGE =
  'Invalid TUI config in ~/.multiai/tui.toml; using defaults.';
...
export const TuiConfigFileSchema = z.object({
  language: z.enum(TuiLanguageValues).optional(),
...
export const TuiConfigSchema = z.object({
  language: z.enum(TuiLanguageValues),
...
type ParsedTuiConfig = z.infer<typeof TuiConfigSchema>;
export type TuiConfig = Omit<ParsedTuiConfig, 'language'> & { language?: TuiLanguage };
...
export function renderTuiConfig(config: TuiConfig): string {
  return `# ~/.multiai/tui.toml
# Client preferences for MultiAI CLI.
# Agent/runtime settings stay in ~/.multiai/config.toml.

language = "${config.language ?? DEFAULT_TUI_CONFIG.language ?? 'en'}" # "en" | "ru"
