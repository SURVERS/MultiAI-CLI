...
import type { ContextMessage } from '@multiai/sdk';
import { isMultiAIError } from '@multiai/sdk';
...
export async function handleUndoCommand(
  ...
  if (host.state.appState.streamingPhase !== 'idle') {
    host.showError(t('Cannot undo while streaming — press Esc or Ctrl-C first.', 'Нельзя отменить действия во время генерации — сначала нажмите Esc или Ctrl-C.'));
    return;
  }
  ...
  if (count === undefined) {
    host.showError(t('Usage: /undo [count], where count is a positive integer.', 'Использование: /undo [число], где число — положительное целое.'));
    return;
  }
...
async function undoByCount(host: SlashCommandHost, count: number): Promise<boolean> {
  ...
  if (lastUserIndex === undefined) {
    showUndoLimitStatus(host, t('Nothing to undo.', 'Нечего отменять.'));
    return false;
  }
  ...
  try {
    ...
    host.showError(t(`Failed to undo: ${message}`, `Не удалось отменить: ${message}`));
    return false;
  }
...
function formatUndoChoiceLabel(
  ...
  if (entry.kind === 'skill_activation') {
    ...
    if (name.length === 0) return t('Skill: unknown', 'Навык: неизвестен');
    return args.length > 0 ? `/${name} ${args}` : `/${name}`;
  }
  if (entry.kind === 'plugin_command' && entry.pluginCommandData !== undefined) {
    return formatPluginCommandSlash(entry.pluginCommandData) ?? t('User message', 'Сообщение пользователя');
  }
  ...
  if (imageCount > 0) {
    return t(
      `User message (${String(imageCount)} ${imageCount === 1 ? 'image' : 'images'})`,
      `Сообщение пользователя (${String(imageCount)} ${imageCount === 1 ? 'изображение' : 'изображения'})`,
    );
  }
  return t('User message', 'Сообщение пользователя');
}
...
function formatUndoLimitMessage(
  ...
  const reason = availability.stoppedAtCompaction
    ? t(' after the last compaction', ' после последнего сжатия')
    : '';
  ...
  return t(
    ...
    `Нельзя отменить ${requested}; в активном контексте можно отменить только ${max}${reason}.`,
  );
}

function formatNothingToUndoMessage(availability: UndoAvailability): string {
  if (availability.stoppedAtCompaction) {
    return t('Nothing to undo after the last compaction.', 'После последнего сжатия нечего отменять.');
  }
  return t('Nothing to undo.', 'Нечего отменять.');
}

function formatPromptCount(count: number): string {
  return t(
    `${String(count)} ${count === 1 ? 'prompt' : 'prompts'}`,
    `${String(count)} ${count === 1 ? 'запрос' : 'запросов'}`,
  );
}
...
function undoLimitFromError(
  ...
  if (!isMultiAIError(error)) return undefined;
