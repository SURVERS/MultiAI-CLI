 ...
 * terminal shows the same ready banner as `multiai web` plus the deep link.
 */
  ...
  host.setExitForegroundTask(async () => {
    ...
    try {
      ...
      process.stderr.write(t(
        ...
        `Не удалось запустить сервер: ${formatErrorMessage(error)}\n`,
      ));
      process.exit(1);
    }
  });
...
function sessionLine(url: string): string {
  ...
  return `${label(t('Session:  ', 'Сессия:   '))}${accent(base)}${frag === '' ? '' : dim(frag)}`;
}
 ...
 * logged), so the browser authenticates on load just like `multiai web`.
 */
