import { ErrorCodes, isMultiAIError, type PermissionMode } from '@multiai/sdk';
...
const resumeGoalInput = (): string => t('Resume the active goal.', 'Продолжи активную цель.');
const startNextGoalNowMessage = (): string => t(
  'No active goal. Starting this goal now.',
  'Активной цели нет. Запускаем эту цель сейчас.',
);
...
export function parseGoalCommand(rawArgs: string): ParsedGoalCommand {
  ...
  if (objective.length === 0) {
    ...
    return {
      kind: 'error',
      severity: 'hint',
      message: t('Provide a goal objective, e.g. `/goal Ship feature X`.', 'Укажите цель, например: `/goal Выпустить функцию X`.'),
    };
  }
  if (objective.length > MAX_GOAL_OBJECTIVE_LENGTH) {
    return {
      kind: 'error',
      message: t(`Goal objective is too long (max ${MAX_GOAL_OBJECTIVE_LENGTH} characters). Reference long details by file path.`, `Цель слишком длинная (максимум ${MAX_GOAL_OBJECTIVE_LENGTH} символов). Укажите длинные детали ссылкой на файл.`),
    };
  }
  ...
  if (objective.length === 0) {
    return {
      ...
      message: t(
        ...
        'Укажите следующую цель, например `/goal next Выпустить функцию X`, или используйте `/goal next manage`.',
      ),
    };
  }
  if (objective.length > MAX_GOAL_OBJECTIVE_LENGTH) {
...
async function queueNextGoal(
  ...
  try {
    ...
    host.showError(t(`Failed to inspect current goal: ${formatErrorMessage(error)}`, `Не удалось проверить текущую цель: ${formatErrorMessage(error)}`));
    return;
  }

  if (!hasCurrentGoal && !isBusy(host)) {
    host.showStatus(startNextGoalNowMessage());
    await createGoal(
      host,
...
async function showGoalQueueManager(
  ...
  try {
    ...
    host.showError(t(`Failed to load upcoming goals: ${formatErrorMessage(error)}`, `Не удалось загрузить следующие цели: ${formatErrorMessage(error)}`));
    return;
  }
    ...
    new GoalQueueManagerComponent({
      ...
      onAction: async (action) => {
        try {
          ...
          host.showError(t(`Failed to update upcoming goals: ${formatErrorMessage(error)}`, `Не удалось обновить следующие цели: ${formatErrorMessage(error)}`));
          return undefined;
        }
      },
      onCancel: () => {
...
async function showGoalQueueEditDialog(
  ...
  if (goal === undefined) {
    host.showStatus(t('Queued goal no longer exists.', 'Цели в очереди больше не существует.'));
    await showGoalQueueManager(host);
    return;
  }
    ...
    new GoalQueueEditDialogComponent({
      ...
      onDone: (result) => {
        void handleGoalQueueEditResult(host, result).catch((error: unknown) => {
          host.showError(t(`Failed to update upcoming goal: ${formatErrorMessage(error)}`, `Не удалось обновить следующую цель: ${formatErrorMessage(error)}`));
        });
      },
    }),
  );
...
function showGoalStartPermissionPrompt(
  ...
  const cancelStart = (): void => {
    ...
    host.showStatus(t('Goal not started.', 'Цель не запущена.'));
  };
  host.mountEditorReplacement(
...
async function setPermissionForGoal(host: GoalCommandHost, mode: PermissionMode): Promise<boolean> {
  try {
    ...
    host.showError(t(`Failed to set permission mode: ${formatErrorMessage(error)}`, `Не удалось изменить режим разрешений: ${formatErrorMessage(error)}`));
    return false;
  }
...
async function startGoal(
  ...
  try {
    ...
    if (isMultiAIError(error) && error.code === ErrorCodes.GOAL_ALREADY_EXISTS) {
      host.showError(
        t(
          'A goal is already active. Use `/goal replace <objective>` to replace it, or `/goal status` to inspect it.',
          'Цель уже активна. Используйте `/goal replace <цель>`, чтобы заменить её, или `/goal status`, чтобы посмотреть её состояние.',
        ),
      );
      return false;
    }
...
async function pauseGoal(host: SlashCommandHost): Promise<void> {
  ...
  try {
    ...
    if (isMultiAIError(error) && error.code === ErrorCodes.GOAL_NOT_FOUND) {
      host.showStatus(t('No goal to pause.', 'Нет цели для приостановки.'));
      return;
    }
    host.showError(formatErrorMessage(error));
    return;
  ...
  host.showStatus(t('Goal paused. Use `/goal resume` to continue.', 'Цель приостановлена. Используйте `/goal resume`, чтобы продолжить.'));
}
...
async function resumeGoal(host: SlashCommandHost): Promise<void> {
  ...
  try {
    ...
    if (isMultiAIError(error) && error.code === ErrorCodes.GOAL_NOT_FOUND) {
      host.showStatus(t('No goal to resume.', 'Нет цели для продолжения.'));
      return;
    }
    host.showError(formatErrorMessage(error));
    return;
  ...
  host.sendNormalUserInput(resumeGoalInput());
}
...
async function cancelGoal(host: SlashCommandHost): Promise<void> {
  ...
  try {
    ...
    if (isMultiAIError(error) && error.code === ErrorCodes.GOAL_NOT_FOUND) {
      host.showStatus(t('No goal to cancel.', 'Нет цели для отмены.'));
      return;
    }
    host.showError(formatErrorMessage(error));
    return;
  ...
  host.showNotice(t('Goal cancelled.', 'Цель отменена.'));
}
...
async function showGoalStatus(host: SlashCommandHost): Promise<void> {
  ...
  if (goal === null) {
    host.showStatus(t('No goal set. Start one with `/goal <objective>`.', 'Цель не задана. Запустите её командой `/goal <цель>`.'));
    return;
  }
  host.state.transcriptContainer.addChild(
