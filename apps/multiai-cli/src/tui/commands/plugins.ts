...
export async function handlePluginsCommand(host: SlashCommandHost, rawArgs: string): Promise<void> {
  ...
  try {
    ...
    if (sub === 'install') {
      ...
      if (source.length === 0) {
        host.showError(t('Usage: /plugins install <local-path-or-zip-url>', 'Использование: /plugins install <локальный-путь-или-url-архива>'));
        return;
      }
      if (!(await confirmInstallTrust(host, source, isOfficialPluginSource(source)))) {
        host.showStatus(t('Install cancelled.', 'Установка отменена.'));
        return;
      }
      const spinner = host.showProgressSpinner(t(`Installing plugin from ${truncateForStatus(source)}…`, `Установка плагина из ${truncateForStatus(source)}…`));
      try {
        await installPluginFromSource(host, source);
        spinner.stop({ ok: true, label: t('Install finished — see details below.', 'Установка завершена — подробности ниже.') });
      } catch (error) {
        spinner.stop({ ok: false, label: t(`Install failed: ${formatErrorMessage(error)}`, `Ошибка установки: ${formatErrorMessage(error)}`) });
        throw error;
      }
      return;
    }
    ...
    if (sub === 'mcp') {
      ...
      if ((action !== 'enable' && action !== 'disable') || id === undefined || server === undefined) {
        host.showError(t('Usage: /plugins mcp enable|disable <id> <server>', 'Использование: /plugins mcp enable|disable <id> <сервер>'));
        return;
      }
      await session.setPluginMcpServerEnabled(id, server, action === 'enable');
      host.showStatus(
        action === 'enable'
          ? t(`Enabled MCP server ${server} for ${id}. Run /reload or /new to apply.`, `MCP-сервер ${server} включён для ${id}. Выполните /reload или /new, чтобы применить изменения.`)
          : t(`Disabled MCP server ${server} for ${id}. Run /reload or /new to apply.`, `MCP-сервер ${server} отключён для ${id}. Выполните /reload или /new, чтобы применить изменения.`),
      );
      return;
    }
    ...
    if (sub === 'remove') {
      const id = rest[0];
      if (id === undefined) {
        host.showError(t('Usage: /plugins remove <id>', 'Использование: /plugins remove <id>'));
        return;
      }
      if (!(await confirmRemovePlugin(host, id))) {
        host.showStatus(t(`Remove cancelled: ${id}.`, `Удаление отменено: ${id}.`));
        return;
      }
    ...
    host.showError(t(`Unknown /plugins action: ${sub}. Run /plugins to choose interactively.`, `Неизвестное действие /plugins: ${sub}. Выполните /plugins для интерактивного выбора.`));
  } catch (error) {
    host.showError(t(`/plugins ${sub ?? ''} failed: ${formatErrorMessage(error)}`, `Ошибка /plugins ${sub ?? ''}: ${formatErrorMessage(error)}`));
  }
}
...
async function showPluginsPicker(
  ...
  try {
    ...
    host.showError(t(`Failed to load plugins: ${formatErrorMessage(error)}`, `Не удалось загрузить плагины: ${formatErrorMessage(error)}`));
    return;
  }
  ...
  const panel = new PluginsPanelComponent({
    ...
    onSelect: (selection) => {
      ...
      void handlePluginsPanelSelection(host, panel, selection).catch((error: unknown) => {
        host.showError(t(`/plugins failed: ${formatErrorMessage(error)}`, `Ошибка /plugins: ${formatErrorMessage(error)}`));
      });
    },
    onCancel: () => {
      host.restoreEditor();
...
async function showPluginMcpPicker(
  ...
  try {
    ...
    host.showError(t(`Failed to load plugin MCP servers: ${formatErrorMessage(error)}`, `Не удалось загрузить MCP-серверы плагина: ${formatErrorMessage(error)}`));
    return;
  }
    ...
    new PluginMcpSelectorComponent({
      ...
      onSelect: (selection) => {
        ...
        void handlePluginMcpSelection(host, selection).catch((error: unknown) => {
          host.showError(t(`/plugins mcp failed: ${formatErrorMessage(error)}`, `Ошибка /plugins mcp: ${formatErrorMessage(error)}`));
        });
      },
      onCancel: () => {
        host.restoreEditor();
...
async function confirmInstallTrust(
  ...
  // MultiAI ships no implicitly trusted plugin source. The boolean remains for
  // local callers, but remote sources currently always resolve to false.
...
async function installFromPanel(
  ...
  if (!(await confirmInstallTrust(host, label, official))) {
    host.showStatus(t(`Install cancelled: ${label}.`, `Установка отменена: ${label}.`));
    host.restoreEditor();
    return;
  }
  ...
  if (official) {
    ...
    host.showStatus(t(`Installing or updating ${label} from marketplace...`, `Установка или обновление ${label} из каталога...`));
  }
  host.state.ui.requestRender();
  try {
    await installPluginFromSource(host, source);
    ...
    host.showError(t(`Failed to install ${label}: ${formatErrorMessage(error)}`, `Не удалось установить ${label}: ${formatErrorMessage(error)}`));
    return;
  }
...
async function applyPluginEnabled(
  ...
  const hasDisabledMcpServers =
    enabled && info !== undefined && info.mcpServerCount > info.enabledMcpServerCount;
  const mcpHint = hasDisabledMcpServers
    ? t(
        ` Some MCP servers are disabled; re-enable with /plugins mcp enable ${id} <server>.`,
        ` Некоторые MCP-серверы отключены; включите их командой /plugins mcp enable ${id} <server>.`,
      )
    : '';
  if (showStatus) {
    host.showStatus(t(`${enabled ? 'Enabled' : 'Disabled'} ${id}. Run /reload or /new to apply.${mcpHint}`, `${enabled ? 'Включён' : 'Отключён'} ${id}. Выполните /reload или /new, чтобы применить изменения.${mcpHint}`));
  }
  const inlineMcpHint = hasDisabledMcpServers ? t(' · MCP servers disabled', ' · MCP-серверы отключены') : '';
  ...
  switch (selection.kind) {
    ...
    case 'remove':
      if (!(await confirmRemovePlugin(host, selection.id))) {
        host.showStatus(t(`Remove cancelled: ${selection.id}.`, `Удаление отменено: ${selection.id}.`));
...
async function removePlugin(host: SlashCommandHost, id: string): Promise<void> {
  ...
  host.showStatus(t(`Removed ${id}.`, `Удалён ${id}.`));
...
async function renderPluginsList(
  ...
  const title = t(` Plugins (${currentPlugins.length}) `, ` Плагины (${currentPlugins.length}) `);
...
const pluginReloadHint = (): string =>
  t('Run /new or /reload to apply plugin changes.', 'Выполните /new или /reload, чтобы применить изменения плагинов.');

const pluginQuotaNote = (): string =>
  t('Note: This plugin consumes your quota.', 'Примечание: этот плагин расходует вашу квоту.');
...
function showPluginInstallResult(
  ...
  const serverWord = summary.mcpServerCount === 1
    ? t('server', 'сервер')
    : t('servers', 'серверов');
  const mcpHint =
    summary.mcpServerCount > 0
      ? t(
          ...
          ` Объявлено MCP-серверов: ${summary.mcpServerCount}; они включены по умолчанию и настраиваются через /plugins.`,
        )
      : '';
...
function describeInstallAction(
  ...
  if (previous === undefined) {
    return t(
      `Installed ${next.displayName}${versionFromTo(undefined, next.version)} ${sourcePhrase(sourceLabel)}`,
      `Установлен ${next.displayName}${versionFromTo(undefined, next.version)} ${sourcePhrase(sourceLabel)}`,
    );
  }
  if (sourceIdentity(previous) !== sourceIdentity(next)) {
    const prevSourceLabel = formatPluginSourceLabel(previous);
    return t(
      `Migrated ${next.displayName}: ${prevSourceLabel} → ${sourceLabel}${versionFromTo(previous.version, next.version)}`,
      `Перенесён ${next.displayName}: ${prevSourceLabel} → ${sourceLabel}${versionFromTo(previous.version, next.version)}`,
    );
  }
  return t(
    `Updated ${next.displayName}${versionFromTo(previous.version, next.version)} ${sourcePhrase(sourceLabel)}`,
    `Обновлён ${next.displayName}${versionFromTo(previous.version, next.version)} ${sourcePhrase(sourceLabel)}`,
  );
}
...
function sourcePhrase(sourceLabel: string): string {
  return sourceLabel.startsWith('via ')
    ? t(sourceLabel, `через ${sourceLabel.slice(4)}`)
    : t(`from ${sourceLabel}`, `из ${sourceLabel}`);
}
...
async function reloadPlugins(host: SlashCommandHost): Promise<void> {
  ...
  const line = t(
    ...
    `Перезагрузка: +${summary.added.length} -${summary.removed.length}` +
      (summary.errors.length > 0 ? ` (ошибок: ${summary.errors.length})` : ''),
  );
...
function pluginInlineChangeHint(): string {
  return t('run /reload or /new to apply', 'выполните /reload или /new, чтобы применить');
}
