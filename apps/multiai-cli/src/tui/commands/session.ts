...
import { LLM_NOT_SET_MESSAGE, NO_ACTIVE_SESSION_MESSAGE } from '../constant/multiai-tui';
...
export async function handleForkCommand(host: SlashCommandHost, args: string): Promise<void> {
  ...
  try {
    await host.switchToSession(
      forked,
      `Session forked (${forked.id}). To return to the original session: multiai -r ${session.id}`,
    );
  } catch (error) {
    const msg = formatErrorMessage(error);
