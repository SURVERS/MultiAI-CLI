...
export async function handleSwarmCommand(host: SlashCommandHost, args: string): Promise<void> {
  ...
  if (host.state.appState.permissionMode === 'manual') {
    showSwarmStartPermissionPrompt(host, `/swarm ${prompt}`, t('Swarm task not started.', 'Задача роя не запущена.'), (choice) =>
...
async function applySwarmMode(
  ...
  if (enabled && host.state.appState.swarmMode) {
    host.showStatus(t('Swarm mode is already on.', 'Режим роя уже включён.'));
    return;
  }
  if (!enabled && !host.state.appState.swarmMode) {
    host.showStatus(t('Swarm mode is already off.', 'Режим роя уже выключен.'));
    return;
  }
  if (enabled && host.state.appState.permissionMode === 'manual') {
    showSwarmStartPermissionPrompt(host, commandText, t('Swarm mode not enabled.', 'Режим роя не включён.'), async (choice) => {
...
async function setSwarmMode(
  ...
  try {
    ...
    host.showError(
      enabled
        ? t(`Failed to enable swarm mode: ${formatErrorMessage(error)}`, `Не удалось включить режим роя: ${formatErrorMessage(error)}`)
        : t(`Failed to disable swarm mode: ${formatErrorMessage(error)}`, `Не удалось выключить режим роя: ${formatErrorMessage(error)}`),
    );
    return false;
  }
