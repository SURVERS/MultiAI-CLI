...
const FEEDBACK_ATTACHMENT_OPTIONS: readonly ChoiceOption[] = [
  ...
  { value: 'logs', label: 'Logs only', description: 'Attach session diagnostics' },
  { value: 'logs+codebase', label: 'Logs + codebase', description: 'Attach diagnostics and codebase' },
];
...
export function promptApiKey(
  host: SlashCommandHost,
  platformName: string,
  subtitleLines: readonly string[] = ['Your key will be saved to ~/.multiai/config.toml'],
): Promise<string | undefined> {
  return new Promise((resolve) => {
