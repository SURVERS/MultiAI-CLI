...
import { showAccount, showMcpServers, showStatusReport, showUsage } from './info';
...
export { showAccount, showMcpServers, showStatusReport, showUsage } from './info';
...
async function executeSlashCommand(host: SlashCommandHost, input: string): Promise<void> {
  ...
  switch (intent.kind) {
    ...
    case 'invalid':
      ...
      host.showError(t(`Invalid slash command: /${intent.commandName}`, `Недопустимая слеш-команда: /${intent.commandName}`));
      return;
    case 'skill': {
...
async function handleBuiltInSlashCommand(
  ...
  switch (name) {
    ...
    case 'help':
      ...
      return;
    case 'version':
      host.showStatus(`MultiAI CLI v${host.state.appState.version}`);
      return;
    case 'new':
    ...
    case 'web':
      ...
      return;
    default:
      host.showError(t(`Unknown slash command: /${String(name)}`, `Неизвестная слеш-команда: /${String(name)}`));
      return;
  }
}
