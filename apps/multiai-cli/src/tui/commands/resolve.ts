...
export function resolveSlashCommandInput(options: ResolveSlashCommandInput): SlashCommandIntent {
  ...
  if (
    command !== undefined &&
    isExperimentalFlagEnabled((command as MultiAISlashCommand).experimentalFlag)
  ) {
...
export function slashBusyMessage(
  ...
  if (reason === 'streaming') {
    return t(
      `Cannot /${commandName} while streaming — press Esc or Ctrl-C first.`,
      `Нельзя выполнить /${commandName} во время потоковой передачи — сначала нажмите Esc или Ctrl-C.`,
    );
  }
  return t(
    `Cannot /${commandName} while compacting — wait for compaction to finish first.`,
    `Нельзя выполнить /${commandName} во время сжатия — сначала дождитесь его завершения.`,
  );
}
