import type { ApprovalRequest, ApprovalResponse, ToolInputDisplay } from '@multiai/sdk';
...
function defaultApprovalChoices(): ApprovalPanelChoice[] {
  return [
    { label: t('Approve once', 'Разрешить один раз'), response: 'approved' },
    { label: t('Approve for this session', 'Разрешить для этой сессии'), response: 'approved_for_session' },
    { label: t('Approve all', 'Разрешить всё'), response: 'approved_all', selected_label: 'Approve all' },
    { label: t('Reject', 'Отклонить'), response: 'rejected' },
    { label: t('Reject with feedback', 'Отклонить с комментарием'), response: 'rejected', requires_feedback: true },
  ];
}

function planRejectChoices(): ApprovalPanelChoice[] {
  return [
    { label: t('Reject', 'Отклонить'), response: 'rejected', selected_label: 'Reject' },
    { label: t('Revise', 'Доработать'), response: 'rejected', selected_label: 'Revise', requires_feedback: true },
  ];
}
...
function adaptPlanReviewChoices(display: ToolInputDisplay): ApprovalPanelChoice[] {
  const optionChoices =
    display.kind === 'plan_review' && display.options !== undefined && display.options.length >= 2
      ...
      : [{ label: t('Approve', 'Разрешить'), response: 'approved' as const, selected_label: 'Approve' }];
