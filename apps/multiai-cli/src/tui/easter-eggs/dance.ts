...
export function tryHandleDanceCommand(host: SlashCommandHost, parsed: ParsedSlashInput): boolean {
  ...
  if (sub === 'off') {
    ...
    host.showStatus(t(`Dancing — use ${cmd('/dance off')} to turn it off.`, `Танцуем — используйте ${cmd('/dance off')}, чтобы выключить.`));
  } else {
    currentDanceController.start({ hold: false });
    host.showStatus(t(`Use ${cmd('/dance on')} to keep the rainbow on.`, `Используйте ${cmd('/dance on')}, чтобы оставить радугу включённой.`));
  }
  return true;
}
