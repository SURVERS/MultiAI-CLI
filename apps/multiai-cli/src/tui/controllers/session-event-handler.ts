import type { Component, Focusable } from '@multiai/pi-tui';
...
import {
  ...
  oauthLoginRequiredStartupNotice,
...
export class SessionEventHandler {
  ...
  async syncMcpServerStatusSnapshot(session: Session): Promise<void> {
    ...
    try {
      ...
      host.showError(t(`Failed to sync MCP server status: ${message}`, `Не удалось синхронизировать состояние MCP-сервера: ${message}`));
      return;
    }
  ...
  private handleTurnEnd(event: TurnEndedEvent, sendQueued: (item: QueuedMessage) => void): void {
    ...
    if (event.reason === 'failed' && event.error?.code === 'provider.filtered') {
      this.host.showStatus(t('Turn stopped: provider safety policy blocked the response.', 'Ход остановлен: политика безопасности провайдера заблокировала ответ.'), 'error');
    }
    if (event.reason === 'blocked') {
      this.host.showStatus(t('Turn stopped: prompt hook blocked the request.', 'Ход остановлен: хук запроса заблокировал запрос.'), 'error');
    }
  ...
  private handleStepCompleted(event: TurnStepCompletedEvent): void {
    ...
    if (event.providerFinishReason === 'filtered') {
      this.host.showNotice(
        t('Provider safety policy blocked the response.', 'Политика безопасности провайдера заблокировала ответ.'),
        t(`The model output was filtered (${event.rawFinishReason ?? 'content_filter'}).`, `Вывод модели отфильтрован (${event.rawFinishReason ?? 'content_filter'}).`),
      );
      return;
    }
    ...
    const title =
      truncatedCount > 0
        ? t('Model hit max_tokens — tool call was truncated before it could run.', 'Модель достигла max_tokens — вызов инструмента был обрезан до запуска.')
        : t('Model hit max_tokens — no tool call was emitted.', 'Модель достигла max_tokens — вызов инструмента не был создан.');
    const detail = this.isAnthropicSessionActive()
      ? t('If this limit is wrong for your model, set `max_output_size` on the model alias in your MultiAI CLI config.', 'Если лимит для модели неверен, задайте `max_output_size` для её алиаса в конфигурации MultiAI CLI.')
      : undefined;
    this.host.showNotice(title, detail);
  }

  private maybeShowDebugTiming(event: TurnStepCompletedEvent): void {
    if (process.env['MULTIAI_DEBUG'] !== '1') return;
  ...
  private handleStepInterrupted(event: TurnStepInterruptedEvent): void {
    ...
    if (reason === 'aborted' || reason === undefined || reason === '') {
      ...
      if (event.message === undefined || event.message === '') {
        this.host.showStatus(t('Interrupted by user', 'Прервано пользователем'), 'error');
      } else {
    ...
    this.host.showError(
      reason === 'max_steps'
        ? t('reached per-turn step limit (max_steps)', 'достигнут лимит шагов на ход (max_steps)')
        : t(`step interrupted (${reason})`, `шаг прерван (${reason})`),
    );
  }
  ...
  private async promoteNextQueuedGoal(): Promise<boolean> {
    ...
    try {
      ...
      host.showError(t(`Failed to read upcoming goals: ${formatErrorMessage(error)}`, `Не удалось прочитать предстоящие цели: ${formatErrorMessage(error)}`));
      return false;
    }
      ...
      next.objective,
      {
        beforeSend: async () => {
          ...
          try {
            ...
            host.showError(
              t(`Queued goal started, but could not be removed from the queue: ${formatErrorMessage(error)}`, `Цель из очереди запущена, но удалить её из очереди не удалось: ${formatErrorMessage(error)}`),
            );
            await this.cancelStartedQueuedGoal(session);
            return false;
          }
  ...
  private async restoreAndCancelStartedQueuedGoal(
    ...
    try {
      ...
      this.host.showError(t(`Queued goal could not be restored: ${formatErrorMessage(error)}`, `Не удалось восстановить цель из очереди: ${formatErrorMessage(error)}`));
    }
    await this.cancelStartedQueuedGoal(session);
  }
  ...
  private async cancelStartedQueuedGoal(session: Session): Promise<void> {
    try {
      ...
      this.host.showError(t(`Queued goal could not be cancelled: ${formatErrorMessage(error)}`, `Не удалось отменить цель из очереди: ${formatErrorMessage(error)}`));
    }
  }
  ...
  private async notifyQueuedGoalWaitingOnBlocked(): Promise<void> {
    ...
    host.showNotice(
      t('Goal blocked.', 'Цель заблокирована.'),
      t('The next queued goal will start only after this goal is complete.', 'Следующая цель из очереди запустится только после завершения этой цели.'),
    );
  }
  ...
  private handleSessionError(event: ErrorEvent): void {
    ...
    if (event.code === OAUTH_LOGIN_REQUIRED_CODE) {
      this.host.showError(oauthLoginRequiredStartupNotice());
      return;
    }
    ...
    if (sessionId.length > 0) {
      this.host.showStatus(
        t('Report reproducible issues at https://github.com/SURVERS/MultiAI-CLI/issues', 'Сообщайте о воспроизводимых проблемах на https://github.com/SURVERS/MultiAI-CLI/issues'),
      );
    }
  }

  private handleSessionWarning(event: WarningEvent): void {
    this.host.showStatus(t(`Warning: ${event.message}`, `Предупреждение: ${event.message}`), 'warning');
  }
  ...
  private renderMcpServerStatus(server: McpServerStatusSnapshot): void {
    ...
    switch (server.status) {
      case 'connected': {
        const message = t(
          `MCP server "${server.name}" connected · ${server.toolCount} tool${server.toolCount === 1 ? '' : 's'} (${server.transport})`,
          `MCP-сервер "${server.name}" подключён · инструментов: ${server.toolCount} (${server.transport})`,
        );
      ...
      case 'failed': {
        const message = t(
          ...
          `Сбой MCP-сервера "${server.name}"${server.error !== undefined ? `: ${server.error}` : ''}`,
        );
      ...
      case 'needs-auth': {
        const message = t(
          ...
          `MCP-серверу "${server.name}" требуется OAuth — выполните /mcp-config login ${server.name}`,
        );
      ...
      case 'disabled':
        this.finalizeMcpServerStatusRow(
          server.name,
          t(`MCP server "${server.name}" disabled`, `MCP-сервер "${server.name}" отключён`),
          'textMuted',
        );
        return;
      case 'pending':
  ...
  private showMcpServerStatusSpinner(name: string): void {
    const { state } = this.host;
    const label = t(`MCP server "${name}" connecting…`, `Подключение MCP-сервера "${name}"…`);
  ...
  private handleSkillActivated(event: SkillActivatedEvent): void {
    ...
    this.host.appendTranscriptEntry({
      ...
      turnId: undefined,
      renderMode: 'plain',
      content: t(`Activated skill: ${event.skillName}`, `Активирован навык: ${event.skillName}`),
