...
import { compressImageForModel, persistOriginalImage, sessionMediaOriginalsDir } from '@multiai/sdk';
...
export interface EditorKeyboardHost {
  ...
  /**
   * The host's harness (MultiAITUI always has one). Its `imageLimits` drives
   * paste-time image compression; hosts without one fall back to the
   * env/built-in default.
   */
  harness?: MultiAIHarness | undefined;
...
export class EditorKeyboardController {
  ...
  install(): void {
    ...
    editor.onCtrlC = () => {
      ...
      this.armPendingExit('ctrl-c', ctrlCHint());
    };
    ...
    editor.onCtrlS = () => {
      ...
      if (!editorIsBash && text.length > 0) {
        try {
          ...
          host.showError(t(`Failed to prepare media attachment: ${formatErrorMessage(error)}`, `Не удалось подготовить медиафайл: ${formatErrorMessage(error)}`));
          return;
        }
        items.push({
          text,
  ...
  private cancelCurrentCompaction(): void {
    ...
    void session.cancelCompaction().catch((error: unknown) => {
      ...
      this.host.showError(t(`Failed to cancel compaction: ${message}`, `Не удалось отменить сжатие контекста: ${message}`));
    });
  }
  ...
  private async openExternalEditor(): Promise<void> {
    ...
    if (cmd === undefined) {
      this.host.showError(t('No editor configured. Set $VISUAL / $EDITOR, or run /editor <command>.', 'Редактор не настроен. Задайте $VISUAL / $EDITOR или выполните /editor <команда>.'));
      return;
    }
    ...
    try {
      ...
      this.host.showError(t(`External editor failed: ${msg}`, `Ошибка внешнего редактора: ${msg}`));
    } finally {
