import { Spacer } from '@multiai/pi-tui';
import type {
  Event,
  MultiAIHarness,
  Session,
...
const btwBusyNotice = (): string =>
  t(
    'Wait for /btw to finish before sending another question.',
    'Дождитесь завершения /btw, прежде чем отправлять другой вопрос.',
  );
...
export class BtwPanelController {
  ...
  private showBusyNotice(
    ...
    active.panel.addTransientNotice(btwBusyNotice());
    this.host.state.ui.requestRender();
  }
  ...
  private promptAgent(agentId: string, prompt: string, panel: BtwPanelComponent): void {
    ...
    if (session === undefined) {
      panel.markFailed(noActiveSessionMessage());
      this.host.state.ui.requestRender();
      return;
    }
    void this.withInteractiveAgent(agentId, () => session.prompt(prompt)).catch((error: unknown) => {
      panel.markFailed(t(`Failed to send /btw prompt: ${formatErrorMessage(error)}`, `Не удалось отправить запрос /btw: ${formatErrorMessage(error)}`));
      this.host.state.ui.requestRender();
    });
  }
  ...
  private async cancelAgent(agentId: string): Promise<void> {
    ...
    await this.withInteractiveAgent(agentId, () => session.cancel()).catch((error: unknown) => {
      this.host.showError(t(`Failed to cancel /btw: ${formatErrorMessage(error)}`, `Не удалось отменить /btw: ${formatErrorMessage(error)}`));
    });
  }
...
function formatBtwTurnEnd(event: TurnEndedEvent): string {
  if (event.reason === 'cancelled') {
    return t('Interrupted by user', 'Прервано пользователем');
  }
  if (event.error?.code === 'provider.filtered') {
    return t('Provider safety policy blocked the response.', 'Политика безопасности провайдера заблокировала ответ.');
  }
  if (event.error !== undefined) {
    return `[${event.error.code}] ${event.error.message}`;
  }
  if (event.reason === 'blocked') {
    return t('Prompt hook blocked the request.', 'Хук запроса заблокировал запрос.');
  }
  return t(`BTW turn ended with reason: ${event.reason}`, `Ход BTW завершён с причиной: ${event.reason}`);
}
