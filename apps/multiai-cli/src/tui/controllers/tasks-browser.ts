import type { BackgroundTaskInfo, Session } from '@multiai/sdk';
import type { Component, ProcessTerminal, TUI } from '@multiai/pi-tui';
...
export class TasksBrowserController {
  ...
  async show(): Promise<void> {
    ...
    if (session === undefined) {
      this.host.showError(t('No active session.', 'Нет активной сессии.'));
      return;
    }
    ...
    try {
      ...
      this.host.showError(
        t(`Failed to load tasks: ${error instanceof Error ? error.message : String(error)}`, `Не удалось загрузить задачи: ${error instanceof Error ? error.message : String(error)}`),
      );
      return;
    }
    if (state.tasksBrowser !== undefined) return;
  ...
  async refreshOutputViewer(opts: { silent?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      if (!opts.silent) {
        ...
        this.flash(t(`Output refresh failed: ${message}`, `Не удалось обновить вывод: ${message}`));
      }
      return;
    }
      ...
      if (!opts.silent) {
        this.flash(
          t(
            ...
            `Не удалось обновить: ${error instanceof Error ? error.message : String(error)}`,
          ),
        );
      }
      return;
  ...
  private buildCallbacks(): {
    ...
    return {
      ...
      onStopIgnored: (taskId, reason) => {
        if (reason === 'terminal') {
          this.flash(t(`${taskId} is already terminal — nothing to stop.`, `${taskId} уже завершена — останавливать нечего.`));
        }
      },
    };
  }
  ...
  private handleRefresh(): void {
    this.flash(t('Refreshing…', 'Обновление…'), 600);
    void this.refresh();
  }
  ...
  private async handleStop(taskId: string): Promise<void> {
    ...
    const session = this.host.session;
    if (session === undefined) {
      this.flash(t('No active session.', 'Нет активной сессии.'));
      return;
    }

    this.flash(t(`Stopping ${taskId}…`, `Остановка ${taskId}…`), 1500);
    try {
      await session.stopBackgroundTask(taskId, { reason: 'User initiated stop' });
      await this.refresh({ silent: true });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.flash(t(`Stop failed: ${message}`, `Не удалось остановить: ${message}`));
    }
  }
  ...
  private async handleOpenOutput(taskId: string): Promise<void> {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.flash(t(`Cannot open output: ${message}`, `Не удалось открыть вывод: ${message}`));
      return;
    }
