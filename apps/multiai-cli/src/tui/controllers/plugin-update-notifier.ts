...
export class PluginUpdateNotifier {
  ...
  private async checkAndNotify(pluginId: string): Promise<void> {
    // MultiAI does not ship or trust a default marketplace. Keep local plugins
    // intact and only perform update checks when the host explicitly injects a
    // catalog (primarily useful to enterprise distributions and tests).
    if (this.deps.loadMarketplace === undefined) return;
    ...
    try {
      ...
      this.deps.notify(
        t(
          `Update detected: ${installed.displayName} ${status.latest} is available. Run /plugins to update it from the configured marketplace.`,
          `Обнаружено обновление: доступна версия ${installed.displayName} ${status.latest}. Запустите /plugins, чтобы обновить плагин из настроенного каталога.`,
        ),
      );
