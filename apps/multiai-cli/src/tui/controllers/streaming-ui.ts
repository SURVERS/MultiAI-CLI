...
/** A mounted assistant text block together with its transcript entry. */
type AssistantStreamBlock = { component: AssistantMessageComponent; entry: TranscriptEntry };
...
export class StreamingUIController {
  ...
  private _streamingBlock: AssistantStreamBlock | null = null;
  /**
   * The block that owns the current step's assistant text. A tool call closes
   * the live block, but text the model emits afterwards is still part of the
   * same assistant message, so it merges back into this block instead of
   * opening a second bulleted block below the tool card.
   */
  private _stepAssistantBlock: AssistantStreamBlock | null = null;
  ...
  setTurnId(turnId: string | undefined): void {
    if (turnId !== this._currentTurnId) this._stepAssistantBlock = null;
    this._currentTurnId = turnId;
  }

  setStep(step: number): void {
    // A new step is a new assistant message, so its text opens its own block.
    if (step !== this._currentStep) this._stepAssistantBlock = null;
  ...
  appendAssistantDelta(delta: string): void {
    if (this._streamingBlock === null) {
      const resumed = this.resumableStepBlock();
      if (resumed === null) {
        this.onStreamingTextStart();
      } else {
        // Resume the step's block: the deltas already carry every space the
        // model emitted, and a tool call can interrupt the stream mid-word, so
        // the halves are concatenated verbatim. Inserting any separator here
        // would corrupt a word split across the interruption.
        this._streamingBlock = resumed;
        this._assistantDraft = resumed.entry.content;
      }
    }
  ...
  /** The current step's text block, when it is still mounted and can absorb more
   *  text. Transcript folding may have dropped it, in which case the caller has
   *  to open a fresh block rather than write into a detached component. */
  private resumableStepBlock(): AssistantStreamBlock | null {
    const block = this._stepAssistantBlock;
    if (block === null) return null;
    if (!this.host.state.transcriptContainer.children.includes(block.component)) {
      this._stepAssistantBlock = null;
      return null;
    }
    return block;
  }
  ...
  // ---------------------------------------------------------------------------
  // Dispose helpers (moved from MultiAITUI)
  // ---------------------------------------------------------------------------
  ...
  finalizeTurn(sendQueued: (item: QueuedMessage) => void): void {
    ...
    notifyTerminalOnce(state, `turn-complete:${completedTurnKey}`, {
      title: t('MultiAI CLI task complete', 'Задача MultiAI CLI выполнена'),
  ...
  onStreamingTextStart(): void {
    ...
    this._stepAssistantBlock = this._streamingBlock;
  ...
  /** Replay counterpart of the delta path: mounts a finished chunk of assistant
   *  text, merging it into the step's existing block the same way live streaming
   *  does so a resumed session renders identically. */
  renderReplayAssistantText(text: string): void {
    const resumed = this.resumableStepBlock();
    if (resumed === null) {
      this.onStreamingTextStart();
      ...
      this._streamingBlock = resumed;
      this.onStreamingTextUpdate(resumed.entry.content + text);
    }
    this.onStreamingTextEnd();
  }
  ...
  onThinkingUpdate(fullText: string): void {
    ...
    if (this._activeThinkingComponent === undefined) {
      this._pendingAgentGroup = null;
      this._pendingReadGroup = null;
      // Text that follows a thinking block cannot merge back above it without
      // reordering the transcript, so it opens its own block.
      this._stepAssistantBlock = null;
  ...
  beginCompaction(instruction?: string): void {
    ...
    const block = new CompactionComponent(state.ui, instruction, tipText(currentWorkingTip()));
