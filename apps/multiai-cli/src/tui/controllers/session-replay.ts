...
export class SessionReplayRenderer {
  ...
  async hydrateFromReplay(session: Session): Promise<boolean> {
    ...
    try {
      ...
      if (main === undefined) {
        this.host.showError(t('Session history is unavailable for this session.', 'История этой сессии недоступна.'));
        return false;
      }
      ...
      this.host.showError(t(`Failed to replay session history: ${message}`, `Не удалось воспроизвести историю сессии: ${message}`));
      return false;
    } finally {
    ...
    switch (record.type) {
      ...
      case 'plan_updated':
        ...
        this.host.appendTranscriptEntry(
          replayEntry(context, 'status', t(`Plan mode: ${record.enabled ? 'ON' : 'OFF'}`, `Режим планирования: ${record.enabled ? 'ВКЛ' : 'ВЫКЛ'}`), 'notice'),
        );
        return;
  ...
  private flushAssistant(context: ReplayRenderContext): void {
    ...
    if (text.length > 0) {
      streamingUI.renderReplayAssistantText(text);
  ...
  private renderSkillActivation(
    ...
    this.host.appendTranscriptEntry({
      ...replayEntry(context, 'skill_activation', t(`Activated skill: ${skill.skillName}`, `Активирован навык: ${skill.skillName}`), 'plain'),
  ...
  private renderCompaction(context: ReplayRenderContext, record: CompactionReplayRecord): void {
    ...
    if (record.result === 'cancelled') {
      this.host.appendTranscriptEntry({
        ...replayEntry(context, 'status', t('Compaction cancelled', 'Сжатие отменено'), 'plain'),
        compactionData: {
          result: 'cancelled',
    ...
    this.host.appendTranscriptEntry({
      ...replayEntry(context, 'status', t('Compaction complete', 'Сжатие завершено'), 'plain'),
      compactionData: {
  ...
  private renderGoalReplayRecord(context: ReplayRenderContext, record: GoalReplayRecord): void {
    ...
    switch (change.kind) {
      case 'created':
        this.host.appendTranscriptEntry({
          ...replayEntry(context, 'goal', t('Goal set', 'Цель задана'), 'plain'),
  ...
  private renderPermissionUpdate(context: ReplayRenderContext, mode: PermissionMode): void {
    if (mode === 'yolo') {
      this.host.appendTranscriptEntry(
        replayEntry(context, 'status', t('YOLO mode: ON', 'Режим YOLO: ВКЛ'), 'notice', {
          detail: t('Tool actions auto-approved; the agent may still ask you questions.', 'Действия инструментов подтверждаются автоматически; агент всё ещё может задавать вопросы.'),
        }),
      );
      return;
    }
    this.host.appendTranscriptEntry(
      replayEntry(
        ...
        'status',
        mode === 'manual'
          ? t('YOLO mode: OFF', 'Режим YOLO: ВЫКЛ')
          : t(`Permission mode: ${mode}`, `Режим разрешений: ${mode}`),
        'notice',
      ),
    );
  }
  ...
  private renderApprovalResult(
    ...
    switch (result.decision) {
      case 'approved':
        parts.push(result.scope === 'session' ? t('Approved for session', 'Подтверждено для сессии') : t('Approved', 'Подтверждено'));
        break;
      case 'rejected':
      ...
      case 'rejected':
        content =
          result.selectedLabel === 'Revise'
            ? t('Plan sent back for revision', 'План отправлен на доработку')
            : t('Plan review rejected', 'Проверка плана отклонена');
        break;
      case 'cancelled':
        content = t('Plan review cancelled', 'Проверка плана отменена');
        break;
    }
    const detail =
      result.feedback !== undefined && result.feedback.length > 0
        ? t(`Feedback: ${result.feedback}`, `Отзыв: ${result.feedback}`)
        : undefined;
...
function goalLifecycleReplayContent(change: GoalReplayLifecycleChange): string {
  switch (change.status) {
    case 'paused':
      return t('Goal paused', 'Цель приостановлена');
    case 'active':
      return t('Goal resumed', 'Цель возобновлена');
    case 'blocked':
      return t('Goal blocked', 'Цель заблокирована');
    case 'complete':
    case undefined:
      return t('Goal updated', 'Цель обновлена');
  }
}
