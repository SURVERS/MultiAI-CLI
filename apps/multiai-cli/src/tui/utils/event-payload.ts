...
function formatProviderFilteredMessage(
  ...
  return t(
    `Provider filtered the response before visible output (finishReason=${normalizedFinishReason}${raw}).`,
    `Провайдер отфильтровал ответ до появления видимого вывода (finishReason=${normalizedFinishReason}${raw}).`,
  );
}
