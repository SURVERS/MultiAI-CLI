import type { ContentPart, ContextMessage, PromptOrigin, ToolCall } from '@multiai/sdk';
...
export function formatContentPartMd(part: ContentPart): string {
  switch (part.type) {
    ...
    case 'think':
      ...
      return `<details><summary>${t('Thinking', 'Размышления')}</summary>\n\n${part.think}\n\n</details>`;
    case 'image_url':
      return t('[image]', '[изображение]');
    case 'audio_url':
...
export function formatToolCallMd(tc: ToolCall): string {
  ...
  let title = t(`#### Tool Call: ${tc.name}`, `#### Вызов инструмента: ${tc.name}`);
  if (hint) {
    title += ` (\`${hint}\`)`;
  }
...
function formatToolResultMd(msg: ContextMessage, toolName: string, hint: string): string {
  ...
  let summary = t(`Tool Result: ${toolName}`, `Результат инструмента: ${toolName}`);
