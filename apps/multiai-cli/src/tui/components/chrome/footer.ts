...
import { ALL_TIPS, tipText, type ToolbarTip } from '#/tui/constant/tips';
...
function tipsForIndex(index: number): { primary: string; pair: string | null } {
  ...
  const currentText = tipText(current);
  if (n === 1 || current.solo) return { primary: currentText, pair: null };
  const next = ROTATION[(offset + 1) % n]!;
  const nextText = tipText(next);
  if (next.solo || nextText === currentText) return { primary: currentText, pair: null };
  ...
  const turns =
    goal.budget.turnBudget !== null
      ? t(
          ...
          `${goal.turnsUsed}/${goal.budget.turnBudget} ходов`,
        )
      : goal.turnsUsed === 1
        ? t(`${goal.turnsUsed} turn`, `${goal.turnsUsed} ход`)
        : t(`${goal.turnsUsed} turns`, `${goal.turnsUsed} ходов`);
  const status =
    goal.status === 'active'
      ? t('active', 'активна')
      : goal.status === 'paused'
        ? t('paused', 'приостановлена')
        : t('blocked', 'заблокирована');
  const label = `${status} · ${formatBadgeElapsed(wallClockMs ?? goal.wallClockMs)} · ${turns}`;
  return (
    chalk.hex(colors.textMuted)(t('[goal ', '[цель ')) +
    chalk.hex(dotColor)('●') +
...
function formatContextStatus(usage: number, tokens?: number, maxTokens?: number): string {
  if (maxTokens !== undefined && maxTokens > 0 && tokens !== undefined) {
    ...
    return t(
      `context: ${pct}% (${formatTokenCount(tokens)}/${formatTokenCount(maxTokens)})`,
      `контекст: ${pct}% (${formatTokenCount(tokens)}/${formatTokenCount(maxTokens)})`,
    );
  }
  return t(
    `context: ${String(usagePercentFromRatio(usage))}%`,
    `контекст: ${String(usagePercentFromRatio(usage))}%`,
  );
}
...
export class FooterComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    if (state.permissionMode === 'auto')
      modes.push(chalk.hex(colors.warning).bold(t('auto', 'авто')));
    if (state.permissionMode === 'yolo') modes.push(chalk.hex(colors.warning).bold('yolo'));
    if (state.planMode) modes.push(chalk.hex(colors.primary).bold(t('plan', 'план')));
    if (state.swarmMode) modes.push(chalk.hex(colors.accent).bold(t('swarm', 'рой')));
      ...
      const thinkingLabel =
        effort !== 'off'
          ? hasEfforts && effort !== 'on'
            ? t(` thinking: ${effort}`, ` рассуждение: ${effort}`)
            : t(' thinking', ' рассуждение')
          : '';
    ...
    if (this.backgroundBashTaskCount > 0) {
      const badge =
        this.backgroundBashTaskCount === 1
          ? t(`[${String(this.backgroundBashTaskCount)} task running]`, `[выполняется ${String(this.backgroundBashTaskCount)} задача]`)
          : t(`[${String(this.backgroundBashTaskCount)} tasks running]`, `[выполняется задач: ${String(this.backgroundBashTaskCount)}]`);
      left.push(chalk.hex(colors.primary)(badge));
    }
    if (this.backgroundAgentCount > 0) {
      const badge =
        this.backgroundAgentCount === 1
          ? t(`[${String(this.backgroundAgentCount)} agent running]`, `[работает ${String(this.backgroundAgentCount)} агент]`)
          : t(`[${String(this.backgroundAgentCount)} agents running]`, `[работает агентов: ${String(this.backgroundAgentCount)}]`);
      left.push(chalk.hex(colors.primary)(badge));
    }
