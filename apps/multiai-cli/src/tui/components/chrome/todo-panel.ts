...
import { truncateToWidth } from '@multiai/pi-tui';
...
export class TodoPanelComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    const lines: string[] = [
      ...
      chalk.hex(c.primary).bold(t('  Todo', '  Задачи')),
    ];
    ...
    if (this.expanded) {
      ...
      if (this.todos.length > MAX_VISIBLE) {
        lines.push(
          chalk.hex(c.textDim)(t(`  all ${String(this.todos.length)} items · ctrl+t to collapse`, `  все задачи: ${String(this.todos.length)} · ctrl+t — свернуть`)),
        );
      }
    } else {
      ...
      if (hidden > 0) {
        ...
        lines.push(
          chalk.hex(c.textDim)(t(`  … +${hidden} more${suffix} · ctrl+t to expand`, `  … ещё ${hidden}${suffix} · ctrl+t — развернуть`)),
        );
      }
    }
...
const STATUS_LABELS: readonly { status: TodoStatus; label: string }[] = [
  { status: 'done', label: t('done', 'готово') },
  { status: 'in_progress', label: t('in progress', 'в работе') },
  { status: 'pending', label: t('pending', 'ожидают') },
];
