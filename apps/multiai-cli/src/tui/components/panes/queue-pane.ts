import { Container, truncateToWidth, visibleWidth } from '@multiai/pi-tui';
      ...
      this.hint =
        options.isCompacting && !options.isStreaming
          ? t(
              ...
              '  ↑ — изменить · будет отправлено после сжатия',
            )
          : canSteer
            ? t(
                '  ↑ to edit · ctrl-s to steer immediately',
                '  ↑ — изменить · ctrl-s — направить сразу',
              )
            : t(
                '  ↑ to edit · will send after current task',
                '  ↑ — изменить · будет отправлено после текущей задачи',
              );
    }
  }
