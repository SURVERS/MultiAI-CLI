import type { Component, MarkdownTheme } from '@multiai/pi-tui';
import {
  Markdown,
  Text,
  truncateToWidth,
...
export class BtwPanelComponent implements Component {
  ...
  private renderTopBorder(width: number, truncated: boolean): string {
    ...
    const hint =
      truncated && this.options.canUseScrollKeys()
        ? t('Esc close · ↑↓ scroll ', 'Esc закрыть · ↑↓ прокрутка ')
        : t('Esc close ', 'Esc закрыть ');
    const title =
    ...
    if (this.turns.length === 0) {
      lines.push(
        chalk.hex(currentTheme.palette.textDim)(
          t('Ready for a side question...', 'Можно задать дополнительный вопрос...'),
        ),
      );
    }
  ...
  private renderTurn(turn: BtwTurn, width: number): string[] {
    ...
    if (answer.length > 0) {
      ...
      lines.push(
        chalk.hex(currentTheme.palette.textDim)(t('Waiting for answer...', 'Ожидание ответа...')),
      );
    }
