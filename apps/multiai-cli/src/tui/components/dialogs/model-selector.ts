import { effectiveModelAlias, type ModelAlias, type ThinkingEffort } from '@multiai/sdk';
import {
  Container,
  Key,
  matchesKey,
...
export function effortLabel(effort: string): string {
  const labels: Readonly<Record<string, string>> = {
    ...
    on: t('On', 'Вкл.'),
    low: t('Low', 'Низкое'),
    medium: t('Medium', 'Среднее'),
    high: t('High', 'Высокое'),
    max: t('Max', 'Максимальное'),
  };
  return labels[effort] ?? (effort.length === 0 ? effort : effort.charAt(0).toUpperCase() + effort.slice(1));
}
...
export class ModelSelectorComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const titleSuffix =
      searchable && view.query.length === 0
        ? currentTheme.fg('textMuted', t('  (type to search)', '  (введите текст для поиска)'))
        : '';
    ...
    if (this.opts.providerSwitchHint) hintParts.push(t('Tab toggle provider', 'Tab — сменить провайдера'));
    hintParts.push(t('↑↓ navigate', '↑↓ — навигация'));
    if (searchable && view.query.length > 0) hintParts.push(t('Backspace clear', 'Backspace — очистить'));
    hintParts.push(t('Enter select', 'Enter — выбрать'));
    if (this.opts.onSessionOnlySelect !== undefined) hintParts.push(t('Alt+S session-only', 'Alt+S — только для сеанса'));
    hintParts.push(t('Esc cancel', 'Esc — отмена'));

    const lines: string[] = [
      currentTheme.fg('primary', '─'.repeat(width)),
      currentTheme.boldFg('primary', t(' Select a model', ' Выберите модель')) + titleSuffix,
    ...
    if (view.items.length === 0) {
      lines.push(currentTheme.fg('textMuted', t('   No matches', '   Совпадений нет')));
    } else {
    ...
    if (view.query.length > 0) {
      ...
      if (below > 0) {
        lines.push('');
        lines.push(currentTheme.fg('textMuted', t(` ▼ ${String(below)} more`, ` ▼ ещё ${String(below)}`)));
      }
    }
    ...
    if (selected !== undefined) {
      ...
      const thinkingHeader = canSwitch
        ? t(' Thinking  (←→ to switch)', ' Рассуждение  (←→ — переключить)')
  ...
  private renderThinkingControl(choice: ModelChoice): string {
    ...
    const unavailable = (label: string): string =>
      currentTheme.fg('textMuted', t(`  ${label} (Unsupported)  `, `  ${label} (не поддерживается)  `));
    ...
    if (efforts.length === 0 && availability === 'always-on') {
      return `  ${segment(effortLabel('on'), true)} ${unavailable(effortLabel('off'))}`;
    }
    if (efforts.length === 0 && availability === 'unsupported') {
      return `  ${unavailable(effortLabel('on'))} ${segment(effortLabel('off'), true)}`;
    }
