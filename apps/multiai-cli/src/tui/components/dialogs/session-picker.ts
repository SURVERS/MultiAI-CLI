...
export class SessionPickerComponent extends Container implements Focusable {
  ...
  private renderLines(width: number): string[] {
    ...
    const title = this.scope === 'all' ? t('All sessions', 'Все сессии') : t('Sessions', 'Сессии');
    const scopeHint =
      this.onToggleScope === undefined
        ? undefined
        : this.scope === 'all'
          ? t('Ctrl+A current cwd', 'Ctrl+A текущий каталог')
          : t('Ctrl+A all', 'Ctrl+A все');

    if (this.loading) {
      lines.push(currentTheme.boldFg('primary', truncateToWidth(title, width, ELLIPSIS)));
      lines.push(
        currentTheme.fg('textMuted', truncateToWidth(t('Loading sessions...', 'Загрузка сессий...'), width, ELLIPSIS)),
      );
    ...
    if (this.sessions.length === 0) {
      const hintParts = [scopeHint, t('Esc cancel', 'Esc — отмена')].filter(
      ...
      lines.push('');
      lines.push(
        currentTheme.fg('textMuted', truncateToWidth(t('No sessions found.', 'Сессии не найдены.'), width, ELLIPSIS)),
      );
      lines.push(currentTheme.fg('primary', '─'.repeat(width)));
      return lines;
    }
    ...
    const view = this.list.view();
    const titleSuffix =
      view.query.length === 0 ? currentTheme.fg('textMuted', t('  (type to search)', '  (введите для поиска)')) : '';
    const hintParts = [
      ...(view.query.length > 0 ? [t('Backspace clear', 'Backspace — очистить')] : []),
      t('↑↓ navigate', '↑↓ — навигация'),
      scopeHint,
      t('Enter select', 'Enter — выбрать'),
      t('Esc cancel', 'Esc — отмена'),
    ...
    if (view.query.length > 0) {
      lines.push(currentTheme.fg('primary', t('Search: ', 'Поиск: ')) + currentTheme.fg('text', view.query));
    }

    const loadedSessions = this.loadedSessions(view.items);
    if (loadedSessions.length === 0) {
      lines.push(currentTheme.fg('textMuted', truncateToWidth(t('No matches', 'Нет совпадений'), width, ELLIPSIS)));
      lines.push(currentTheme.fg('primary', '─'.repeat(width)));
      return lines;
    }
    ...
    if (loadedSessions.length > visibleSessions.length || view.query.length > 0) {
      ...
      const totalSuffix =
        view.query.length > 0
          ? t(`${String(loadedSessions.length)} loaded / ${String(filteredCount)} matches`, `${String(loadedSessions.length)} загружено / ${String(filteredCount)} совпадений`)
          : loadedSessions.length === this.sessions.length
            ? t(`${String(loadedSessions.length)} sessions`, `Сессий: ${String(loadedSessions.length)}`)
            : t(`${String(loadedSessions.length)} loaded / ${String(this.sessions.length)} sessions`, `${String(loadedSessions.length)} загружено / ${String(this.sessions.length)} сессий`);
      const footer = t(`Showing ${String(visibleStart + 1)}-${String(visibleStart + visibleSessions.length)} of ${totalSuffix}`, `Показаны ${String(visibleStart + 1)}-${String(visibleStart + visibleSessions.length)} из ${totalSuffix}`);
