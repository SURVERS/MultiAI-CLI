...
export class UndoSelectorComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const hintParts = [t('↑↓ navigate', '↑↓ — навигация'), t('Enter select', 'Enter — выбрать'), t('Esc cancel', 'Esc — отмена')];

    const lines: string[] = [
      currentTheme.fg('primary', '─'.repeat(width)),
      currentTheme.boldFg('primary', t(' Select messages to undo', ' Выберите сообщения для отмены')),
      currentTheme.fg('textMuted', ' ' + hintParts.join(' · ')),
      '',
    ];

    if (view.items.length === 0) {
      lines.push(currentTheme.fg('textMuted', t('   No messages', '   Нет сообщений')));
    } else {
