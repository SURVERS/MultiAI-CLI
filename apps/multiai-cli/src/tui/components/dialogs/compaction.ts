...
import { Container, Text, Spacer } from '@multiai/pi-tui';
...
export class CompactionComponent extends Container {
  ...
  private buildHeader(): string {
    if (this.done) {
      ...
      const label = currentTheme.boldFg('success', t('Compaction complete', 'Сжатие завершено'));
      const detail =
        this.tokensBefore !== undefined && this.tokensAfter !== undefined
          ? currentTheme.dim(
              t(
                ` (${String(this.tokensBefore)} → ${String(this.tokensAfter)} tokens)`,
                ` (${String(this.tokensBefore)} → ${String(this.tokensAfter)} токенов)`,
              ),
            )
          : '';
      const shortcutHint =
        this.summary !== undefined && this.summary.length > 0
          ? currentTheme.dim(
              this.expanded
                ? t(' (Ctrl-O to hide compaction summary)', ' (Ctrl-O — скрыть сводку сжатия)')
                : t(' (Ctrl-O to show compaction summary)', ' (Ctrl-O — показать сводку сжатия)'),
            )
          : '';
    ...
    if (this.canceled) {
      ...
      const label = currentTheme.boldFg('warning', t('Compaction cancelled', 'Сжатие отменено'));
      return `${bullet}${label}`;
    }
    const bullet = this.blinkOn ? currentTheme.fg('text', STATUS_BULLET) : '  ';
    const label = currentTheme.boldFg('primary', t('Compacting context...', 'Сжатие контекста...'));
    const tip = this.tip
      ? currentTheme.fg('textDim', ` · ${t('Tip', 'Совет')}: ${this.tip}`)
      : '';
