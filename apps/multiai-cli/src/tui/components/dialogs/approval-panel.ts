...
function renderShellDisplayBlock(
  ...
  if (block.danger !== undefined) {
    lines.push(s.errorBold(t(`Dangerous: ${block.danger}`, `Опасно: ${block.danger}`)));
  }
  ...
  switch (block.type) {
    case 'diff':
      return renderDiffLinesClustered(block.old_text, block.new_text, block.path, {
        contextLines: 3,
        expandKeyHint: t('ctrl+e to preview', 'ctrl+e — предпросмотр'),
    ...
    case 'file_content': {
      ...
      if (remaining > 0) {
        lines.push(
          s.dim(
            remaining === 1
              ? t('     … 1 more line hidden (ctrl+e to preview)', '     … скрыта ещё 1 строка (ctrl+e — предпросмотр)')
              : t(
                  `     … ${String(remaining)} more lines hidden (ctrl+e to preview)`,
                  `     … скрыто ещё строк: ${String(remaining)} (ctrl+e — предпросмотр)`,
                ),
          ),
        );
      }
    ...
    case 'search': {
      const lines = [`${s.accent(t('search', 'поиск'))} ${s.strong(block.query)}`];
      if (block.scope !== undefined && block.scope.length > 0) {
        lines.push(s.dim(t(`scope: ${block.scope}`, `область: ${block.scope}`)));
      }
      return lines;
    }
    case 'invocation': {
    ...
    case 'background_task':
      return [
        s.strong(t(
          ...
          `${block.status} · задача ${block.kind} ${block.task_id}: ${block.description}`,
        )),
      ];
    case 'todo':
...
function headerFor(toolName: string): string {
  switch (toolName) {
    case 'Bash':
      return t('Run this command?', 'Выполнить эту команду?');
    case 'Write':
      return t('Write this file?', 'Записать этот файл?');
    case 'Edit':
      return t('Apply these edits?', 'Применить эти изменения?');
    case 'TaskStop':
      return t('Stop this task?', 'Остановить эту задачу?');
    case 'ExitPlanMode':
      return t('Ready to build with this plan?', 'Начать работу по этому плану?');
    default:
      return t(`Approve ${toolName}?`, `Разрешить ${toolName}?`);
  }
}
...
export class ApprovalPanelComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    lines.push('');
    if (this.feedbackMode) {
      lines.push(indent(dim(t('Type feedback · ↵ submit.', 'Введите комментарий · ↵ отправить.'))));
    } else {
      const expandHint = hasPreviewable
        ? t(' · ctrl+e preview', ' · ctrl+e — предпросмотр')
        : '';
      lines.push(
        indent(
          dim(
            t(
              ...
              `↑/↓ выбрать · ${buildNumericHint(data.choices.length)} выбор · ↵ подтвердить${expandHint}`,
            ),
          ),
        ),
      );
