...
export class ChoicePickerComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const navParts = [t('↑↓ navigate', '↑↓ — навигация')];
    if (view.page.pageCount > 1) navParts.push(t('←→ page', '←→ страница'));
    navParts.push(t('Enter select', 'Enter — выбрать'), t('Esc cancel', 'Esc — отмена'));
    const hint = this.opts.hint ?? navParts.join(' · ');

    const titleSuffix =
      searchable && view.query.length === 0
        ? currentTheme.fg('textMuted', t('  (type to search)', '  (введите для поиска)'))
        : '';
