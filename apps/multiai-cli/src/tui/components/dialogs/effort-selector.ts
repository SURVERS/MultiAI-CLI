...
import type { ThinkingEffort } from '@multiai/sdk';
...
export class EffortSelectorComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    const hintParts = [t('←→ switch', '←→ — переключить'), t('Enter select', 'Enter — выбрать')];
    if (this.opts.onSessionOnlySelect !== undefined) {
      hintParts.push(t('Alt+S session-only', 'Alt+S только для сессии'));
    }
    ...
    const lines: string[] = [
      ...
      currentTheme.boldFg(
        'primary',
        ` ${this.opts.title ?? t('Select thinking effort', 'Выберите глубину размышлений')}`,
      ),
