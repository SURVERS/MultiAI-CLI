...
import type { PluginInfo, PluginMcpServerInfo, PluginSummary } from '@multiai/sdk';
...
export class PluginMcpSelectorComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const lines: string[] = [
      ...
      chalk.hex(colors.primary).bold(t(` MCP servers · ${info.displayName}`, ` MCP-серверы · ${info.displayName}`)),
      mutedHintLine(t(' ↑↓ navigate · Enter/Space enable/disable · Esc cancel', ' ↑↓ — навигация · Enter/Пробел — включить/выключить · Esc — отмена'), colors),
      '',
      sectionLabel(t(`MCP servers (${info.enabledMcpServerCount}/${info.mcpServerCount} enabled)`, `MCP-серверы (включено ${info.enabledMcpServerCount}/${info.mcpServerCount})`), colors),
    ];

    if (serverItems.length === 0) {
      lines.push(chalk.hex(colors.textMuted)(t('  No MCP servers declared.', '  MCP-серверы не объявлены.')));
    } else {
    ...
    lines.push('');
    lines.push(sectionLabel(t('Actions', 'Действия'), colors));
...
export class PluginRemoveConfirmComponent extends ChoicePickerComponent {
  constructor(opts: PluginRemoveConfirmOptions) {
    super({
      title: t(`Remove ${opts.displayName} (${opts.id})?`, `Удалить ${opts.displayName} (${opts.id})?`),
      hint: t('↑↓ navigate · Enter/Space select · ←/Esc cancel', '↑↓ — навигация · Enter/Пробел — выбрать · ←/Esc — отмена'),
      formatHint: mutedHintLine,
      options: [
        {
          value: REMOVE_CONFIRM_CANCEL,
          label: t('Cancel', 'Отмена'),
          description: t('Keep this plugin installed.', 'Оставить плагин установленным.'),
        },
        {
          value: REMOVE_CONFIRM_REMOVE,
          label: t('Remove plugin', 'Удалить плагин'),
          tone: 'danger',
          description: t('Remove only the install record; plugin files are left in place.', 'Удалить только запись об установке; файлы плагина останутся на месте.'),
        },
      ],
      onSelect: (value) => {
 ...
 * to proceed with a plugin that MultiAI has not reviewed.
 */
export class PluginInstallTrustConfirmComponent extends ChoicePickerComponent {
  constructor(opts: PluginInstallTrustConfirmOptions) {
    super({
      title: t(`Install third-party plugin ${opts.label}?`, `Установить сторонний плагин ${opts.label}?`),
      hint: t('↑↓ navigate · Enter/Space select · ←/Esc cancel', '↑↓ — навигация · Enter/Пробел — выбрать · ←/Esc — отмена'),
      formatHint: mutedHintLine,
      notice: t(
        '⚠️ This is a third-party plugin that MultiAI has not reviewed. It can bundle MCP servers, skills, or files that run code and access your workspace. Install it only if you trust the source.',
        '⚠️ Это сторонний плагин, который MultiAI не проверял. Он может содержать MCP-серверы, навыки или файлы, запускающие код и получающие доступ к рабочей области. Устанавливайте его, только если доверяете источнику.',
      ),
      noticeTone: 'warning',
      options: [
        {
          ...
          label: t('Exit', 'Выйти'),
          description: t('Cancel the installation.', 'Отменить установку.'),
        },
        {
          value: INSTALL_TRUST_TRUST,
          label: t('Trust and install', 'Доверять и установить'),
          tone: 'danger',
          description: t('Install this third-party plugin anyway.', 'Всё равно установить этот сторонний плагин.'),
        },
      ],
      onSelect: (value) => {
...
function overviewPluginDescription(plugin: PluginSummary): string {
  const state = plugin.state === 'ok' ? '' : t(` · state ${plugin.state}`, ` · состояние ${plugin.state}`);
  const skills =
    plugin.skillCount === 1
      ? t(`${plugin.skillCount} skill`, `${plugin.skillCount} навык`)
      : t(`${plugin.skillCount} skills`, `${plugin.skillCount} навыков`);
  const mcp =
    plugin.mcpServerCount > 0
      ? ` · MCP ${plugin.enabledMcpServerCount}/${plugin.mcpServerCount}`
      : '';
  const diagnostics = plugin.hasErrors ? t(' · diagnostics available', ' · доступна диагностика') : '';
...
function pluginStatus(plugin: PluginSummary): string | undefined {
  ...
  return plugin.enabled ? t('enabled', 'включён') : t('disabled', 'отключён');
}
...
function marketplaceStatusStyle(status: string, colors: ColorPalette): (text: string) => string {
  ...
  if (status.startsWith('update') || status.startsWith('обновить')) return chalk.hex(colors.warning);
  if (status.startsWith('installed') || status.startsWith('установлен')) return chalk.hex(colors.success);
...
const PLUGINS_PANEL_TABS: readonly { id: PluginsPanelTabId; label: string }[] = [
  { id: 'installed', label: t('Installed', 'Установленные') },
  { id: 'official', label: t('Official', 'Официальные') },
  { id: 'third-party', label: t('Third-party', 'Сторонние') },
  { id: 'custom', label: t('Custom', 'Свои') },
];
...
export class PluginsPanelComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const hint =
      tab === 'installed'
        ? this.installedHint()
        : tab === 'custom'
          ? t(' Tab switch · Enter install · Esc cancel', ' Tab — сменить вкладку · Enter — установить · Esc — отмена')
          : t(' Tab switch · ↑↓ navigate · Enter open/install · Esc cancel', ' Tab — сменить вкладку · ↑↓ — навигация · Enter — открыть/установить · Esc — отмена');
    const lines: string[] = [
      chalk.hex(colors.primary)('─'.repeat(width)),
      chalk.hex(colors.primary).bold(t(' Plugins', ' Плагины')),
  ...
  private renderInstalled(lines: string[], width: number): void {
    ...
    if (installed.length === 0) {
      lines.push(chalk.hex(colors.textMuted)(t('  No plugins installed.', '  Установленных плагинов нет.')));
    } else {
    ...
    lines.push('');
    lines.push(mutedHintLine(t(` ${installed.length} installed`, ` установлено: ${installed.length}`), colors));
  }
  ...
  private installedHint(): string {
    ...
    const enter = hasUpdate ? t('Enter update', 'Enter — обновить') : t('Enter details', 'Enter — подробнее');
    return t(
      ` Tab switch · Space toggle · D remove · M MCP · ${enter} · I details · R reload · Esc cancel`,
      ` Tab — вкладка · Пробел — вкл/выкл · D — удалить · M — MCP · ${enter} · I — подробнее · R — перезагрузить · Esc — отмена`,
    );
  }
  ...
  private renderInstalledRow(plugin: PluginSummary, index: number, width: number): string[] {
    ...
    if (update !== undefined) {
      const badge = t(
        `update ${update.local} → ${update.latest}`,
        `обновить ${update.local} → ${update.latest}`,
      );
  ...
  private renderMarketplaceTab(
    ...
    if (this.market.status === 'loading' || this.market.status === 'idle') {
      lines.push(chalk.hex(colors.textMuted)(t('  Loading marketplace…', '  Загрузка каталога…')));
      return;
    }
    if (this.market.status === 'error') {
      lines.push(chalk.hex(colors.warning)(t(`  Marketplace unavailable: ${this.market.message}`, `  Каталог недоступен: ${this.market.message}`)));
      lines.push(mutedHintLine(t('  Use the Custom tab to install from a URL.', '  Используйте вкладку «Свои» для установки по URL.'), colors));
      return;
    }
    if (entries.length === 0) {
      lines.push(chalk.hex(colors.textMuted)(t('  No plugins found.', '  Плагины не найдены.')));
    } else {
    ...
    lines.push('');
    lines.push(
      mutedHintLine(t(` ${installedCount} installed · ${entries.length - installedCount} available`, ` установлено: ${installedCount} · доступно: ${entries.length - installedCount}`), colors),
    );
    lines.push(mutedHintLine(t(` Source: ${this.market.source}`, ` Источник: ${this.market.source}`), colors));
  }

  private renderOfficial(lines: string[], width: number): void {
    this.renderMarketplaceTab(lines, width, this.officialCatalogEntries);
  }
  ...
  private renderCustom(lines: string[], width: number): void {
    const colors = currentTheme.palette;
    lines.push(mutedHintLine(t(' Install from a GitHub URL (or zip URL / local path):', ' Установить по URL GitHub (или URL zip-архива / локальному пути):'), colors));
    lines.push('');
  ...
  private renderInstalling(width: number): string[] {
    ...
    const lines = [
      ...
      chalk.hex(colors.primary).bold(t(' Plugins', ' Плагины')),
      '',
      chalk.hex(colors.textMuted)(t(`  Installing ${this.installing} from marketplace…`, `  Установка ${this.installing} из каталога…`)),
      '',
      chalk.hex(colors.primary)('─'.repeat(width)),
    ];
    return lines.map((line) => truncateToWidth(line, width, ELLIPSIS));
...
function buildMcpItems(info: PluginInfo): PluginsOverviewItem[] {
  const items: PluginsOverviewItem[] = info.mcpServers.map((server) => ({
    ...
    kind: 'plugin',
    label: server.name,
    status: server.enabled ? t('enabled', 'включён') : t('disabled', 'отключён'),
    description: mcpServerDescription(server),
  }));
  items.push({
    value: 'back',
    kind: 'action',
    label: t('Back to installed plugins', 'Назад к установленным плагинам'),
    description: t('Return to the local plugin manager.', 'Вернуться к локальному менеджеру плагинов.'),
  });
  return items;
}
...
function mcpServerDescription(server: PluginMcpServerInfo): string {
  const action = server.enabled
    ? t('Enter/Space disable', 'Enter/Пробел — отключить')
    : t('Enter/Space enable', 'Enter/Пробел — включить');
...
function marketplaceTierLabel(tier: PluginMarketplaceEntry['tier']): string {
  if (tier === 'official') return t('Official plugin', 'Официальный плагин');
  if (tier === 'curated') return t('Curated plugin', 'Отобранный плагин');
  return t('Plugin', 'Плагин');
}

function installStatus(entry: PluginMarketplaceEntry): string {
  return entry.version === undefined
    ? t('install', 'установить')
    : t(`install v${entry.version}`, `установить v${entry.version}`);
}
...
function marketplaceEntryStatus(
  ...
  switch (status.kind) {
    case 'update':
      return t(
        `update ${status.local} → ${status.latest}`,
        `обновить ${status.local} → ${status.latest}`,
      );
    case 'up-to-date':
      return status.version === undefined
        ? t('installed', 'установлен')
        : t(`installed · v${status.version}`, `установлен · v${status.version}`);
    case 'not-installed':
...
function statusStyle(
  ...
  if (
    item.status === t('enabled', 'включён') ||
    item.status === t('installed', 'установлен')
  ) return chalk.hex(colors.success);
  if (item.status?.startsWith(t('install', 'установить'))) return chalk.hex(colors.primary);
  if (item.status === t('disabled', 'отключён')) return chalk.hex(colors.textDim);
