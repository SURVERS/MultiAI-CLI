 ...
 * user feedback before submitting it to the managed MultiAI CLI platform.
 *
...
const title = (): string => t('Send feedback to MultiAI CLI', 'Отправить отзыв в MultiAI CLI');
const subtitleDefault = (): string => t(
  "Tell us what's working or what's not.",
  'Расскажите, что работает, а что нет.',
);
const subtitleEmpty = (): string => t('Feedback cannot be empty.', 'Отзыв не может быть пустым.');
const footer = (): string => t('Enter to submit  ·  Esc to cancel', 'Enter — отправить  ·  Esc — отмена');
...
export class FeedbackInputDialogComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const subtitleText = this.emptyHinted ? subtitleEmpty() : subtitleDefault();
