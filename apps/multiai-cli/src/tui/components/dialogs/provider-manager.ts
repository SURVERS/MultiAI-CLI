 ...
 * MultiAI CLI OAuth (`DEFAULT_OAUTH_PROVIDER_NAME`) is intentionally hidden
 ...
 * through callbacks. The host (`MultiAITui`) is responsible for performing
 * the harness / config mutations and then pushing a fresh snapshot via
 * `setOptions`.
 */

import type { ProviderConfig } from '@multiai/sdk';
import {
  getOpenPlatformById,
  isOpenPlatformId,
...
const addRowLabel = (): string => t('[ Add New Platform ]', '[ Добавить новую платформу ]');
const PAGE_SIZE = 8;
const headerHint = (): string => t(
  '↑↓ navigate · D delete · Esc cancel',
  '↑↓ — навигация · D — удалить · Esc — отмена',
);

// Narrows a `ProviderConfig` blob to a `CustomRegistrySource` payload.
// Mirrors `readCustomRegistrySource` in `multiai-tui.ts`. We can't import
...
function buildRows(opts: ProviderManagerOptions): readonly Row[] {
  ...
  return [...sources, { kind: 'add', id: '__add__', label: addRowLabel() }];
}
...
export class ProviderManagerComponent extends Container implements Focusable {
  ...
  private armDeleteConfirm(): void {
    ...
    const prompt =
      ids.length === 1
        ? t(`Delete platform "${selected.label}"?`, `Удалить платформу «${selected.label}»?`)
        : t(
            `Delete platform "${selected.label}" and all ${String(ids.length)} providers?`,
            `Удалить платформу «${selected.label}» и всех её провайдеров (${String(ids.length)})?`,
          );
    this.confirm = {
      label: prompt,
      providerIds: ids,
  ...
  override render(width: number): string[] {
    ...
    lines.push(border);
    lines.push(currentTheme.boldFg('primary', t(' Providers', ' Провайдеры')));
    lines.push(currentTheme.fg('textMuted', ' ' + headerHint()));
    lines.push('');

    const rows = this.rows;
    if (rows.length === 0) {
      lines.push(currentTheme.fg('textMuted', t('  No providers configured.', '  Провайдеры не настроены.')));
    } else {
