...
import type { ExperimentalFeatureState } from '@multiai/sdk';
...
export class ExperimentsSelectorComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const titleSuffix =
      view.query.length === 0
        ...
        : '';
    const hintParts = [t('↑↓ navigate', '↑↓ — навигация')];
    if (view.page.pageCount > 1) hintParts.push(t('PgUp/PgDn page', 'PgUp/PgDn — страница'));
    hintParts.push(
      t('Space toggle', 'Пробел — переключить'),
      t('Enter apply', 'Enter — применить'),
      t('Esc cancel', 'Esc — отмена'),
    );
    if (view.query.length > 0) hintParts.push(t('Backspace clear', 'Backspace — очистить'));

    const lines: string[] = [
      currentTheme.fg('primary', '─'.repeat(width)),
      currentTheme.boldFg('primary', t(' Experimental features', ' Экспериментальные функции')) + titleSuffix,
      ...
      lines.push(
        currentTheme.fg(
          ...
          t(
            ...
            ` ▼ ещё ${String(view.items.length - view.page.end)}`,
          ),
        ),
      );
    }
  ...
  private renderApplyButton(): string {
    ...
    const label = t('[ Apply changes and reload ]', '[ Применить изменения и перезагрузить ]');
    const summary =
      count === 0
        ? t('no changes', 'без изменений')
        : count === 1
          ? t(`${String(count)} change`, `${String(count)} изменение`)
          : t(`${String(count)} changes`, `${String(count)} изменений`);
  ...
  private renderFeature(
    ...
    const enabled = this.effectiveEnabled(feature);
    const status = enabled ? t('enabled', 'включено') : t('disabled', 'отключено');
    ...
    const detail = this.isDraftChanged(feature)
      ? `${featureDetail(feature)} · ${t('modified', 'изменено')}`
...
function sourceLabel(feature: ExperimentalFeatureState): string {
  switch (feature.source) {
    case 'master-env':
      return t('locked by MULTIAI_EXPERIMENTAL_FLAG', 'заблокировано MULTIAI_EXPERIMENTAL_FLAG');
    case 'env':
      return t(`locked by ${feature.env}`, `заблокировано ${feature.env}`);
    case 'config':
      return t('config', 'конфигурация');
    case 'default':
      return t('default', 'по умолчанию');
  }
}
