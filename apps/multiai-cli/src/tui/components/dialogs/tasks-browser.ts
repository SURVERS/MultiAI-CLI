 ...
 * footer key hint.
 *
 * Mounted by `multiai-tui.ts` via container swap rather than `showOverlay`
...
const STATUS_LABEL: Record<BackgroundTaskStatus, string> = {
  ...
  timed_out: t('timed out', 'время истекло'),
  killed: t('killed', 'остановлено'),
  lost: t('lost', 'потеряно'),
};
...
function formatRelativeTime(ts: number | null | undefined): string {
  ...
  if (diffSec < 60) return t('just now', 'только что');
  const minutes = Math.floor(diffSec / 60);
  if (minutes < 60) return t(`${String(minutes)}m ago`, `${String(minutes)} мин назад`);
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return t(`${String(hours)}h ago`, `${String(hours)} ч назад`);
  const days = Math.floor(hours / 24);
  return t(`${String(days)}d ago`, `${String(days)} дн назад`);
}
...
export class TasksBrowserApp extends Container implements Focusable {
  ...
  private renderHeader(width: number): string {
    const title = currentTheme.boldFg('primary', t(' TASK BROWSER ', ' БРАУЗЕР ЗАДАЧ '));
    const filterText = currentTheme.fg(
      'textMuted',
      this.props.filter === 'all' ? t(' filter=ALL ', ' фильтр=ВСЕ ') : t(' filter=ACTIVE ', ' фильтр=АКТИВНЫЕ '),
    );
    ...
    if (counts.running > 0)
      countSegments.push(currentTheme.fg('success', t(` ${String(counts.running)} running `, ` выполняется: ${String(counts.running)} `)));
    if (counts.completed > 0)
      countSegments.push(currentTheme.fg('textDim', t(` ${String(counts.completed)} completed `, ` завершено: ${String(counts.completed)} `)));
    if (counts.terminalFailed > 0)
      countSegments.push(
        currentTheme.fg('error', t(` ${String(counts.terminalFailed)} interrupted `, ` прервано: ${String(counts.terminalFailed)} `)),
      );
    const totals = currentTheme.fg('textMuted', t(` ${String(visible.length)} total `, ` всего: ${String(visible.length)} `));
  ...
  private renderFooter(width: number): string {
    ...
    if (this.pendingStopTaskId !== undefined) {
      ...
      const line =
        ` ${warn(t('Stop', 'Остановить'))} ${currentTheme.fg('text', this.pendingStopTaskId)}? ` +
        `${key('Y')} ${dim(t('confirm', 'подтвердить'))}  ${key('N')}${dim('/')}${key('esc')} ${dim(t('cancel', 'отмена'))} `;
      return fitExactly(line, width);
    }

    const parts = [
      ` ${key('↑↓')} ${dim(t('select', 'выбрать'))}`,
      `${key('Enter/O')} ${dim(t('output', 'вывод'))}`,
      `${key('S')} ${dim(t('stop', 'остановить'))}`,
      `${key('R')} ${dim(t('refresh', 'обновить'))}`,
      `${key('Tab')} ${dim(t('filter', 'фильтр'))}`,
      `${key('Q/Esc')} ${dim(t('cancel', 'отмена'))} `,
    ];
  ...
  private renderListFrame(width: number, height: number): string[] {
    const title = t(
      `Tasks [${this.props.filter}]`,
      `Задачи [${this.props.filter === 'active' ? 'активные' : 'все'}]`,
    );
    ...
    if (this.sortedVisible.length === 0) {
      const empty =
        this.props.filter === 'active'
          ? t('No active tasks. Tab = show all.', 'Нет активных задач. Tab — показать все.')
          : t('No background tasks in this session.', 'В этом сеансе нет фоновых задач.');
  ...
  private renderDetailFrame(width: number, height: number): string[] {
    ...
    const task = this.sortedVisible[this.selectedIndex];
    if (task === undefined) {
      const empty = currentTheme.fg('textMuted', t('Select a task from the list.', 'Выберите задачу из списка.'));
      const lines: string[] = [empty];
      while (lines.length < innerHeight) lines.push('');
      return this.renderFrame(t('Detail', 'Подробности'), lines, width, height);
    }
    ...
    const lines: string[] = [
      `${label(t('Task ID:', 'ID задачи:'))}${value(task.taskId)}`,
      `${label(t('Status:', 'Статус:'))}${currentTheme.fg(statusColor(task.status), STATUS_LABEL[task.status])}`,
      `${label(t('Description:', 'Описание:'))}${value(singleLine(task.description) || '—')}`,
    ];
    if (task.kind === 'process' && task.command && task.command !== task.description) {
      lines.push(`${label(t('Command:', 'Команда:'))}${value(singleLine(task.command))}`);
    }
    if (task.kind === 'agent' && task.agentId !== undefined) {
      lines.push(`${label(t('Agent ID:', 'ID агента:'))}${value(task.agentId)}`);
    }
    if (task.kind === 'agent' && task.subagentType !== undefined) {
      lines.push(`${label(t('Agent type:', 'Тип агента:'))}${value(task.subagentType)}`);
    }
    if (task.kind === 'question') {
      lines.push(`${label(t('Questions:', 'Вопросы:'))}${currentTheme.fg('textMuted', String(task.questionCount))}`);
      if (task.toolCallId !== undefined) {
        lines.push(`${label(t('Tool call:', 'Вызов инструм.:'))}${currentTheme.fg('textMuted', task.toolCallId)}`);
      }
    }
    const timing =
      task.status === 'running'
        ? t(`running ${formatRelativeTime(task.startedAt)}`, `выполняется ${formatRelativeTime(task.startedAt)}`)
        : task.endedAt !== null && task.endedAt !== undefined
          ? t(`finished ${formatRelativeTime(task.endedAt)}`, `завершено ${formatRelativeTime(task.endedAt)}`)
          : '';
    if (timing.length > 0) lines.push(`${label(t('Time:', 'Время:'))}${currentTheme.fg('textMuted', timing)}`);
    if (task.kind === 'process' && task.pid > 0) {
      lines.push(`${label(t('PID:', 'PID:'))}${currentTheme.fg('textMuted', String(task.pid))}`);
    }
    if (task.kind === 'process' && task.exitCode !== null) {
      lines.push(`${label(t('Exit code:', 'Код выхода:'))}${currentTheme.fg('textMuted', String(task.exitCode))}`);
    }
    if (task.stopReason !== undefined && task.stopReason.length > 0) {
      lines.push(`${label(t('Reason:', 'Причина:'))}${currentTheme.fg('textMuted', task.stopReason)}`);
    }
    while (lines.length < innerHeight) lines.push('');
    return this.renderFrame(t('Detail', 'Подробности'), lines, width, height);
  }
  ...
  private renderPreviewFrame(width: number, height: number): string[] {
    ...
    const task = this.sortedVisible[this.selectedIndex];
    if (task === undefined) {
      const lines: string[] = [currentTheme.fg('textMuted', t('No task selected.', 'Задача не выбрана.'))];
      while (lines.length < innerHeight) lines.push('');
      return this.renderFrame(t('Preview Output', 'Предпросмотр вывода'), lines, width, height);
    }

    let body: string;
    if (this.props.tailLoading) body = t('[loading…]', '[загрузка…]');
    else if (this.props.tailOutput === undefined || this.props.tailOutput.length === 0)
      body = t('[no output captured]', '[вывод не записан]');
    ...
    return this.renderFrame(t('Preview Output', 'Предпросмотр вывода'), styled, width, height);
  }
  ...
  private renderTooSmall(width: number, rows: number): string[] {
    ...
    const msg = currentTheme.fg(
      ...
      t(
        ...
        `Терминал слишком мал (нужно ≥ ${String(MIN_WIDTH)} × ${String(MIN_HEIGHT)})`,
      ),
    );
