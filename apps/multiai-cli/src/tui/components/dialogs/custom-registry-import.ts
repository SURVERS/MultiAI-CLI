...
const title = (): string => t('Import custom provider registry', 'Импорт пользовательского реестра провайдеров');
const subtitleDefault = (): string => t('Paste an api.json URL and its Bearer token.', 'Вставьте URL файла api.json и его Bearer-токен.');
const subtitleUrlEmpty = (): string => t('Registry URL cannot be empty.', 'URL реестра не может быть пустым.');
const subtitleTokenEmpty = (): string => t('Bearer token cannot be empty.', 'Bearer-токен не может быть пустым.');
const footerNotLast = (): string => t('Tab / ↑↓ to switch  ·  Enter for next field  ·  Esc to cancel', 'Tab / ↑↓ — сменить поле  ·  Enter — следующее поле  ·  Esc — отмена');
const footerLast = (): string => t('Tab / ↑↓ to switch  ·  Enter to submit  ·  Esc to cancel', 'Tab / ↑↓ — сменить поле  ·  Enter — отправить  ·  Esc — отмена');
...
export class CustomRegistryImportDialogComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const footerStyled = currentTheme.fg(
      'textDim',
      this.activeField === 'url' ? footerNotLast() : footerLast(),
    );

    const urlLabelText = t('Registry URL', 'URL реестра');
    const tokenLabelText = t('Bearer token', 'Bearer-токен');
