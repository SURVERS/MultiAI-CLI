...
const defaultOtherLabel = (): string => t('Other', 'Другое');
const notAnsweredLabel = (): string => t('Not answered', 'Нет ответа');
const reviewTitle = (): string => t('Review your answer before submit', 'Проверьте ответы перед отправкой');
const submitPrompt = (): string => t('Ready to submit your answers?', 'Готовы отправить ответы?');
const unansweredWarning = (): string => t('Some questions are still unanswered.', 'На некоторые вопросы ещё нет ответа.');
const submitActions = (): readonly string[] => [t('Submit', 'Отправить'), t('Cancel', 'Отмена')];
...
export class QuestionDialogComponent extends Container implements Focusable {
  ...
  private renderQuestionTab(width: number): string[] {
    ...
    const lines: string[] = [accent('─'.repeat(renderWidth)), currentTheme.boldFg('primary', t(' question', ' вопрос')), ''];
    this.pushTabs(lines);
    lines.push('');

    appendWrapped(lines, ' ? ', '   ', question.question, renderWidth, accent);
    if (this.isEditingOther()) {
      lines.push(dim(t('   Type your answer, then press Enter to save.', '   Введите ответ и нажмите Enter для сохранения.')));
    }
    ...
    if (question.body !== undefined && question.body.trim().length > 0) {
      ...
      if (bodyLines.length > visibleBodyLines.length) {
        lines.push(dim(t(`   ... ${String(bodyLines.length - visibleBodyLines.length)} more lines`, `   ... ещё строк: ${String(bodyLines.length - visibleBodyLines.length)}`)));
      }
    }
    ...
    if (visibleEnd < options.length || visibleStart > 0) {
      lines.push(
        dim(
          t(`   showing ${String(visibleStart + 1)}-${String(visibleEnd)} of ${String(options.length)}`, `   показаны ${String(visibleStart + 1)}-${String(visibleEnd)} из ${String(options.length)}`),
        ),
      );
    }
  ...
  private renderSubmitTab(width: number): string[] {
    ...
    this.pushTabs(lines);
    lines.push('');
    lines.push(currentTheme.boldFg('text', ` ${reviewTitle()}`));
    const reviewWarning =
      this.reviewMessage ?? (this.hasUnansweredQuestions() ? unansweredWarning() : undefined);
    ...
    for (let i = 0; i < this.request.data.questions.length; i++) {
      ...
      if (answer !== undefined && answer.length > 0) {
        ...
        lines.push(`  ${dim('→')}  ${dim(notAnsweredLabel())}`);
      }
    }

    lines.push('');
    lines.push(text(` ${submitPrompt()}`));
    lines.push('');
  ...
  private pushTabs(lines: string[]): void {
    ...
    const submitLabel = t('Submit', 'Отправить');
  ...
  private buildQuestionHint(dim: (s: string) => string, questionIdx: number): string {
    if (this.isEditingOther()) {
      const parts: string[] = [
        t('type answer', 'введите ответ'),
        t('↵ save', '↵ сохранить'),
        ...(this.totalTabs() > 1 ? [t('tab switch', 'tab переключить')] : []),
        t('esc cancel', 'esc отменить'),
      ];
    ...
    const question = this.request.data.questions[questionIdx];
    if (question === undefined) return dim(`  ${t('esc cancel', 'esc отменить')}`);

    const parts: string[] = [
      t('↑↓ select', '↑↓ выбрать'),
      `${numberHint} / ↵ ${question.multi_select ? t('toggle', 'переключить') : t('choose', 'выбрать')}`,
    ];
    if (this.totalTabs() > 1) parts.push(t('←/→/tab switch', '←/→/tab переключить'));
    parts.push(t('esc cancel', 'esc отменить'));
    return dim(`  ${parts.join('  ')}`);
  }

  private buildSubmitHint(dim: (s: string) => string): string {
    const parts: string[] = [t('↑↓ select', '↑↓ выбрать'), t('1/2 choose', '1/2 выбрать'), t('↵ confirm', '↵ подтвердить')];
    if (this.totalTabs() > 1) parts.push(t('←/→/tab switch', '←/→/tab переключить'));
    parts.push(t('esc cancel', 'esc отменить'));
    return dim(`  ${parts.join('  ')}`);
  }
  ...
  private displayOptions(questionIdx: number): DisplayOption[] {
    ...
    return [
      ...
      {
        label: question.other_label?.length ? question.other_label : defaultOtherLabel(),
