...
export class GoalQueueManagerComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const hint = this.movingGoalId === undefined
      ? t(
          ...
          '↑↓ — навигация · Пробел — выбрать · E — изменить · D — удалить · Esc — отмена',
        )
      : t(
          '↑↓ reorder · Space done · E edit · D delete · Esc cancel',
          '↑↓ — переместить · Пробел — готово · E — изменить · D — удалить · Esc — отмена',
        );
    const lines: string[] = [
      currentTheme.fg('primary', '─'.repeat(width)),
      currentTheme.boldFg('primary', t(' Upcoming goals', ' Следующие цели')),
      currentTheme.fg('textMuted', ` ${hint}`),
      '',
    ];

    if (this.goals.length === 0) {
      lines.push(currentTheme.fg('textMuted', t('  No upcoming goals.', '  Следующих целей нет.')));
    } else {
      ...
      if (below > 0) {
        ...
        lines.push(
          currentTheme.fg(
            'textMuted',
            t(` ▼ ${String(below)} more`, ` ▼ ещё ${String(below)}`),
          ),
        );
      }
    }
  ...
  private renderGoal(goal: UpcomingGoal, index: number, selected: boolean, width: number): string {
    ...
    const stateLabel = moving ? t('  selected', '  выбрано') : '';
...
export class GoalQueueEditDialogComponent extends Container implements Focusable {
  ...
  override render(width: number): string[] {
    ...
    const title = truncateToWidth(
      currentTheme.boldFg('textStrong', t('Edit upcoming goal', 'Изменить следующую цель')),
      innerWidth,
      ELLIPSIS,
    );
    const subtitle = truncateToWidth(
      currentTheme.fg(
        ...
        this.error ?? t('Update the queued objective.', 'Обновите цель в очереди.'),
      ),
      innerWidth,
      ELLIPSIS,
    );
    ...
    const footer = truncateToWidth(
      currentTheme.fg(
        ...
        t(
          ...
          'Enter — отправить · Shift-Enter/Ctrl-J — новая строка · Esc — отмена',
        ),
      ),
      innerWidth,
      ELLIPSIS,
  ...
  private submit(value: string): void {
    ...
    if (objective.length === 0) {
      this.error = t('Goal objective cannot be empty.', 'Цель не может быть пустой.');
      return;
    }
    if (objective.length > MAX_GOAL_OBJECTIVE_LENGTH) {
      this.error = t(
        `Goal objective cannot exceed ${MAX_GOAL_OBJECTIVE_LENGTH} characters.`,
        `Цель не может быть длиннее ${MAX_GOAL_OBJECTIVE_LENGTH} символов.`,
      );
      return;
    }
