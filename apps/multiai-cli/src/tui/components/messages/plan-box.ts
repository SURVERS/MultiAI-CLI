...
import { Markdown, truncateToWidth, visibleWidth, type Component, type MarkdownTheme } from '@multiai/pi-tui';
...
const titlePrefix = (): string => t(' plan: ', ' план: ');
const TITLE_SUFFIX = ' ';
...
export class PlanBoxComponent implements Component {
  ...
  private buildTitle(horzLen: number): string {
    const fallback = t(' plan ', ' план ');
    const statusSuffix = this.buildStatusSuffix();
    const fallbackWithStatus = t(` plan${statusSuffix} `, ` план${statusSuffix} `);
    ...
    const linked = path.isAbsolute(planPath)
      ...
      : basename;
    const title = titlePrefix() + linked + statusSuffix + TITLE_SUFFIX;
