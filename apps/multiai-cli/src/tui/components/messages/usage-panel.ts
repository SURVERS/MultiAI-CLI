...
import type { SessionUsage, TokenUsage } from '@multiai/sdk';
...
function formatDuration(totalSeconds: number): string {
  ...
  if (remainder > 0 && parts.length === 0) parts.push(`${String(remainder)}s`);
...
function usageRowLabel(row: ManagedUsageRow): string {
  ...
  if (window !== undefined) {
    if (window.unit === 'week') return t('Weekly limit', 'Недельный лимит');
    return t(`${String(window.duration)}${window.unit[0] ?? ''} limit`, `Лимит: ${String(window.duration)} ${window.unit}`);
  }
  return row.name ?? t('Limit', 'Лимит');
}
...
function usageRowResetHint(row: ManagedUsageRow): string | undefined {
  ...
  if (diffSec <= 0) return t('reset', 'сброшено');
  return t(`resets in ${formatDuration(diffSec)}`, `сброс через ${formatDuration(diffSec)}`);
}
...
function buildSessionUsageSection(
  ...
  if (entries.length === 0) return [muted(`  ${t('No token usage recorded yet.', 'Расход токенов пока не зафиксирован.')}`)];
  ...
  for (const [model, row] of entries) {
    ...
    totalOutput += output;
    lines.push(
      `  ${muted(model)}  ${t('input', 'вход')} ${value(formatTokenCount(input))}  ${t('output', 'выход')} ${value(
        formatTokenCount(output),
      )}  ${t('total', 'итого')} ${value(formatTokenCount(input + output))}`,
    );
  }
  if (entries.length > 1) {
    lines.push(
      `  ${muted(t('total', 'итого'))}  ${t('input', 'вход')} ${value(formatTokenCount(totalInput))}  ${t('output', 'выход')} ${value(
        formatTokenCount(totalOutput),
      )}  ${t('total', 'итого')} ${value(formatTokenCount(totalInput + totalOutput))}`,
    );
  }
  return lines;
}
...
function buildManagedUsageSection(
  ...
  if (error !== undefined) return [accent(t('Plan usage', 'Использование тарифа')), errorStyle(`  ${error}`)];
  if (usage === undefined) return [];
  const { summary, limits } = usage;
  if (summary === null && limits.length === 0) {
    return [accent(t('Plan usage', 'Использование тарифа')), muted(`  ${t('No usage data available.', 'Данные об использовании недоступны.')}`)];
  }
  ...
  const pctWidth = Math.max(...rows.map((r) => t(`${Math.round(usedRatio(r) * 100)}% used`, `использовано ${Math.round(usedRatio(r) * 100)}%`).length));

  const out: string[] = [accent(t('Plan usage', 'Использование тарифа'))];
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i]!;
    const ratioUsed = usedRatio(row);
    const bar = renderProgressBar(ratioUsed, 20);
    const pct = t(`${Math.round(ratioUsed * 100)}% used`, `использовано ${Math.round(ratioUsed * 100)}%`);
...
export function buildExtraUsageSection(
  ...
  if (hasMonthlyLimit) {
    ...
    rows.push({ label: t('Used this month', 'Использовано за месяц'), ...used });
    rows.push({ label: t('Monthly limit', 'Месячный лимит'), ...limit });
    rows.push({ label: t('Balance', 'Баланс'), ...balance });
  } else {
    rows.push({ label: t('Used this month', 'Использовано за месяц'), ...used });
    rows.push({ label: t('Monthly limit', 'Месячный лимит'), symbol: '', number: t('Unlimited', 'Без ограничений') });
    rows.push({ label: t('Balance', 'Баланс'), ...balance });
  }
  ...
  const lines: string[] = [accent(t('Extra Usage', 'Дополнительное использование'))];
...
export function buildUsageReportLines(options: UsageReportOptions): string[] {
  ...
  const lines: string[] = [
    accent(t('Session usage', 'Использование сессии')),
