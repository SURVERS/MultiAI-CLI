...
export class StepSummaryComponent implements Component {
  ...
  render(_width: number): string[] {
    const parts: string[] = [];
    if (this.thinking > 0) parts.push(t(`thinking ${this.thinking} times`, `размышлений: ${this.thinking}`));
    if (this.tool > 0) parts.push(t(`call ${this.tool} tools`, `вызвано инструментов: ${this.tool}`));
    if (this.message > 0) parts.push(t(`${this.message} messages`, `сообщений: ${this.message}`));
