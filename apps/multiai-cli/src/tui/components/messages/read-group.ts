...
export class ReadGroupComponent extends Container {
  ...
  private buildHeader(total: number, pending: number, failed: number, totalLines: number): string {
    ...
    if (pending > 0) {
      ...
      const label = currentTheme.boldFg('primary', t(`Reading ${String(total)} files…`, `Чтение файлов: ${String(total)}…`));
      return `${bullet}${label}`;
    }
    ...
    if (failed === total) {
      ...
      const label = currentTheme.boldFg('error', t(`Read ${String(total)} files`, `Прочитано файлов: ${String(total)}`));
      return `${bullet}${label}${currentTheme.fg('error', t(' · failed', ' · ошибка'))}`;
    }

    const bullet = currentTheme.fg('success', STATUS_BULLET);
    const label = currentTheme.boldFg('primary', t(`Read ${String(total)} files`, `Прочитано файлов: ${String(total)}`));
    const linesPart = dim(t(` · ${String(totalLines)} ${totalLines === 1 ? 'line' : 'lines'}`, ` · строк: ${String(totalLines)}`));
    const failPart = failed > 0 ? currentTheme.fg('error', t(` · ${String(failed)} failed`, ` · ошибок: ${String(failed)}`)) : '';
  ...
  private buildBodyLine(snap: ToolCallReadSnapshot, isLast: boolean): string {
    ...
    if (snap.phase === 'pending') {
      tail = dim(t(' · reading…', ' · чтение…'));
    } else if (snap.phase === 'failed') {
      tail = currentTheme.fg('error', t(' · failed', ' · ошибка'));
    } else {
      tail = dim(t(` · ${String(snap.lines)} ${snap.lines === 1 ? 'line' : 'lines'}`, ` · строк: ${String(snap.lines)}`));
    }
