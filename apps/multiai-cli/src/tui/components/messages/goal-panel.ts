...
import type { GoalSnapshot, GoalStatus } from '@multiai/sdk';
...
export class GoalSetMessageComponent implements Component {
  ...
  render(width: number): string[] {
    return renderLifecycleLine(t('Goal set', 'Цель задана'), width);
  }
}
...
export class UpcomingGoalAddedMessageComponent implements Component {
  ...
  render(width: number): string[] {
    return renderLifecycleLine(
      t('Upcoming goal added. It will start after the current goal is complete.', 'Следующая цель добавлена. Она запустится после завершения текущей цели.'),
      width,
    );
  }
}
...
export function goalPanelTitle(goal: GoalSnapshot): string {
  const status = ({ active: t('active', 'активна'), complete: t('complete', 'завершена'), blocked: t('blocked', 'заблокирована'), paused: t('paused', 'приостановлена') } as const)[goal.status];
  return ` ${t('Goal', 'Цель')} · ${status} `;
}
...
export function buildGoalReportLines(goal: GoalSnapshot, wrapWidth: number = WRAP_WIDTH): string[] {
  ...
  lines.push(row(t('Running', 'Выполняется'), value(formatGoalElapsed(goal.wallClockMs))));
  lines.push(row(t('Turns', 'Ходы'), value(`${goal.turnsUsed}`)));
  lines.push(row(t('Tokens', 'Токены'), value(formatTokenCount(goal.tokensUsed))));
  if (!isComplete) {
    const stop = formatStopRow(goal);
    lines.push(
      stop !== null
        ? row(t('Stop', 'Остановка'), value(stop))
        : muted(t('No stop condition — runs until evaluated complete.', 'Нет условия остановки — выполняется до завершения оценки.')),
    );
  }
  return lines;
}
...
function formatStopRow(goal: GoalSnapshot): string | null {
  ...
  if (budget.turnBudget !== null) {
    parts.push(t(`after ${budget.turnBudget} turns (${goal.turnsUsed}/${budget.turnBudget})`, `после ${budget.turnBudget} ходов (${goal.turnsUsed}/${budget.turnBudget})`));
  }
  if (budget.tokenBudget !== null) {
    parts.push(t(`at ${formatTokenCount(budget.tokenBudget)} tokens`, `при ${formatTokenCount(budget.tokenBudget)} токенах`));
  }
  if (budget.wallClockBudgetMs !== null) {
    parts.push(t(`after ${formatGoalElapsed(budget.wallClockBudgetMs)}`, `через ${formatGoalElapsed(budget.wallClockBudgetMs)}`));
  }
