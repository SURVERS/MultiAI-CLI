import { truncateToWidth, visibleWidth, type Component } from '@multiai/pi-tui';
...
const orchestratingLabel = (): string => t('Orchestrating...', 'Оркестрация...');
const promptingLabel = (): string => t('Prompting...', 'Подготовка запроса...');
const workingLabel = (): string => t('Working...', 'Выполняется...');
const completedLabel = (): string => t('Completed.', 'Завершено.');
const failedLabel = (): string => t('Failed.', 'Ошибка.');
const abortedLabel = (): string => t('Aborted.', 'Прервано.');
const cancelledLabel = (): string => t('Cancelled.', 'Отменено.');
const queuedLabel = (): string => t('Queued...', 'В очереди...');
const suspendedLabel = (): string => t('Rate limited...', 'Ограничение частоты...');
const resumedItemLabel = (): string => t('(resumed)', '(возобновлено)');
...
function phaseLabel(phase: AgentSwarmPhase): string {
  const labels: Record<AgentSwarmPhase, string> = {
    pending: queuedLabel(),
    queued: queuedLabel(),
...
export class AgentSwarmProgressComponent implements Component {
  ...
  private renderHeader(width: number, _summary: AgentSwarmSummary | undefined): string {
    ...
    const title = gradientText(t('Agent Swarm', 'Рой агентов'), this.colors.primary, this.colors.accent, AGENT_SWARM_TITLE_ACCENT_BIAS);
    const description =
  ...
  private renderOrchestratingStatusLine(width: number): string {
    if (this.itemsStarted) {
      return truncateToWidth(
        renderStatusLabel(orchestratingLabel(), this.colors.primary),
        width,
      );
    }
    ...
    const label = renderStatusLabel(
      promptTemplate.length > 0 ? promptingLabel() : orchestratingLabel(),
      this.colors.primary,
    );
  ...
  private cancelMember(member: AgentSwarmMember, nowMs: number): void {
    ...
    if (previousPhase === 'pending' || previousPhase === 'queued' || previousPhase === 'suspended') {
      member.cancelledLabelText = cancelledLabel();
      ...
      member.cancelledLabelText = abortedLabel();
...
function agentSwarmResumeItemsFromArgs(args: Record<string, unknown>): string[] {
  ...
  return Object.keys(resumeAgentIds).map(() => resumedItemLabel());
}
...
function renderCancelledCellLabel(
  ...
  const labelText = member.cancelledLabelText ?? abortedLabel();
...
function renderPendingCell(
  ...
  const prefix = `${id} `;
  const itemText = collapseWhitespace(member.itemText);
  const label = itemText.length > 0 ? itemText : queuedLabel();
  const labelWidth = Math.max(1, width - visibleWidth(prefix));
...
function renderQueuedCell(
  ...
  const prefix = `${id} `;
  const labelWidth = Math.max(1, width - visibleWidth(prefix));
  return prefix + truncateWithColor(queuedLabel(), labelWidth, colors.textDim);
}
