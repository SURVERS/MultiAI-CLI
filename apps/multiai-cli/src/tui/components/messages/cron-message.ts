...
import { Spacer, Text, visibleWidth } from '@multiai/pi-tui';
...
export class CronMessageComponent implements Component {
  ...
  constructor(
    ...
    this.data = data;
    this.title = missed ? t('Missed scheduled reminders', 'Пропущенные запланированные напоминания') : t('Scheduled reminder fired', 'Сработало запланированное напоминание');
...
function cronDetail(data: CronTranscriptData): string | undefined {
  ...
  if (data.jobId !== undefined && data.jobId.length > 0) parts.push(`${t('job', 'задача')} ${data.jobId}`);
  if (data.recurring === false) parts.push(t('one-shot', 'однократно'));
  if (data.coalescedCount !== undefined && data.coalescedCount > 1) {
    parts.push(t(`${String(data.coalescedCount)} fires coalesced`, `Объединено срабатываний: ${String(data.coalescedCount)}`));
  }
  if (data.missedCount !== undefined) {
    parts.push(t(`${String(data.missedCount)} missed`, `Пропущено: ${String(data.missedCount)}`));
  }
  if (data.stale === true) parts.push(t('final delivery', 'последняя доставка'));
