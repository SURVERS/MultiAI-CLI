...
import { truncateToWidth, type Component } from '@multiai/pi-tui';
import type { GoalChange } from '@multiai/sdk';
  ...
  if (change.kind === 'lifecycle') {
    switch (change.status) {
      ...
      case 'blocked':
        ...
        return { headline: t('Goal blocked', 'Цель заблокирована'), accentToken: 'warning' };
      default:
        return null;
    }
  }
...
function pausedHeadline(reason: string | undefined, actor: GoalMarkerActor | undefined): string {
  if (reason === 'Paused after interruption') return t("Goal paused due to user's interruption", 'Цель приостановлена из-за прерывания пользователем');
  if (actor === 'user') return t('Goal paused by the user.', 'Цель приостановлена пользователем.');
  if (reason?.startsWith('Paused ') === true) return t(`Goal ${lowercaseFirst(reason)}`, `Цель приостановлена: ${reason}`);
  if (reason !== undefined && reason.length > 0) return t(`Goal paused: ${reason}`, `Цель приостановлена: ${reason}`);
  if (actor === 'model') return t('Goal paused by the agent.', 'Цель приостановлена агентом.');
  return t('Goal paused', 'Цель приостановлена');
}

function resumedHeadline(actor: GoalMarkerActor | undefined): string {
  if (actor === 'user') return t('Goal resumed by the user.', 'Цель возобновлена пользователем.');
  if (actor === 'model') return t('Goal resumed by the agent.', 'Цель возобновлена агентом.');
