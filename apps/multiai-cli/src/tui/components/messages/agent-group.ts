 ...
 * - Mounting: `MultiAITUI` attaches the group to the transcript at the
...
import type { TUI } from '@multiai/pi-tui';
import { Container, Spacer, Text } from '@multiai/pi-tui';
...
const detachHintText = (): string => t('Press Ctrl+B to run in background', 'Нажмите Ctrl+B, чтобы запустить в фоне');
...
export class AgentGroupComponent extends Container {
  ...
  private flushRender(): void {
    ...
    if (this.shouldShowDetachHint(snapshots)) {
      this.bodyContainer.addChild(new Text(currentTheme.dim(detachHintText()), 2, 0));
    }
  ...
  private buildHeader(snapshots: readonly ToolCallSubagentSnapshot[]): string {
    ...
    if (allDone) {
      ...
      const headerLabel =
        types.size === 1
          ? t(`${String(total)} ${[...types][0]} agents finished`, `Агенты ${[...types][0]} завершены: ${String(total)}`)
          : t(`${String(total)} agents finished`, `Агентов завершено: ${String(total)}`);
    ...
    const headerText = parts.length > 0
      ? t(`Running ${String(total)} agents (${parts.join(', ')})`, `Запущено агентов: ${String(total)} (${parts.join(', ')})`)
      : t(`Running ${String(total)} agents`, `Запущено агентов: ${String(total)}`);
  ...
  private appendLines(snap: ToolCallSubagentSnapshot, isLast: boolean): void {
    ...
    const agentType = snap.agentName ?? t('agent', 'агент');
    const desc = snap.toolCallDescription || t('(no description)', '(без описания)');
    ...
    if (snap.phase === 'failed') {
      ...
      const errLine = (snap.errorText ?? t('Failed', 'Ошибка')).split('\n').at(0) ?? t('Failed', 'Ошибка');
      const errStr = currentTheme.fg('error', t(`Error: ${errLine}`, `Ошибка: ${errLine}`));
...
function formatBreakdownParts(counts: PhaseCounts): string[] {
  const parts: string[] = [];
  if (counts.done > 0) parts.push(t(`${String(counts.done)} done`, `завершено: ${String(counts.done)}`));
  if (counts.failed > 0) parts.push(t(`${String(counts.failed)} failed`, `ошибок: ${String(counts.failed)}`));
  if (counts.backgrounded > 0) parts.push(t(`${String(counts.backgrounded)} backgrounded`, `в фоне: ${String(counts.backgrounded)}`));
  if (counts.running > 0) parts.push(t(`${String(counts.running)} running`, `работают: ${String(counts.running)}`));
  if (counts.waiting > 0) parts.push(t(`${String(counts.waiting)} waiting`, `ожидают: ${String(counts.waiting)}`));
  if (counts.starting > 0) parts.push(t(`${String(counts.starting)} starting`, `запускаются: ${String(counts.starting)}`));
  return parts;
}

function formatStats(snap: ToolCallSubagentSnapshot): string {
  const parts = [t(`${String(snap.toolCount)} tool${snap.toolCount === 1 ? '' : 's'}`, `инструментов: ${String(snap.toolCount)}`)];
...
function formatLineTail(snap: ToolCallSubagentSnapshot): string {
  ...
  switch (snap.phase) {
    case 'done':
      return separator + currentTheme.fg('success', t('✓ Completed', '✓ Завершён'));
    case 'failed':
      return separator + currentTheme.fg('error', t('✗ Failed', '✗ Ошибка'));
    case 'backgrounded':
      return separator + currentTheme.dim(t('◐ backgrounded', '◐ в фоне'));
    case 'queued':
      return separator + currentTheme.fg('primary', t('Waiting', 'Ожидание'));
    case 'running':
      return separator + currentTheme.fg('primary', t('Running', 'Выполняется'));
    case 'spawning':
    case undefined:
      return separator + currentTheme.fg('primary', t('Starting', 'Запуск'));
  }
}
...
function fallbackActivityForPhase(phase: ToolCallSubagentSnapshot['phase']): string {
  switch (phase) {
    case 'queued':
      return t('Waiting to start…', 'Ожидание запуска…');
    case 'running':
      return t('Still working…', 'Всё ещё работает…');
    case 'spawning':
    case undefined:
      return t('Starting…', 'Запуск…');
    case 'done':
    case 'failed':
    case 'backgrounded':
      return '';
...
function formatHeaderTail(args: {
  ...
  const parts: string[] = [];
  if (args.toolCount > 0) parts.push(t(`${String(args.toolCount)} tool${args.toolCount === 1 ? '' : 's'}`, `инструментов: ${String(args.toolCount)}`));
...
function formatElapsed(seconds: number): string {
  if (seconds < 60) return t(`${String(seconds)}s`, `${String(seconds)}с`);
  const minutes = Math.floor(seconds / 60);
  const remainder = seconds % 60;
  return t(`${String(minutes)}m ${String(remainder)}s`, `${String(minutes)}м ${String(remainder)}с`);
}

function formatTokens(n: number): string {
  return t(`${formatTokenCount(n)} tok`, `${formatTokenCount(n)} ток.`);
}
