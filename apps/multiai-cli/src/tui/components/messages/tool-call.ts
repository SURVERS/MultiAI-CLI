...
import { Container, Spacer, Text, truncateToWidth, visibleWidth } from '@multiai/pi-tui';
import type { Component, TUI } from '@multiai/pi-tui';
...
import type { TokenUsage } from '@multiai/sdk';
...
const detachHintText = (): string =>
  t('Press Ctrl+B to run in background', 'Нажмите Ctrl+B, чтобы запустить в фоне');
...
function backgroundFailureMessage(
  ...
  switch (status) {
    case 'lost':
      return t('Background agent lost (session restarted before completion)', 'Фоновый агент потерян (сеанс перезапущен до завершения)');
    case 'killed':
      return t('Background agent killed', 'Фоновый агент остановлен');
    case 'timed_out':
      return t('Background agent timed out', 'Время ожидания фонового агента истекло');
    case 'failed':
      return t('Background agent failed', 'Сбой фонового агента');
    case 'completed':
    case undefined:
      return undefined;
  }
...
function formatSubagentContextTokens(contextTokens: number | undefined): string | undefined {
  ...
  return `${formatTokenCount(contextTokens)} ${t('tok', 'ток.')}`;
}
...
function formatSubagentTokens(usage: TokenUsage | undefined): string | undefined {
  ...
  return `${formatTokenCount(total)} ${t('tok', 'ток.')}`;
}
...
function formatElapsed(seconds: number): string {
  if (seconds < 60) return `${String(seconds)}${t('s', 'с')}`;
  const minutes = Math.floor(seconds / 60);
  const remainder = seconds % 60;
  return `${String(minutes)}${t('m', 'м')} ${String(remainder)}${t('s', 'с')}`;
}
...
function extractKeyArgument(
  ...
  if (toolName === 'Glob') {
    ...
    if (args['include_ignored'] === true) {
      summary += t(' · include ignored', ' · включая игнорируемые');
    }
...
function formatSubagentLabel(agentName: string | undefined): string {
  ...
  if (raw === undefined || raw.length === 0) return t('SubAgent', 'Субагент');
  const label = raw
    .split(/[-_\s]+/)
  ...
  return t(`${label} Agent`, `Агент ${label}`);
}
...
export class ToolCallComponent extends Container {
   ...
   * `args.plan` is empty. `MultiAITUI` calls `setPlanInfo` with
  ...
  // when MultiAITUI routes a `subagent.event` with this tool call
  ...
  appendLiveOutput(text: string): void {
    ...
    if (this.liveOutput.length > MAX_LIVE_OUTPUT_CHARS) {
      this.liveOutput = `${t('[...truncated]', '[...обрезано]')}\n${this.liveOutput.slice(
   ...
   * this: plan-file mode leaves `args.plan` empty, so `MultiAITUI` fetches
  ...
  // ── Subagent API (called by MultiAITUI event routing) ───────────────
  ...
  private buildDetachHintBlock(): void {
    if (!this.detachHintVisible) return;
    if (this.result !== undefined) return;
    this.addChild(new Text(currentTheme.dim(detachHintText()), 2, 0));
  }
  ...
  private buildHeader(): string {
    ...
    if (toolCall.name === 'ExitPlanMode') {
      const label = currentTheme.boldFg('primary', t('Current plan', 'Текущий план'));
      ...
      if (outcome.kind === 'approved') {
        const chipText =
          outcome.chosen !== undefined && outcome.chosen.length > 0
            ? t(`Approved: ${outcome.chosen}`, `Одобрено: ${outcome.chosen}`)
            : t('Approved', 'Одобрено');
      ...
      if (outcome.kind === 'auto_approved') {
        ...
        return `${label}${currentTheme.fg('warning', t(' · Auto-approved', ' · Одобрено автоматически'))}`;
      }
      return label;
    }
    ...
    if (toolCall.name === 'AskUserQuestion') {
      ...
      const label = isFinished
        ? isError
          ? t('Could not collect your input', 'Не удалось получить ваш ответ')
          : isBackgroundAsk
            ? t('Started background question', 'Фоновый вопрос запущен')
          : t('Collected your answers', 'Ответы получены')
        : isBackgroundAsk
          ? t('Starting background question', 'Запуск фонового вопроса')
          : t('Waiting for your input', 'Ожидание вашего ответа');
    ...
    if (toolCall.name === 'Bash') {
      ...
      if (isTruncated) {
        return `${bullet}${currentTheme.fg('error', t('Truncated', 'Обрезан'))} ${currentTheme.boldFg('primary', 'Bash')}`;
      }
      const label = isFinished ? t('Ran a command', 'Команда выполнена') : t('Running a command', 'Выполнение команды');
      const tone = isError ? 'error' : 'primary';
    ...
    const verb = isFinished ? t('Used', 'Использован') : isTruncated ? t('Truncated', 'Обрезан') : t('Using', 'Используется');
  ...
  private buildSubagentBlock(): void {
    ...
    const headerLabel =
      this.subagentAgentName !== undefined
        ? t(
          ...
          `субагент ${this.subagentAgentName} (${this.formatAgentId()})`,
        )
        : t(`subagent (${this.formatAgentId()})`, `субагент (${this.formatAgentId()})`);
    this.addChild(new Text(`  ${currentTheme.dim(`↳ ${headerLabel}`)}${phaseChip}`, 0, 0));

    if (this.hiddenSubCallCount > 0) {
      const count = String(this.hiddenSubCallCount);
      const hiddenLabel = this.hiddenSubCallCount === 1
        ? t(`${count} more tool call ...`, `ещё один вызов инструмента ...`)
        : t(`${count} more tool calls ...`, `ещё вызовов инструментов: ${count} ...`);
      this.addChild(
        new Text(
          currentTheme.italic(currentTheme.dim(`    ${hiddenLabel}`)),
          0,
          0,
        ),
      );
    ...
    for (const sub of this.finishedSubCalls) {
      ...
      this.addChild(new Text(`    ${mark} ${t('Used', 'Использован')} ${nameCol}${argCol}`, 0, 0));
    }
    ...
    for (const [id, call] of this.ongoingSubCalls) {
      ...
      const argCol = keyArg ? currentTheme.dim(` (${keyArg})`) : '';
      void id;
      this.addChild(new Text(`    ${currentTheme.dim('…')} ${t('Using', 'Используется')} ${nameCol}${argCol}`, 0, 0));
    }
  ...
  private formatPhaseChip(): string {
    ...
    switch (this.subagentPhase) {
      case 'queued':
        parts.push(t('○ queued', '○ в очереди'));
        break;
      case 'spawning':
        parts.push(t('↻ starting…', '↻ запускается…'));
        break;
      case 'running':
        parts.push(t('↻ running', '↻ выполняется'));
        break;
      case 'done': {
        parts.push(currentTheme.fg('success', t('✓ done', '✓ готово')));
        const toolCount = this.finishedSubCalls.length + this.hiddenSubCallCount;
        if (toolCount > 0) {
          parts.push(toolCount === 1
            ? t('1 tool', '1 инструмент')
            : t(`${String(toolCount)} tools`, `${String(toolCount)} инструментов`));
        }
        const tokens =
      ...
      case 'failed':
        parts.push(currentTheme.fg('error', t('✗ failed', '✗ ошибка')));
        break;
      case 'backgrounded':
        parts.push(t('◐ backgrounded', '◐ в фоне'));
        break;
    }
  ...
  private buildSingleSubagentHeader(): string {
    ...
    if (isDone) {
      return `${marker}${currentTheme.boldFg('success', labelText)} ${currentTheme.fg('success', `${t('Completed', 'Завершён')}${descriptionPlain}${statsText}`)}`;
    }
  ...
  private formatSingleSubagentStatus(phase: SubagentPhase | undefined): string {
    switch (phase) {
      case 'done':
        return currentTheme.fg('success', t('Completed', 'Завершён'));
      case 'failed':
        return currentTheme.fg('error', t('Failed', 'Ошибка'));
      case 'running':
        return currentTheme.fg('primary', t('Running', 'Выполняется'));
      case 'backgrounded':
        return t('Backgrounded', 'В фоне');
      case 'queued':
        return currentTheme.fg('primary', t('Queued', 'В очереди'));
      case 'spawning':
      case undefined:
        return currentTheme.fg('primary', t('Starting', 'Запускается'));
    }
  }
  ...
  private buildSingleSubagentSummaryLine(): string {
    ...
    const verb = current.phase === 'ongoing' ? t('Using', 'Используется') : t('Used', 'Использован');
      ...
      this.addChild(
        new Text(
          currentTheme.dim(t(
            ...
            'Аргументы вызова инструмента обрезаны из-за max_tokens — вызов не выполнен.',
          )),
          2,
          0,
        ),
          ...
          new Text(
            currentTheme.dim(
              t(
                ...
                `... (ещё строк: ${String(remaining)}, всего: ${String(allLines.length)}, ctrl+o — развернуть)`,
              ),
            ),
            2,
            0,
  ...
  private buildStreamingPreview(streamText: string): void {
    ...
    if (name === 'Edit') {
      ...
      const target = filePath.length > 0 ? t(` for ${filePath}`, ` для ${filePath}`) : '';
      const progress = t(
        `Preparing changes${target}... ${formatByteSize(bytes)} · ${formatElapsed(elapsedSeconds)} elapsed`,
        `Подготовка изменений${target}... ${formatByteSize(bytes)} · прошло ${formatElapsed(elapsedSeconds)}`,
      );
  ...
  private resolvePlanBoxStatus(): { label: string; colorHex: string } | undefined {
    ...
    return { label: t('Rejected', 'Отклонено'), colorHex: currentTheme.color('error') };
  }
    ...
    if (this.toolCall.name === 'ExitPlanMode' && isExitPlanModeOutcomeOutput(result.output)) {
      ...
      if (outcome.kind === 'rejected' && outcome.feedback !== undefined) {
        ...
        if (trimmed.length > 0) {
          ...
          this.addChild(new Text(labelTone(t('↪ Suggestion', '↪ Предложение')), 2, 0));
  ...
  private buildAgentSwarmResultSummary(result: ToolResultBlockData): void {
    ...
    if (summary.completed > 0) {
      segments.push(
        currentTheme.fg('success', t(
          `${SUCCESS_MARK.trimEnd()} ${String(summary.completed)} completed`,
          `${SUCCESS_MARK.trimEnd()} завершено: ${String(summary.completed)}`,
        )),
      );
    }
    if (summary.failed > 0) {
      segments.push(
        currentTheme.fg('error', t(
          `${FAILURE_MARK.trimEnd()} ${String(summary.failed)} failed`,
          `${FAILURE_MARK.trimEnd()} с ошибкой: ${String(summary.failed)}`,
        )),
      );
    }
    if (summary.aborted > 0) {
      segments.push(
        currentTheme.fg('warning', t(
          `${ABORTED_MARK} ${String(summary.aborted)} aborted`,
          `${ABORTED_MARK} прервано: ${String(summary.aborted)}`,
        )),
      );
    }

    if (segments.length > 0) {
      this.addChild(new Text(`${dim(t('Agent swarm: ', 'Рой агентов: '))}${segments.join(dim(' · '))}`, 2, 0));
      return;
    }
    ...
    const label = isAborted
      ? t(`${ABORTED_MARK} Aborted.`, `${ABORTED_MARK} Прервано.`)
      : result.is_error === true
        ? t(`${FAILURE_MARK.trimEnd()} Failed.`, `${FAILURE_MARK.trimEnd()} Ошибка.`)
        : t(`${SUCCESS_MARK.trimEnd()} Completed.`, `${SUCCESS_MARK.trimEnd()} Завершено.`);
    this.addChild(new Text(`${dim(t('Agent swarm: ', 'Рой агентов: '))}${currentTheme.fg(colorToken, label)}`, 2, 0));
  }
  ...
  private renderAskUserQuestionResult(output: string): boolean {
    ...
    if (!hasAnswers) {
      const noteText =
        typeof note === 'string' && note.length > 0
          ? note
          : t('User dismissed the question.', 'Пользователь закрыл вопрос.');
    ...
    for (const [question, answer] of Object.entries(answers as Record<string, unknown>)) {
      ...
      this.addChild(new Text(`  ${currentTheme.dim(t('Q', 'В'))}  ${question}`, 0, 0));
...
function computeLatestActivity(
  ...
  if (ongoing.size > 0) {
    ...
    if (lastOngoing !== undefined) {
      return formatActivityLine(t('Using', 'Используется'), lastOngoing.name, lastOngoing.args, workspaceDir);
    }
  }
  if (finished.length > 0) {
    const last = finished.at(-1);
    if (last !== undefined) {
      return formatActivityLine(t('Used', 'Использован'), last.name, last.args, workspaceDir);
    }
  }
  if (text.length > 0) {
    const tail = text
...
function formatTokens(n: number): string {
  return `${formatTokenCount(n)} ${t('tok', 'ток.')}`;
}
