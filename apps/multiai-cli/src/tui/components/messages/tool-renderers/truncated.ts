import { Text, truncateToWidth, type Component } from '@multiai/pi-tui';
...
export class TruncatedOutputComponent implements Component {
  ...
  render(width: number): string[] {
    ...
    if (this.tail) {
      ...
      return [
        this.renderHint(width, t(`... (${String(remaining)} earlier lines)`, `... (предыдущих строк: ${String(remaining)})`)),
        ...shown,
      ];
    }
    ...
    const hint = this.expandHint
      ? t(`... (${String(remaining)} more lines, ctrl+o to expand)`, `... (ещё строк: ${String(remaining)}, ctrl+o — развернуть)`)
      : t(`... (${String(remaining)} more lines)`, `... (ещё строк: ${String(remaining)})`);
