...
export function formatWriteChip(stats: WriteStats): string {
  return t(pluralize(stats.lines, 'line'), `${String(stats.lines)} строк`);
}
...
const readChip: ChipProvider = (_toolCall, result) => {
  const count = countNonEmptyLines(result.output);
  return t(pluralize(count, 'line'), `${String(count)} строк`);
};

const grepChip: ChipProvider = (_toolCall, result) => {
  const matches = countNonEmptyLines(result.output);
  if (matches === 0) return t('no matches', 'нет совпадений');
  return t(pluralize(matches, 'match', 'matches'), `${String(matches)} совпадений`);
};

const globChip: ChipProvider = (_toolCall, result) => {
  const files = countNonEmptyLines(result.output);
  if (files === 0) return t('no files', 'нет файлов');
  return t(pluralize(files, 'file'), `${String(files)} файлов`);
};
...
const webSearchChip: ChipProvider = (_toolCall, result) => {
  ...
  if (count === 0) return lines.length === 0 ? t('no results', 'нет результатов') : t('web result', 'веб-результат');
  return t(pluralize(count, 'result'), `${String(count)} результатов`);
};
