...
import type { MultiAIDeviceAuthorization } from '@multiai/oauth';
import type {
  ApprovalRequest,
  ApprovalResponse,
  BackgroundTaskInfo,
...
import { tipText } from './constant/tips';
import {
...
import { setTuiLanguage, t } from './i18n';
import {
  MAIN_AGENT_ID,
  PRODUCT_NAME,
  llmNotSetMessage,
  noActiveSessionMessage,
} from './constant/multiai-tui';
...
export interface MultiAITUIStartupInput {
...
function createInitialAppState(input: MultiAITUIStartupInput): AppState {
...
export class MultiAITUI {
  ...
  private skillCommands: readonly MultiAISlashCommand[] = [];
  readonly skillCommandMap = new Map<string, string>();
  private pluginCommands: readonly MultiAISlashCommand[] = [];
  ...
  track(event: string, properties?: Parameters<MultiAIHarness['track']>[1]): void {
    this.harness.track(event, properties);
  }

  constructor(harness: MultiAIHarness, startupInput: MultiAITUIStartupInput) {
    this.harness = harness;
    const tuiOptions: MultiAITUIOptions = {
    ...
    setTuiLanguage(this.state.appState.language ?? 'en');
  ...
  private getSlashCommands(): readonly MultiAISlashCommand[] {
  ...
  private async refreshProviderModelsInBackground(): Promise<void> {
    try {
      ...
      for (const c of result.changed) {
        ...
        this.showStatus(t(`${c.providerName} · ${deltas.join('/')} models.`, `${c.providerName} · моделей: ${deltas.join('/')}.`));
      }
      for (const f of result.failed) {
        this.showStatus(
          t(
            `Skipped refreshing ${f.provider}: ${f.reason}`,
            `Не удалось обновить ${f.provider}: ${f.reason}`,
          ),
          'warning',
        );
      }
  ...
  private async finishStartup(shouldReplayHistory: boolean): Promise<void> {
    ...
    if (resumeState?.warning !== undefined) {
      this.showStatus(t(`Warning: ${resumeState.warning}`, `Предупреждение: ${resumeState.warning}`), 'warning');
    }
  ...
  private async showSessionWarnings(session: Session): Promise<void> {
    try {
      ...
      for (const warning of warnings) {
        ...
        this.showStatus(t(`Warning: ${warning.message}`, `Предупреждение: ${warning.message}`), severity);
      }
    } catch {
  ...
  private async init(): Promise<boolean> {
    ...
    try {
      if (isResumeStartup) {
        ...
        if (startup.sessionFlag !== undefined) {
          ...
          if (target === undefined) {
            throw new Error(t(`Session "${startup.sessionFlag}" not found.`, `Сессия «${startup.sessionFlag}» не найдена.`));
          }
              ...
              `${currentTheme.fg(
                ...
                t(
                  `Session "${startup.sessionFlag}" was created under a different directory.\n` +
                    `  cd "${target.workDir}" && multiai -r ${startup.sessionFlag}`,
                  `Сессия «${startup.sessionFlag}» создана в другой папке.\n` +
                    `  cd "${target.workDir}" && multiai -r ${startup.sessionFlag}`,
                ),
              )}\n\n`,
            );
          ...
          if (target !== undefined) {
            ...
            this.startupNotice = combineStartupNotice(
              ...
              t(
                ...
                `В папке «${workDir}» нет сессий для продолжения; запускается новая сессия.`,
              ),
            );
          }
        }
    ...
    if (session === undefined) {
      throw new Error(t('Startup session was not initialized.', 'Сессия при запуске не была инициализирована.'));
    }
  ...
  handleUserInput(text: string): void {
    ...
    if (this.state.appState.isReplaying) {
      this.showError(t('Cannot send input while session history is replaying.', 'Нельзя отправлять сообщения во время воспроизведения истории сессии.'));
      return;
    }
  ...
  private runShellCommandFromInput(command: string): void {
    ...
    if (session === undefined) {
      this.showError(t('No active session for shell command.', 'Нет активной сессии для команды оболочки.'));
      return;
    }
    ...
    void session.runShellCommand(command, { commandId }).then(
      ...
      (error: unknown) => {
        ...
        this.showError(t(`Shell command failed: ${message}`, `Команда оболочки завершилась с ошибкой: ${message}`));
      },
    );
  }
  ...
  cancelRunningShellCommand(): void {
    ...
    for (const commandId of this.shellOutputStreams.keys()) {
      void session.cancelShellCommand(commandId).catch((error: unknown) => {
        this.showError(t(`Failed to cancel shell command: ${formatErrorMessage(error)}`, `Не удалось отменить команду оболочки: ${formatErrorMessage(error)}`));
      });
    }
  }
  ...
  sendNormalUserInput(text: string): void {
    ...
    try {
      ...
      this.showError(t(`Failed to prepare media attachment: ${formatErrorMessage(error)}`, `Не удалось подготовить медиафайл: ${formatErrorMessage(error)}`));
      return;
    }
  ...
  validateMediaCapabilities(extraction: {
    ...
    if (
      ...
      this.showError(t('Current model does not support image input.', 'Текущая модель не поддерживает изображения.'));
      return false;
    }
    if (
      ...
      this.showError(t('Current model does not support video input.', 'Текущая модель не поддерживает видео.'));
      return false;
    }
    return true;
  }
  ...
  private sendMessageInternal(session: Session, input: string, options?: SendMessageOptions): void {
    ...
    if (this.state.appState.goal?.status === 'active') {
      void session.steer(sdkInput).catch((error: unknown) => {
        ...
        this.failSessionRequest(t(`Failed to steer: ${message}`, `Не удалось скорректировать запрос: ${message}`));
      });
      return;
    }
    void session.prompt(sdkInput).catch((error: unknown) => {
      const message = formatErrorMessage(error);
      this.failSessionRequest(t(`Failed to send: ${message}`, `Не удалось отправить: ${message}`));
    });
  }
  ...
  sendSkillActivation(session: Session, skillName: string, skillArgs: string): void {
    ...
    void session.activateSkill(skillName, rewrite.text).catch((error: unknown) => {
      const message = formatErrorMessage(error);
      this.failSessionRequest(t(`Skill "${skillName}" failed: ${message}`, `Навык «${skillName}» завершился с ошибкой: ${message}`));
    });
  }
  ...
  activatePluginCommand(
    ...
    void session
      ...
      .catch((error: unknown) => {
        const message = formatErrorMessage(error);
        this.failSessionRequest(t(`Command "${pluginId}:${commandName}" failed: ${message}`, `Команда «${pluginId}:${commandName}» завершилась с ошибкой: ${message}`));
      });
  }
  ...
  steerMessage(session: Session, input: readonly SteerInputItem[]): void {
    ...
    void session.steer(combineSteerInput(input)).catch((error: unknown) => {
      const message = formatErrorMessage(error);
      this.showError(t(`Failed to steer: ${message}`, `Не удалось скорректировать запрос: ${message}`));
    });
  }
  ...
  requireSession(): Session {
    if (this.session === undefined) {
      throw new Error(noActiveSessionMessage());
    }
    return this.session;
  }
  ...
  private async createSessionFromCurrentState(): Promise<Session> {
    ...
    if (model.length === 0) {
      throw new Error(llmNotSetMessage());
    }
  ...
  private async showResumeOtherWorkDirHint(session: SessionRow): Promise<void> {
    ...
    const command = `cd ${quoteShellArg(session.work_dir)} && multiai --resume ${quoteShellArg(session.id)}`;
    const message = t(
      `Current session is in a different working directory.\n  To resume, run: ${command}`,
      `Текущая сессия находится в другой рабочей папке.\n  Для продолжения выполните: ${command}`,
    );
    try {
      await copyTextToClipboard(command);
      this.showStatus(t(`${message}\n  Command copied to clipboard`, `${message}\n  Команда скопирована в буфер обмена`), 'warning');
    } catch {
      this.showStatus(t(`${message}\n  Failed to copy command to clipboard`, `${message}\n  Не удалось скопировать команду в буфер обмена`), 'warning');
    }
  }

  private async resumeSession(targetSessionId: string): Promise<boolean> {
    if (targetSessionId === this.state.appState.sessionId) {
      this.showStatus(t('Already on this session.', 'Эта сессия уже открыта.'));
      return true;
    }
    if (this.state.appState.streamingPhase !== 'idle') {
      this.showError(t('Cannot switch sessions while streaming — press Esc or Ctrl-C first.', 'Нельзя сменить сессию во время генерации — сначала нажмите Esc или Ctrl-C.'));
      return false;
    }
    if (this.state.appState.isReplaying) {
      this.showError(t('Cannot switch sessions while history is replaying.', 'Нельзя сменить сессию во время воспроизведения истории.'));
      return false;
    }
    ...
    try {
      ...
      this.showError(t(`Failed to resume session ${targetSessionId}: ${msg}`, `Не удалось продолжить сессию ${targetSessionId}: ${msg}`));
      return false;
    }

    await this.switchToSession(session, t(`Resumed session (${session.id}).`, `Сессия продолжена (${session.id}).`));
    return true;
  }
  ...
  async switchToSession(session: Session, statusMessage: string): Promise<void> {
    ...
    try {
      ...
      const msg = formatErrorMessage(error);
      this.showError(t(`Failed to replay session history: ${msg}`, `Не удалось воспроизвести историю сессии: ${msg}`));
    } finally {
      this.sessionEventHandler.startSubscription();
    }
  ...
  async createNewSession(): Promise<void> {
    if (this.state.appState.isReplaying) {
      this.showError(t('Cannot start a new session while history is replaying.', 'Нельзя начать новую сессию во время воспроизведения истории.'));
      return;
    }
    ...
    try {
      ...
      const msg = formatErrorMessage(error);
      this.showError(t(`Failed to start a new session: ${msg}`, `Не удалось начать новую сессию: ${msg}`));
      return;
    }
    ...
    try {
      ...
      this.sessionEventHandler.startSubscription();
      const msg = formatErrorMessage(error);
      this.showError(t(`Post-create setup failed: ${msg}`, `Не удалось настроить созданную сессию: ${msg}`));
      return;
    }
    try {
      await this.refreshSkillCommands(this.session);
    ...
    this.sessionEventHandler.startSubscription();
    this.clearTranscriptAndRedraw();
    this.showStatus(t(`Started a new session (${session.id}).`, `Начата новая сессия (${session.id}).`));
    void this.showSessionWarnings(session);
  ...
  private appendApprovalTranscriptEntry(
    ...
    switch (response.decision) {
      case 'approved':
        parts.push(response.scope === 'session' ? t('Approved for session', 'Разрешено для сессии') : t('Approved', 'Разрешено'));
        break;
      case 'rejected':
        parts.push(t('Rejected', 'Отклонено'));
        break;
      case 'cancelled':
        parts.push(t('Cancelled', 'Отменено'));
        break;
    }
  ...
  showError(message: string): void {
    this.showStatus(t(`Error: ${message}`, `Ошибка: ${message}`), 'error');
  }
  ...
  showLoginAuthorizationPrompt(auth: MultiAIDeviceAuthorization): LoginProgressSpinnerHandle {
    openUrl(auth.verificationUriComplete);
    this.state.transcriptContainer.addChild(
      new DeviceCodeBoxComponent({
        title: t('Sign in to MultiAI', 'Вход в MultiAI'),
        url: auth.verificationUriComplete,
        code: auth.userCode,
        hint: t('Press Ctrl-C to cancel', 'Нажмите Ctrl-C для отмены'),
      }),
    );
    this.state.ui.requestRender();
    return this.showLoginProgressSpinner(t('Waiting for authorization…', 'Ожидание авторизации…'));
  }
  ...
  updateActivityPane(): void {
    ...
    if (effectiveMode === 'idle' || effectiveMode === 'session' || effectiveMode === 'hidden') {
      ...
      this.currentLoadingTip = {
        kind: tipKind,
        tip: tipText(pickRandomWorkingTip(previousTip)),
      };
    }
    ...
    switch (effectiveMode) {
      ...
      case 'composing': {
        const spinner = this.ensureActivitySpinner('braille', t('working...', 'работаю...'), (s) =>
  ...
  private async detachRunningShellCommand(): Promise<void> {
    ...
    if (next.done) {
      this.showDetachHint(t('No shell command running.', 'Нет запущенной команды оболочки.'));
      return;
    }
    const [commandId, stream] = next.value;
    if (stream.taskId === undefined) {
      this.showDetachHint(t('Command is still starting — try again.', 'Команда ещё запускается — попробуйте снова.'));
      return;
    }
    const session = this.session;
    if (session === undefined) return;
    try {
      ...
      if (info === undefined) {
        this.showDetachHint(t('Command already finished.', 'Команда уже завершена.'));
        return;
      }
    } catch (error) {
      this.showError(t(`Failed to move to background: ${formatErrorMessage(error)}`, `Не удалось переместить в фон: ${formatErrorMessage(error)}`));
      return;
    }
    ...
    stream.entry.content = t('Moved to background.', 'Перемещено в фон.');
    this.shellOutputStreams.delete(commandId);
    ...
    this.showDetachHint(t('Moved to background. /tasks to view.', 'Перемещено в фон. Для просмотра: /tasks.'));
  }
  ...
  async detachCurrentForegroundTask(): Promise<void> {
    ...
    const session = this.session;
    if (session === undefined) {
      this.showError(noActiveSessionMessage());
      return;
    }
    ...
    try {
      ...
      this.showError(t(`Failed to list tasks: ${formatErrorMessage(error)}`, `Не удалось получить список задач: ${formatErrorMessage(error)}`));
      return;
    }

    const targets = pickForegroundTasks(tasks);
    if (targets.length === 0) {
      this.showDetachHint(t('No foreground task running.', 'Нет запущенной задачи на переднем плане.'));
      return;
    }
    ...
    for (const target of targets) {
      try {
        ...
        this.showError(t(`Failed to detach ${target.taskId}: ${formatErrorMessage(error)}`, `Не удалось отсоединить ${target.taskId}: ${formatErrorMessage(error)}`));
      }
    }
    ...
    if (detached === 0 && alreadyFinished > 0) {
      hint =
        alreadyFinished === 1
          ? t('Task already finished.', 'Задача уже завершена.')
          : t('Tasks already finished.', 'Задачи уже завершены.');
    } else if (detached === targets.length) {
      hint =
        detached === 1
          ? t('Moved 1 task to background.', '1 задача перемещена в фон.')
          : t(
              `Moved ${detached} tasks to background.`,
              `${detached} задач перемещено в фон.`,
            );
    } else {
      hint = t(
        `Moved ${detached} of ${targets.length} tasks to background.`,
        `Перемещено в фон задач: ${detached} из ${targets.length}.`,
      );
    }
    if (detached > 0) hint = t(`${hint} /tasks to view.`, `${hint} Для просмотра: /tasks.`);
      ...
      new SessionPickerComponent({
        ...
        onSelect: (session: SessionRow) => {
          void this.handleSessionPickerSelect(session, options.applyStartupModes === true).catch(
            (error) => {
              this.showError(t(`Failed to apply startup flags: ${formatErrorMessage(error)}`, `Не удалось применить параметры запуска: ${formatErrorMessage(error)}`));
            },
          );
        },
  ...
  private showApprovalPanel(payload: ApprovalPanelData): void {
    ...
    notifyTerminalOnce(this.state, `approval:${payload.id}`, {
      title: t('MultiAI CLI approval required', 'MultiAI CLI: требуется подтверждение'),
  ...
  private async enableApproveAll(response: ApprovalPanelResponse): Promise<void> {
    try {
      ...
      this.setAppState({ permissionMode: 'yolo' });
      this.showNotice(
        t('YOLO mode: ON', 'Режим YOLO: ВКЛ'),
        t(
          'Tool actions auto-approved; the agent may still ask you questions.',
          'Действия инструментов подтверждаются автоматически; агент всё ещё может задавать вопросы.',
        ),
      );
      this.approvalController.respond(adaptPanelResponse(response));
    } catch (error) {
      this.showError(t(`Failed to enable Approve all: ${formatErrorMessage(error)}`, `Не удалось включить «Разрешить всё»: ${formatErrorMessage(error)}`));
    }
  }
  ...
  private showQuestionDialog(payload: QuestionPanelData): void {
    ...
    notifyTerminalOnce(this.state, `question:${payload.id}`, {
      title: t('MultiAI CLI needs your answer', 'MultiAI CLI ждёт вашего ответа'),
