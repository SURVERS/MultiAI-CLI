...
const KEYRING_PACKAGE_PATTERN = /^@napi-rs\/keyring(?:-|$)/;
...
export function installNativeModuleHook(): void {
  ...
  moduleBuiltin._load = function loadWithNativeAssets(
    ...
    if (typeof request === 'string' && KEYRING_PACKAGE_PATTERN.test(request)) {
    ...
    if (
      ...
      const pkgRoot = getNativePackageRoot('@multiai/pi-tui');
      if (pkgRoot !== null) {
