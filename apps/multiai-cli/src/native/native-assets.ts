...
import { MULTIAI_BUILD_INFO } from '#/cli/build-info';
...
function currentTarget(): string {
  return MULTIAI_BUILD_INFO.buildTarget ?? `${process.platform}-${process.arch}`;
}
...
export function getNativeCacheBase(options: NativeAssetOptions = {}): string {
  ...
  const cacheDirEnv = optionalEnvValue(env, 'MULTIAI_CACHE_DIR');
  if (cacheDirEnv !== null) return cacheDirEnv;

  if (platform === 'darwin') return joinPosix(home, 'Library', 'Caches', 'multiai');
  if (platform === 'win32') {
    const localAppData = optionalEnvValue(env, 'LOCALAPPDATA');
    return localAppData !== null
      ? pathWin32.join(localAppData, 'multiai')
      : pathWin32.join(home, 'AppData', 'Local', 'multiai', 'Cache');
  }

  return joinPosix(optionalEnvValue(env, 'XDG_CACHE_HOME') ?? joinPosix(home, '.cache'), 'multiai');
}
...
export function getNativeAssetCacheRoot(
  ...
  const version = sanitizeSegment(options.version ?? MULTIAI_BUILD_INFO.version ?? 'dev');
...
function ensureEntryFile(cacheRoot: string): void {
  const entryPath = join(cacheRoot, 'node_modules', '.multiai-native-entry.cjs');
  ensureFile(
    entryPath,
...
export function cleanupStaleNativeCacheForCurrent(
  ...
  const version = MULTIAI_BUILD_INFO.version ?? 'dev';
