 ...
 * the process itself. It reports here whether a headless (`multiai -p`) run
...
export async function handleMainCommand(
  ...
  try {
    ...
    if (error instanceof OptionConflictError) {
      process.stderr.write(`${t('error', 'ошибка')}: ${error.message}\n`);
      process.exit(1);
    }
    throw error;
  }
  ...
  const program = createProgram(
    ...
    (opts) => {
      void handleMainCommand(opts, version)
        .then(async (outcome) => {
          ...
          // from the run can't wedge a completed `multiai -p` until an external
        ...
        .catch(async (error: unknown) => {
          ...
          // 0 and `process.exit(1)` never runs — headless (`multiai -p`) failures
          // would then exit 0 nondeterministically. Setting `process.exitCode`
          // up front makes that drain-exit report failure too.
          process.exitCode = 1;
          const operation = opts.prompt !== undefined ? t('run prompt', 'выполнить запрос') : t('start shell', 'запустить оболочку');
          ...
          process.stderr.write(`${t('See log:', 'См. журнал:')} ${resolveGlobalLogPath(resolveMultiAIHome())}\n`);
          process.exit(1);
        });
    },
    (entry, args) => {
    ...
    () => {
      void handleUpgradeCommand(version).catch(async (error: unknown) => {
        ...
        process.stderr.write(formatStartupError(error, { operation: t('upgrade', 'обновить') }));
        process.stderr.write(`${t('See log:', 'См. журнал:')} ${resolveGlobalLogPath(resolveMultiAIHome())}\n`);
        process.exit(1);
      });
    },
