...
async function runMultiAI(args) {
  try {
...
async function runMultiAIWithEnv(args, env) {
  try {
    const { stdout, stderr } = await execFileAsync(executablePath, args, {
      cwd: appRoot,
      env: { ...process.env, ...env },
...
const versionOutput = await runMultiAI(['--version']);
assertIncludes(versionOutput, expectedVersion, '--version');

const helpOutput = await runMultiAI(['--help']);
assertIncludes(helpOutput, 'Usage: multiai', '--help');

const exportHelpOutput = await runMultiAI(['export', '--help']);
assertIncludes(exportHelpOutput, 'Usage: multiai export', 'export --help');

const nativeAssetOutput = await runMultiAIWithEnv(['--version'], {
  MULTIAI_HOME: smokeHome,
  MULTIAI_NATIVE_ASSET_SMOKE: '1',
});
