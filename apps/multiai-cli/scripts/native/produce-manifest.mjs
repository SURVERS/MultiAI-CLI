 ...
 *
 * Input dir must contain files matching: multiai-cli-<target>.zip.sha256
...
// Accept package tags (`@multiai/cli@x.y.z`) and conventional version tags.
const version = tag.replace(/^@multiai\/cli@/, '').replace(/^v/, '');

const entries = await readdir(inputDir);
const sumFiles = entries.filter((f) => /^multiai-cli-[a-z0-9-]+\.zip\.sha256$/.test(f));

if (sumFiles.length === 0) {
  console.error(`No multiai-cli-<target>.zip.sha256 files found in ${inputDir}`);
  process.exit(1);
}
...
for (const sumFile of sumFiles.sort()) {
  ...
  // multiai-cli-darwin-arm64.zip → darwin-arm64
  const target = filename.replace(/^multiai-cli-/, '').replace(/\.zip$/, '');
