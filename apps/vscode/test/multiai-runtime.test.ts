 ...
 * Wiring: MultiAIRuntime and SessionRuntime are real; in-memory Session/MultiAIHarness fakes form the public SDK boundary.
 * Run: pnpm exec vitest run --config apps/vscode/vitest.config.ts test/multiai-runtime.test.ts
 */
...
import { MultiAIRuntime, type OpenSessionOptions } from "../src/runtime/multiai-runtime";
...
describe("Kimi runtime (owns shared SDK sessions for Webviews)", () => {
    ...
    expect(sdk.createInputs).toEqual([
      {
        ...
        thinking: "high",
        permission: "yolo",
        metadata: { vscode_multiai_approval: { yolo: true, afk: false } },
      },
    ]);
  ...
  it("uses the yolo setting as the initial value for an unmarked resumed session", async () => {
    ...
    expect(session.metadataUpdates).toEqual([
      { vscode_multiai_approval: { yolo: true, afk: false } },
    ]);
  });
  ...
  it("lets the global yolo setting override a persisted off flag on resume", async () => {
    ...
    const session = sdk.addSession(
      ...
      "/workspace",
      { permission: "manual" },
      { vscode_multiai_approval: { yolo: false, afk: false } },
    );
    ...
    expect(opened.sessionApprovalFlags).toEqual({ yolo: true, afk: false });
  });
  ...
  it("lets the global yolo setting disable a persisted session yolo flag on resume", async () => {
    ...
    expect(opened.sessionApprovalFlags).toEqual({ yolo: false, afk: false });
  });
  ...
  it("keeps the persisted afk flag while applying the global yolo setting on resume", async () => {
    ...
    const session = sdk.addSession(
      ...
      "/workspace",
      { permission: "manual" },
      { vscode_multiai_approval: { yolo: false, afk: true } },
    );

    const opened = await runtime.openSession(openOptions({ sessionId: "saved-1", yoloMode: true }));

    expect(session.setPermissions).toEqual(["auto"]);
    expect(opened.sessionApprovalFlags).toEqual({ yolo: true, afk: true });
  });
