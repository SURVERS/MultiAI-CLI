...
const EXTENSION_ID = "multiai.multiai-cli";
const EXPECTED_COMMANDS = [
  "multiai.clearAllState",
  "multiai.focusInput",
  "multiai.insertMention",
  "multiai.logout",
  "multiai.newConversation",
  "multiai.openInSideBar",
  "multiai.openInTab",
  "multiai.reset",
  "multiai.showLogs",
];
...
exports.run = async function run() {
  const isolatedHome = process.env.MULTIAI_VSCODE_SMOKE_OS_HOME;
  ...
  const sourceManifestPath = process.env.MULTIAI_VSCODE_SMOKE_SOURCE_MANIFEST
  ...
  assert.ok(process.env.MULTIAI_HOME, "MULTIAI_HOME must point at the isolated test home");
  ...
  const config = vscode.workspace.getConfiguration("multiai");
  ...
  await vscode.commands.executeCommand("multiai.openInTab");
  await waitFor(() => {
    return vscode.window.tabGroups.all.some((group) =>
      group.tabs.some((tab) =>
        tab.input instanceof vscode.TabInputWebview && isMultiAIPanelViewType(tab.input.viewType)));
  }, 5_000, () => `MultiAI Webview tab did not open; tabs=${describeTabs()}`);

  await vscode.commands.executeCommand("multiai.showLogs");
  await vscode.commands.executeCommand("multiai.reset");
  ...
  assert.ok(process.env.MULTIAI_VSCODE_SMOKE_REPORT, "Extension Host report path must be provided");
  await writeFile(
    process.env.MULTIAI_VSCODE_SMOKE_REPORT,
