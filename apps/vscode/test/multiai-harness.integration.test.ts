/**
 * Scenario: the VS Code host and another Node SDK client share one in-process MultiAI home.
 ...
 * Wiring: MultiAIRuntime, MultiAIHarness, core, storage, and HTTP provider adapter are real; only the remote provider is local.
 * Run: pnpm --filter multiai-vscode exec vitest run test/multiai-harness.integration.test.ts
 */
...
import { MultiAIRuntime } from "../src/runtime/multiai-runtime";
...
interface RuntimeRig {
  ...
  readonly runtime: MultiAIRuntime;
...
async function createRuntimeRig(extraAliases: readonly string[] = []): Promise<RuntimeRig> {
  const rootDir = await mkdtemp(join(tmpdir(), "multiai-vscode-harness-"));
  ...
  const runtime = new MultiAIRuntime({
    version,
    homeDir,
...
async function createPlainHarness(homeDir: string): Promise<MultiAIHarness> {
  const harness = createMultiAIHarness({
    homeDir,
    identity: { userAgentProduct: "multiai-cli", version: "test" },
  });
...
async function createMcpHandlerRig(): Promise<McpHandlerRig> {
  const homeDir = await mkdtemp(join(tmpdir(), "multiai-vscode-mcp-handler-"));
...
describe("VS Code Kimi harness integration (shares one in-process SDK home)", () => {
  ...
  it("keeps a slash-added directory after VS Code closes and resumes the session", async () => {
    ...
    expect(
      resumed.session.summary?.additionalDirs?.map((path) => path.replaceAll("\\", "/")),
    ).toContain(additionalDir.replaceAll("\\", "/"));
  });
  ...
  it("keeps /yolo and /afk independent when they are combined", async () => {
    ...
    expect(runtime.sessionApprovalFlags).toEqual({ yolo: true, afk: false });
    await expect(runtime.session.getStatus()).resolves.toMatchObject({ permission: "yolo" });

    await runSlash(runtime, "/afk");
    expect(runtime.sessionApprovalFlags).toEqual({ yolo: true, afk: true });
  ...
  it("applies the global yolo setting when a closed VS Code session reopens", async () => {
    ...
    expect(reopened.sessionApprovalFlags).toEqual({ yolo: false, afk: false });
    ...
    expect(yoloReopened.sessionApprovalFlags).toEqual({ yolo: true, afk: false });
  ...
  it("exports current context as Markdown under the workspace", async () => {
    ...
    expect(markdown).toContain("# MultiAI Session Export");
