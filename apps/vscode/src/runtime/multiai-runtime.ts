...
export interface MultiAIRuntimeOptions {
...
export class MultiAIRuntime {
  readonly harness: MultiAIHarness;

  private readonly broadcast: RuntimeBroadcast;
  private readonly captureBaseline: MultiAIRuntimeOptions["captureBaseline"];
  private readonly log: MultiAIRuntimeOptions["log"];
  private readonly sessions = new Map<string, SessionRuntime>();
  private readonly sessionByView = new Map<string, string>();
  private closed = false;

  constructor(options: MultiAIRuntimeOptions) {
    ...
    this.harness =
      ...
      createMultiAIHarness({
        ...
        identity: {
          userAgentProduct: "multiai-vscode",
  ...
  async openSession(options: OpenSessionOptions): Promise<SessionRuntime> {
    ...
    if (
      ...
      await applySessionSettings(current.session, options, current.sessionApprovalFlags);
    ...
    if (runtime !== undefined) {
      ...
      await applySessionSettings(runtime.session, options, runtime.sessionApprovalFlags);
      await this.detachView(options.webviewId);
    } else {
      const defaultApproval: SessionApprovalFlags = { yolo: options.yoloMode, afk: false };
      const session =
        requestedId === undefined
          ? await this.harness.createSession({
              ...
              permission: corePermissionForSessionApproval(defaultApproval),
              metadata: sessionApprovalMetadata(defaultApproval),
            })
          : await this.harness.resumeSession({ id: requestedId, includeSubagents: true });
      try {
        assertSessionWorkDir(session, options.workDir);
        const storedApproval = readSessionApprovalFlags(session.summary?.metadata);
        ...
        if (storedApproval === undefined || flagsDiffer(storedApproval, approval)) {
          await session.updateMetadata(sessionApprovalMetadata(approval));
        }
  ...
  async attachResumedSession(
    ...
    if (runtime === undefined) {
      try {
        ...
        const permission = corePermissionForSessionApproval(approval);
  ...
  private wrapSession(session: Session, sessionApproval: SessionApprovalFlags): SessionRuntime {
  ...
  private ensureOpen(): void {
    if (this.closed) throw new Error("MultiAI runtime is closed.");
  }
}
...
async function applySessionSettings(
  ...
  sessionApproval: SessionApprovalFlags,
): Promise<void> {
  const status = await session.getStatus();
  ...
  const permission = corePermissionForSessionApproval(sessionApproval);
...
function flagsDiffer(a: SessionApprovalFlags, b: SessionApprovalFlags): boolean {
