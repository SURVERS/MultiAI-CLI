 ...
 *   current builds  `<MultiAI home>/server/instances/<serverId>.json`
 *   pre-registry builds  `<MultiAI home>/server/lock`
 * and persists the bearer token at `<MultiAI home>/server.token` (one token per
 ...
 * `~/.multiai/server.token` by hand — no new exposure beyond the local dev
 * session.
 */
...
/** home resolution per request: `MULTIAI_HOME` env, else `~/.multiai`. */
export function resolveMultiAIHomeDir(env: NodeJS.ProcessEnv = process.env): string {
  const fromEnv = env['MULTIAI_HOME'];
...
export interface DiscoverOptions {
  /** The dev-proxy target (`MULTIAI_SERVER_URL`); merged as a `proxy` entry. */
...
export async function discoverLocalServers(
  ...
  const home = options.homeDir ?? resolveMultiAIHomeDir();
...
export function serverDiscoveryPlugin(options: { proxyTarget: string }): Plugin {
  ...
  return {
    name: 'multiai-inspect-server-discovery',
