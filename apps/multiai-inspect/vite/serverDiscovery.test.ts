...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'multiai-inspect-discovery-'));
});
...
describe('resolveMultiAIHomeDir', () => {
  it('honors MULTIAI_HOME, else falls back to ~/.multiai', () => {
    expect(resolveMultiAIHomeDir({ MULTIAI_HOME: '/tmp/kh' })).toBe('/tmp/kh');
    expect(resolveMultiAIHomeDir({})).toBe(join(process.env['HOME'] ?? '', '.multiai'));
  });
});
