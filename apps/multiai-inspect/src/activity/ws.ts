 ...
 * `multiai.bearer.<token>` subprotocol (the only credential channel a
...
const WS_BEARER_PROTOCOL_PREFIX = 'multiai.bearer.';
...
export class GlobalEventsWs {
  ...
  private connect(): void {
    ...
    ws.addEventListener('open', () => {
      ...
      this.send({
        type: 'client_hello',
        id: `multiai-inspect-global-${Date.now().toString(36)}`,
        payload: { client_id: 'multiai-inspect', subscriptions: [] },
      });
