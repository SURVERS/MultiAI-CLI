# multiai-web Agent Guide

Package-local rules for `apps/multiai-web` (`@multiai/web`).

## What it is

The browser web UI for MultiAI CLI — a peer to the TUI in `apps/multiai-cli`. It talks to the local server over REST + WebSocket under `/api/v1`. Stack: Vue 3 + Vite 6 + TypeScript (strict) + vue-i18n v11. (Tailwind was removed; all styling is via design tokens in `src/style.css` + scoped component styles.) There is no client router and no Pinia; state lives in composables/refs and provide/inject.
...
- **Run `pnpm --filter @multiai/web check:style`** (`scripts/check-style.mjs`) — it enforces the §06 anti-pattern rules (no-gradient, no-glassmorphism except TopBar `frost`, no-emoji-icon except moon, no-hardcoded-hex/font, radius/z/weight from scale). Do not add new violations.
...
- `api/` — server client. `index.ts` exposes the `getMultiAIWebApi()` singleton; `config.ts` builds REST/WS URLs; `daemon/` holds the wire client (`http.ts`, `ws.ts`, `wire.ts`, `mappers.ts`, `agentEventProjector.ts`, `eventReducer.ts`).
...
- `composables/` — reusable state logic, `useX` naming (`useMultiAIWebClient`, `useIsDark`, `usePaneLayout`, …).
...
- Locale files: `src/i18n/locales/{en,ru}/<namespace>.ts`, each `export default { ... } as const`. New namespaces are registered in `src/i18n/locales/index.ts`.
- Reference with `const { t } = useI18n()` and `t('namespace.key')` (same form in templates).
- **Adding a key:** add it to **both** `en/<ns>.ts` and `ru/<ns>.ts`. **Adding a namespace:** create the file in both locales **and** register it in `locales/index.ts`.
- There is **no automated missing-key or en/ru parity check**. Keeping the two locales in sync is a manual responsibility — do not leave a key present in only one locale.

## Commands

All via `pnpm --filter @multiai/web …`:

- `dev` — Vite dev server (port `WEB_PORT`, default 5175; proxies `/api/v1` to `MULTIAI_SERVER_URL`, default `http://127.0.0.1:58627`).
...
Debugging against kap-server instances: start one from the repo root with `pnpm dev:server` (port 58627), optionally a second with `pnpm dev:v2` (port 58628 — instances share the home dir via the registry, so both can run at once). The dev server proxies `/api/v1` to the `default` preset; the Sidebar brand row carries a dev-only backend pill (engine generation `v1`/`v2` from `GET /api/v1/meta`'s `backend` field + endpoint) whose menu repoints the proxy at runtime — no Vite restart. Presets default to `http://127.0.0.1:58627` / `:58628`, overridable via `MULTIAI_BACKEND_DEFAULT_URL` / `MULTIAI_BACKEND_MULTI_URL`; the switcher endpoints (`GET/POST /__multiai-dev/backend`, dev-only, see `backendSwitcherPlugin` in `vite.config.ts`) drive the menu.

## Gotchas / hard rules

- **Do not depend on `@multiai/agent-core`** (mirrors the CLI/SDK rule). The web app is decoupled from core/protocol; wire types are re-implemented locally in `src/api/daemon/wire.ts`. Keep it that way.
- **Same-origin by default:** the browser only talks to its own origin; Vite proxies `/api/v1` for both HTTP and WS. Set `VITE_MULTIAI_SERVER_HTTP_URL` only when you intentionally want direct (CORS) mode.
- Vite-injected globals (`__MULTIAI_DEV_PROXY_TARGET__`, `__MULTIAI_DEV_BACKENDS__`, `__MULTIAI_WEB_VERSION__`, `__MULTIAI_WEB_COMMIT__`) are declared in `src/env.d.ts` and defined in `vite.config.ts`. Do not hand-edit `dist/`.
