 ...
 *   GET  /__multiai-dev/backend           → { current, presets }
 *   POST /__multiai-dev/backend { name }  → switch to presets[name]
...
export default defineConfig({
  plugins: [
    ...
    Icons({
      compiler: 'vue3',
      // Local MultiAI design system icons (24×24 outlined, fill="currentColor"),
      // copied from the design-system icon pack into src/icons/multiai/ and
      // imported as `~icons/multiai/<file-name>` (plus `?raw`), same as the ri
      // collection. Registered in src/lib/icons.ts only.
      customCollections: {
        multiai: FileSystemIconLoader(
          fileURLToPath(new URL('./src/icons/multiai', import.meta.url)),
        ),
      },
    }),
  ],
  ...
  define: {
    __MULTIAI_DEV_PROXY_TARGET__: JSON.stringify(serverTarget),
    // Named backend presets for the Sidebar switcher menu (dev only).
    __MULTIAI_DEV_BACKENDS__: JSON.stringify(backendPresets),
    __MULTIAI_WEB_VERSION__: JSON.stringify(pkg.version),
    // True only for the web bundle embedded in the MultiAI Desktop app (set by the
    ...
    __MULTIAI_WEB_DESKTOP__: JSON.stringify(process.env.MULTIAI_WEB_DESKTOP === '1'),
  },
  server: {
    port: webPort,
    strictPort: false,
