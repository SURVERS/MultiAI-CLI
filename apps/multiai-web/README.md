...
A browser client for MultiAI CLI — a peer to the TUI (`apps/multiai-cli`) that talks
...
WEB_PORT=5197 MULTIAI_SERVER_URL=http://192.168.97.91:58627 pnpm -C apps/multiai-web run dev
#   …or from the repo root:  pnpm dev:web   (uses the defaults below)

# checks
pnpm -C apps/multiai-web run typecheck     # vue-tsc --noEmit
pnpm -C apps/multiai-web run test          # vitest (pure logic only)
pnpm -C apps/multiai-web run build         # vite build
```
...
| `MULTIAI_SERVER_URL` | `http://127.0.0.1:58627`  | where `/api/v1` (and `/api/v1/ws`) is forwarded |
...
server (REST + WS)
  ...
  └─ src/composables/useMultiAIWebClient.ts   the ONLY place that imports api + state;
...
- **i18n** (`src/i18n/`): vue-i18n, en/ru, per-namespace flat camelCase keys.
...
1. **Develop** — `pnpm dev:web` (or `pnpm -C apps/multiai-web run dev`).
2. **Build** — `pnpm -C apps/multiai-web run build` produces `apps/multiai-web/dist`.
3. **Bundle into CLI** — `pnpm -C apps/multiai-cli run build` runs
   `scripts/copy-web-assets.mjs`, which copies `apps/multiai-web/dist` into
   `apps/multiai-cli/dist-web`.
4. **Publish** — the root `.github/workflows/release.yml` publishes
   `multiai-cli` to npm; `dist-web` is listed in the package `files`
   array, so the built web assets travel with the CLI package.
5. **Serve** — `multiai web` serves `dist-web` from the
   installed package.

The web UI does not display its own package version or build commit. It is
bundled into the CLI package and follows the published `multiai-cli`
release.

### Suggested improvements

- **Keep the current coupling for now.** Because MultiAI CLI is primarily a local
...
- **Add an independent web-deploy workflow only when needed.** If a public
  ...
  `.github/workflows/web-deploy.yml` that builds `apps/multiai-web` and uploads
...
- **Keep versioning owned by the CLI release.** `apps/multiai-web/package.json`
...
- **Ensure the web build is exercised in CI.** The root `build` script already
  builds every workspace, so `pnpm run build` in CI covers `apps/multiai-web`.
