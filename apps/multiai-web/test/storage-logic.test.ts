...
describe('draftStorageKey', () => {
  it('uses the session id when present', () => {
    expect(draftStorageKey('abc')).toBe('multiai-web.draft.abc');
  });

  it('falls back to __new__ when sid is empty/undefined', () => {
    expect(draftStorageKey(undefined)).toBe('multiai-web.draft.__new__');
    expect(draftStorageKey('')).toBe('multiai-web.draft.__new__');
  });
});

describe('STORAGE_KEYS', () => {
  it('keeps the legacy key strings unchanged', () => {
    expect(STORAGE_KEYS.theme).toBe('multiai-web.theme');
    expect(STORAGE_KEYS.activeWorkspace).toBe('multiai-active-workspace');
    expect(STORAGE_KEYS.notifyOnComplete).toBe('multiai-web.notify-on-complete');
    expect(STORAGE_KEYS.notifyOnQuestion).toBe('multiai-web.notify-on-question');
    expect(STORAGE_KEYS.soundOnComplete).toBe('multiai-web.sound-on-complete');
    expect(STORAGE_KEYS.locale).toBe('multiai-locale');
  });
});
