// apps/multiai-web/test/daemon-client.test.ts
// DaemonMultiAIWebApi public REST adapter: session export binary/error contracts,
...
// Run: pnpm --filter @multiai/web exec vitest run test/daemon-client.test.ts

import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';

import { DaemonMultiAIWebApi } from '../src/api/daemon/client';
import { DaemonApiError, DaemonNetworkError } from '../src/api/errors';
import { clearTrace, traceToJsonl } from '../src/debug/trace';
import type { AppEvent, MultiAIEventConnection, MultiAIEventMeta } from '../src/api/types';
...
function createApi(): DaemonMultiAIWebApi {
  return new DaemonMultiAIWebApi({
...
describe('DaemonMultiAIWebApi.exportSession', () => {
  beforeEach(() => {
...
describe('DaemonMultiAIWebApi.getSessionGoal', () => {
  beforeEach(() => {
    vi.stubGlobal('fetch', vi.fn());
  });
...
describe('DaemonMultiAIWebApi.connectEvents', () => {
  let connection: MultiAIEventConnection | undefined;
  ...
  it('delivers raw assistant stream coordinates with the projected delta', () => {
    ...
    const received: Array<{ event: AppEvent; meta: MultiAIEventMeta }> = [];
