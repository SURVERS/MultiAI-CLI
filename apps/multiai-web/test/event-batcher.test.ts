 ...
 * Run: pnpm --filter @multiai/web exec vitest run test/event-batcher.test.ts
 */

import { describe, expect, it, vi } from 'vitest';

import { createInitialState, reduceAppEvent, type MultiAIClientState } from '../src/api/daemon/eventReducer';
import type {
  AppEvent,
  AppMessage,
  AppSession,
...
vi.mock('../src/api', () => ({
  getMultiAIWebApi: () => clientApiMock,
}));
...
function assistantState(content: AppMessage['content'] = [{ type: 'text', text: '' }]): MultiAIClientState {
...
describe('useMultiAIWebClient (resync integration)', () => {
  it('flushes queued deltas around an authoritative snapshot before live streaming resumes', async () => {
    ...
    let handlers: MultiAIEventHandlers | undefined;
    ...
    let seedCalls = 0;
    const connection: MultiAIEventConnection = {
      subscribe: vi.fn(),
      unsubscribe: vi.fn(),
    ...
    const api: Partial<MultiAIWebApi> = {
      getAuth: vi.fn(async () => ({
        ready: true,
    ...
    try {
      const { useMultiAIWebClient } = await import('../src/composables/useMultiAIWebClient');
