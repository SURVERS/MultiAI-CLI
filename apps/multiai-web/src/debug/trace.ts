// apps/multiai-web/src/debug/trace.ts
...
// `localStorage["multiai-web.debug"]="1"`; key lifecycle metadata is always on.
...
export function downloadTraceLog(list: readonly TraceEntry[] = entries): void {
  ...
  try {
    ...
    anchor.href = url;
    anchor.download = `multiai-web-log-${new Date().toISOString().replaceAll(/[:.]/g, '-')}.jsonl`;
