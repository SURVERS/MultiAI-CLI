// apps/multiai-web/src/api/types.ts
// App-facing camelCase model + MultiAIWebApi interface.
...
export interface MultiAIEventHandlers {
  onEvent(event: AppEvent, meta: MultiAIEventMeta): void;
...
export interface MultiAIEventMeta {
  sessionId: string;
  seq: number;
  stream?: {
    turnId: number;
...
export interface MultiAIEventConnection {
...
// ---------------------------------------------------------------------------
// MultiAIWebApi — the app-facing interface
// ---------------------------------------------------------------------------
...
export interface MultiAIWebApi {
...
export interface AccountSnapshot {
  ...
  wallet: { total: number; classic: number; new: number; billingMode: string };
  subscription: {
    active: boolean;
    available: boolean;
    limits: Record<string, { enabled: boolean; remainingPercent: number; resetAt?: string }>;
  };
  keys: { id: number; name: string; maskedKey: string; status: string }[];
  connection: {
    id: string;
    clientId: string;
    clientName: string;
