...
export function readMultiAIApiConfig(): MultiAIApiConfig {
  return {
    serverHttpUrl: normalizeServerOrigin(import.meta.env.VITE_MULTIAI_SERVER_HTTP_URL),
...
//  - prod: `multiai web` serves this built SPA from the server itself, so the
//    server's origin already is the API origin.
// Set VITE_MULTIAI_SERVER_HTTP_URL to connect directly to an absolute server
 ...
 *  - VITE_MULTIAI_SERVER_HTTP_URL set → that absolute server origin (direct mode).
...
export function serverEndpointLabel(): string {
  const direct = import.meta.env.VITE_MULTIAI_SERVER_HTTP_URL;
  if (direct && direct.trim()) return shortOrigin(normalizeServerOrigin(direct));

  const proxy =
    typeof __MULTIAI_DEV_PROXY_TARGET__ !== 'undefined' ? __MULTIAI_DEV_PROXY_TARGET__ : '';
...
function webClientVersion(): string {
  return typeof __MULTIAI_WEB_VERSION__ === 'string' && __MULTIAI_WEB_VERSION__.trim()
    ? __MULTIAI_WEB_VERSION__
    : '0.0.0-dev';
}
