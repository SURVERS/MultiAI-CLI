// apps/multiai-web/src/api/devBackend.ts
...
//   POST /__multiai-dev/backend { name } → repoint the /api/v1 proxy
...
export function initialDevBackendState(): DevBackendState | null {
  ...
  const presets =
    typeof __MULTIAI_DEV_BACKENDS__ !== 'undefined' ? __MULTIAI_DEV_BACKENDS__ : null;
  if (!presets) return null;
  const current =
    typeof __MULTIAI_DEV_PROXY_TARGET__ !== 'undefined' && __MULTIAI_DEV_PROXY_TARGET__
      ? __MULTIAI_DEV_PROXY_TARGET__
      : presets.default;
...
export async function fetchDevBackendState(): Promise<DevBackendState | null> {
  if (!import.meta.env.DEV) return null;
  try {
    const res = await fetch('/__multiai-dev/backend');
