// apps/multiai-web/src/api/daemon/wire.ts
...
//     as already complete.
interface WireOAuthLoginStartCommon {
  flow_id: string;
  provider: string;
...
interface WireOAuthBrowserLoginStartPending extends WireOAuthLoginStartCommon {
  method: 'browser';
  status: 'pending';
...
interface WireOAuthDeviceLoginStartPending extends WireOAuthLoginStartCommon {
  method: 'device';
  status: 'pending';
...
export type WireOAuthLoginStartResult =
  | WireOAuthBrowserLoginStartPending
  | WireOAuthDeviceLoginStartPending
...
export interface WireAccountSnapshot {
  user: {
    sub: string;
    display_name?: string;
    avatar_url?: string;
  ...
  account: {
    wallet: { total: number; classic: number; new: number; billing_mode: string };
    subscription: {
      active: boolean;
      available: boolean;
      limits: Record<string, { enabled: boolean; remaining_percent: number; reset_at?: string }>;
    };
    generated_at: string;
  };
  keys: { id: number; name: string; key: string; status: string }[];
  connection: {
    id: string;
    client_id: string;
    client_name: string;
