// apps/multiai-web/src/api/daemon/client.ts
// DaemonMultiAIWebApi — implements MultiAIWebApi using the daemon REST + WS APIs.

import type { MultiAIApiConfig } from '../config';
...
import type {
  ...
  FsBrowseResult,
  FsEntry,
  MultiAIEventConnection,
  MultiAIEventHandlers,
  MultiAIWebApi,
...
export class DaemonMultiAIWebApi implements MultiAIWebApi {
  private readonly http: DaemonHttpClient;
  private readonly config: MultiAIApiConfig;

  constructor(config: MultiAIApiConfig) {
    this.config = config;
  ...
  async startOAuthLogin(input: {
    ...
    if (data.method === 'browser') {
      return {
        ...
        persistence: data.persistence,
        status: 'pending',
        authorizationUri: data.authorization_uri,
  ...
  async getAccount(): Promise<import('../types').AccountSnapshot> {
    const data = await this.http.get<WireAccountSnapshot>('/oauth/account');
    return {
      user: {
        sub: data.user.sub,
      ...
      wallet: {
        total: data.account.wallet.total,
        classic: data.account.wallet.classic,
        new: data.account.wallet.new,
        billingMode: data.account.wallet.billing_mode,
      },
      subscription: {
        active: data.account.subscription.active,
        available: data.account.subscription.available,
        limits: Object.fromEntries(
          Object.entries(data.account.subscription.limits).map(([key, value]) => [
            key,
            {
      ...
      connection: {
        ...
        clientId: data.connection.client_id,
        clientName: data.connection.client_name,
        deviceName: data.connection.device_name,
        scopes: data.connection.scopes,
        expiresAt: data.connection.expires_at,
      },
  ...
  connectEvents(handlers: MultiAIEventHandlers): MultiAIEventConnection {
