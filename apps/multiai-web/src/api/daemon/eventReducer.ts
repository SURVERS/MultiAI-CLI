// apps/multiai-web/src/api/daemon/eventReducer.ts
// Pure TypeScript state reducer for MultiAIClient.
...
export interface MultiAIClientState {
  sessions: AppSession[];
...
export function createInitialState(): MultiAIClientState {
  return {
    sessions: [],
...
function cloneState(s: MultiAIClientState): MultiAIClientState {
  return {
    ...s,
...
function advanceSeq(state: MultiAIClientState, sessionId: string | undefined, seq: number | undefined): void {
...
export function reduceAppEvent(
  ...
  switch (event.type) {
    ...
    // (see useMultiAIWebClient.processEvent), not reducer state. Advance seq
    // silently.
