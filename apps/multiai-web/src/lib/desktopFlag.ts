// apps/multiai-web/src/lib/desktopFlag.ts
//
// Detects whether the web UI is running inside the MultiAI Desktop app, and on
// which platform.
//
// The desktop app shares the local MultiAI daemon with the CLI / browser / TUI, so
...
// unreliable. Instead, the desktop app appends `?multiai_desktop=1&platform=<os>`
// to the URL it loads; we persist
// those values in sessionStorage so they survive any in-app navigation or
// redirect that drops the query string. The compile-time __MULTIAI_WEB_DESKTOP__
// is kept as an additional signal for the case where the web bundle itself was
// built for the desktop.

const QUERY_KEY = 'multiai_desktop';
const PLATFORM_KEY = 'platform';
const STORAGE_KEY = 'multiai-desktop';
const PLATFORM_STORAGE_KEY = 'multiai-desktop-platform';
...
function detect(): DesktopEnv {
  // `__MULTIAI_WEB_DESKTOP__` is injected by Vite `define`, but that replacement
  ...
  let desktop = typeof __MULTIAI_WEB_DESKTOP__ !== 'undefined' ? __MULTIAI_WEB_DESKTOP__ : false;
...
/** True when running inside the MultiAI Desktop app (any platform). */
