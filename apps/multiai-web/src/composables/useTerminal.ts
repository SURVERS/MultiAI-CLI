...
import type { AppTerminal, MultiAIEventConnection } from '../api/types';
...
export function useTerminal(sessionId: Ref<string>) {
  ...
  let conn: MultiAIEventConnection | null = null;

  function ensureConnection(): MultiAIEventConnection | null {
    if (conn !== null) return conn;
    if (typeof WebSocket === 'undefined') return null;
    conn = getMultiAIWebApi().connectEvents({
      onEvent: () => {},
      onResync: () => {},
      onError: (_code, msg) => {
        error.value = msg;
  ...
  async function close(): Promise<void> {
    ...
    try {
      ...
      await getMultiAIWebApi().closeTerminal(current.sessionId, current.id);
    } catch (error_) {
      error.value = error_ instanceof Error ? error_.message : String(error_);
    }
  }
