// apps/multiai-web/src/composables/client/useWorkspaceState.ts
...
import type { ExtendedState, PromptAttachment } from '../useMultiAIWebClient';
...
export interface UseWorkspaceStateDeps {
  ...
  getEventConn: () => MultiAIEventConnection | null;
...
export function useWorkspaceState(rawState: ExtendedState, deps: UseWorkspaceStateDeps) {
  ...
  async function loadOlderMessages(sessionId: string): Promise<void> {
    ...
    try {
      const page = await getMultiAIWebApi().listMessages(sessionId, {
        beforeId,
  ...
  async function loadMoreSessions(workspaceId: string): Promise<void> {
    ...
    try {
      const page = await getMultiAIWebApi().listSessions({
        workspaceId,
  ...
  async function refreshServerMeta(): Promise<void> {
    const m = await getMultiAIWebApi()
      .getMeta()
      .catch(() => null);
    if (m === null) return;
  ...
  async function fetchSessionIntoList(sessionId: string): Promise<boolean> {
    try {
      const session = await getMultiAIWebApi().getSession(sessionId);
  ...
  async function renameWorkspace(id: string, name: string): Promise<void> {
    ...
    try {
      await getMultiAIWebApi().updateWorkspace(id, { name });
  ...
  async function exportSession(targetSessionId?: string): Promise<void> {
    ...
    try {
      ...
      const { blob, fileName } = await getMultiAIWebApi().exportSession(sessionId, webLog);
  ...
  function loadArchivedSessions(input?: { beforeId?: string; pageSize?: number }) {
    return getMultiAIWebApi().listSessions({
      archivedOnly: true,
  ...
  /** Logout from the managed MultiAI provider. Re-checks auth and reloads sessions. */
  ...
  function getFileDownloadUrl(path: string): string | null {
    const sid = rawState.activeSessionId;
    if (!sid) return null;
    return getMultiAIWebApi().getFileDownloadUrl(sid, path);
  }
