// apps/multiai-web/src/composables/client/useModelProviderState.ts
...
import type { ExtendedState } from '../useMultiAIWebClient';
...
export function useModelProviderState(
  ...
  async function setModel(modelId: string): Promise<boolean> {
    ...
    try {
      await getMultiAIWebApi().updateSession(sid, {
        model: modelId,
  ...
  async function refreshProvider(id: string): Promise<void> {
    try {
      const result = await getMultiAIWebApi().refreshProvider(id);
  ...
  async function refreshAllProviders(): Promise<void> {
    try {
      const result = await getMultiAIWebApi().refreshAllProviders();
      for (const failure of result.failed) {
  ...
  /** Start the managed MultiAI browser flow, or its explicit device fallback. */
  async function startOAuthLogin(
    ...
    try {
      const api = getMultiAIWebApi();
      return await api.startOAuthLogin({ method, persistence });
    } catch {
      return null;
    }
  }
  ...
  async function loadAccount(): Promise<AccountSnapshot | null> {
    try {
      account.value = await getMultiAIWebApi().getAccount();
    } catch {
      account.value = null;
    }
