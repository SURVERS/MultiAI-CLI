// apps/multiai-web/src/composables/client/useNotification.ts
...
function maybeNotifyCompletion(sid: string, ctx: NotifyCompletionCtx): void {
  maybeNotify(
    ...
    `multiai-complete-${sid}-${ctx.promptId ?? Date.now()}`,
  );
}
...
function maybeNotifyQuestion(ctx: NotifyQuestionCtx): void {
  maybeNotify(
    ...
    `multiai-question-${ctx.questionId}`,
  );
}
...
function maybeNotifyApproval(ctx: NotifyApprovalCtx): void {
  maybeNotify(
    ...
    `multiai-approval-${ctx.approvalId}`,
  );
}
