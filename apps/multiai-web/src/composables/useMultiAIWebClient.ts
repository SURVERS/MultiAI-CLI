// apps/multiai-web/src/composables/useMultiAIWebClient.ts
...
import { getMultiAIWebApi } from '../api';
...
import { createInitialState, reduceAppEvent, type CompactionStatus, type MultiAIClientState } from '../api/daemon/eventReducer';
...
// existing `import type { ColorScheme, Accent } from './useMultiAIWebClient'`
...
export interface ExtendedState extends MultiAIClientState {
  connected: boolean;
...
async function refreshSessionStatus(sessionId: string): Promise<void> {
  ...
  try {
    st = await getMultiAIWebApi().getSessionStatus(sessionId);
  } catch {
...
async function refreshSessionGoal(sessionId: string): Promise<void> {
  ...
  try {
    goal = await getMultiAIWebApi().getSessionGoal(sessionId);
  } catch {
...
let eventConn: MultiAIEventConnection | null = null;
...
function applyEvent(event: ReturnType<typeof toAppEvent>, sessionId: string, seq: number): void {
  const snapshot: MultiAIClientState = {
...
function processEvent(appEvent: AppEvent, meta: MultiAIEventMeta): void {
...
function pushOperationFailure(
  ...
  console.error(`[multiai-web] operation failed: ${operation}`, err);
  const api = isDaemonApiError(err);
  const network = isDaemonNetworkError(err);
...
async function pullSessionWarnings(sessionId: string): Promise<void> {
  ...
  try {
    const warnings = await getMultiAIWebApi().getSessionWarnings(sessionId);
...
const turns = computed<ChatTurn[]>(() => {
  ...
  return messagesToTurns(
    messages,
    approvals,
    (fileId) => getMultiAIWebApi().getFileUrl(fileId),
    turnActive.value,
    rawState.planReviewByToolCallId,
  );
});
...
export function useMultiAIWebClient() {
  ...
  return {
    ...
    loadAccount: modelProvider.loadAccount,
