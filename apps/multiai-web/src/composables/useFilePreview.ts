// apps/multiai-web/src/composables/useFilePreview.ts
...
import type { useMultiAIWebClient } from './useMultiAIWebClient';

type MultiAIWebClient = ReturnType<typeof useMultiAIWebClient>;
...
export function useFilePreview({ client, detailTarget }: UseFilePreviewOptions) {
  ...
  function openMediaPreview(media: ToolMedia): void {
    ...
    if (media.fileId) {
      ...
      void getMultiAIWebApi().getFileBlob(media.fileId).then((blob) => {
