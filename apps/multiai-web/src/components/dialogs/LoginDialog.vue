<!-- apps/multiai-web/src/components/dialogs/LoginDialog.vue -->
<!-- MultiAI browser-first OAuth dialog with device-code and session-only fallbacks. -->
...
type Step = 'starting' | 'browser' | 'device-code' | 'success' | 'expired' | 'error';
...
async function startFlow(
  ...
  const result = await props.onStartOAuthLogin(method, persistence);
  if (!result) {
    step.value = 'error';
    return;
  }
  ...
  step.value = result.method === 'browser' ? 'browser' : 'device-code';
  startCountdown();
  scheduleNextPoll(result.method === 'device' ? result.interval : 1);
}
...
async function copyCode(): Promise<void> {
  ...
  if (flow.value.userCode === undefined) return;
...
async function close(): Promise<void> {
  stopTimers();
  // Best-effort cancel
  if (step.value === 'browser' || step.value === 'device-code') {
...
<template>
  <Dialog v-model:open="open" :title="t('login.title')" :close-on-overlay="false" @close="close">
    ...
    <!-- Device-code step -->
    <div v-else-if="step === 'browser' && flow && flow.authorizationUri" class="nb">
      <div class="nb-lead">Continue in your browser to sign in to MultiAI.</div>
      <a
        class="nb-primary"
      ...
      >
        Open MultiAI sign-in
        <Icon name="external-link" size="sm" />
      </a>
      <div class="nb-status">
        <Spinner size="sm" label="Waiting for browser authorization" />
        <span class="nb-status-text">This dialog closes automatically.</span>
        <span class="nb-countdown">{{ formatSeconds(secondsLeft) }}</span>
      </div>
      <div class="actions">
        <Button variant="secondary" size="sm" @click="startFlow('device')">Use device code</Button>
      </div>
    </div>

    <!-- Device-code step -->
    <div
      v-else-if="step === 'device-code' && flow && flow.verificationUri && flow.verificationUriComplete && flow.userCode"
      class="nb"
    >
    ...
    <template v-else-if="step === 'expired'">
      ...
      <div class="actions">
        ...
        <Button variant="secondary" @click="startFlow('browser', 'session')">Session only</Button>
