<!-- apps/multiai-web/src/components/ui/Dialog.vue -->
...
.ui-dialog__overlay {
  ...
  animation: multiai-dialog-overlay-in var(--duration-base) var(--ease-out);
}
@keyframes multiai-dialog-overlay-in {
  from { opacity: 0; }
  to { opacity: 1; }
}
.ui-dialog {
  ...
  outline: none;
  overflow: hidden;
  animation: multiai-card-in var(--duration-slow) var(--ease-out);
}
