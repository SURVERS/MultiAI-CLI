<!-- apps/multiai-web/src/components/Sidebar.vue -->
...
async function chooseBackend(name: BackendName): Promise<void> {
  ...
  if (next === null) {
    console.warn('[multiai-web] dev backend switch failed:', name);
    closeBackendMenu();
    return;
  }
...
// Logo easter-egg: clicking the MultiAI mark plays one quick blink. It's a one-shot
...
// Logo long-press easter-egg: holding the MultiAI mark for 1 second opens the
      ...
      <div class="ch">
        <div class="ch-brand">
          <template v-if="!isMacosDesktop">
            <img ref="logoRef" class="ch-logo" :class="{ 'is-dev': isDev }" src="/assets/multiai-logo-dark.png" alt="MultiAI CLI" @click="onLogoClick" @pointerdown="onLogoPointerDown" @pointerup="onLogoPointerUp" @pointercancel="onLogoPointerUp" />
            <span class="ch-name">MultiAI CLI</span>
            <Pill
              v-if="isDev"
              class="ch-backend"
