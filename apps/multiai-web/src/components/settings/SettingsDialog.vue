<!-- apps/multiai-web/src/components/settings/SettingsDialog.vue -->
...
import { useMultiAIWebClient } from '../../composables/useMultiAIWebClient';
import type { AppSession } from '../../api/types';
import { useDialogFocus } from '../../composables/useDialogFocus';
import LanguageSwitcher from './LanguageSwitcher.vue';
import { serverEndpointLabel } from '../../api/config';
import { downloadTraceLog, isTraceEnabled } from '../../debug/trace';
import type { Accent, ColorScheme } from '../../composables/useMultiAIWebClient';
import type { AccountSnapshot, AppConfig, AppModel } from '../../api/types';
...
// ---------------------------------------------------------------------------
const client = useMultiAIWebClient();
      ...
      <div class="body">
        ...
        <section v-show="activeTab === 'account'" class="panel">
          <section class="sec">
            ...
            <div class="row">
              <span class="rlabel">{{ authReady ? 'managed:multiai' : t('sidebar.notSignedIn') }}</span>
              <Tooltip :text="accountModel">
                <span v-if="authReady && accountModel" class="rvalue">{{ accountModel }}</span>
              </Tooltip>
            </div>
            <template v-if="authReady && account">
              <div class="row">
                <span class="rlabel">Profile</span>
                <span class="rvalue">{{ account.user.displayName ?? account.user.email ?? account.user.sub }}</span>
              </div>
              <div v-if="account.user.email" class="row">
                <span class="rlabel">Email</span>
                <span class="rvalue">{{ account.user.email }}</span>
              </div>
              <div class="row">
                <span class="rlabel">Wallet</span>
                <span class="rvalue mono">{{ account.wallet.total }}</span>
              </div>
              <div class="row">
                <span class="rlabel">Scopes</span>
                <span class="rvalue mono">{{ account.connection.scopes.join(', ') }}</span>
              </div>
              <div class="row">
                <span class="rlabel">API keys</span>
                <span class="rvalue mono">
                  {{ account.keys.length === 0 ? 'None' : account.keys.map((key) => `${key.name}: ${key.maskedKey}`).join(', ') }}
                </span>
              </div>
            </template>
            <div class="actions">
