...
import type { PromptAttachment } from './composables/useMultiAIWebClient';
...
// KAP/daemon debug panel — opt-in via ?debug=1 or localStorage multiai-web.debug=1.
...
async function handleStartOAuthLogin(
  ...
  return client.startOAuthLogin(method, persistence);
}
...
watch(
  client.authReady,
  (ready) => {
    if (ready) void client.loadAccount();
  },
  { immediate: true },
);
  ...
  <div class="app-shell">
    ...
    <section v-if="showAuthGate" class="auth-page">
      <div class="auth-page-inner">
        <img ref="authLogoRef" class="auth-page-logo ch-logo" src="/assets/multiai-logo-dark.png" alt="MultiAI CLI" @mousedown.prevent @click="blinkAuthLogo" />
