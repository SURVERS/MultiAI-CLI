...
function validBundle(): Record<string, string> {
  return {
    'manifest.json': JSON.stringify({ sessionId: 'session_orig', multiAIVersion: '0.20.2', workspaceDir: '/home/u/proj', title: 'imported demo' }),
    ...
    'logs/multiai.log': '2026-06-01T00:00:00.000Z INFO  hello  k=v\n',
  };
}
...
describe('import-store', () => {
  ...
  it('imports a valid bundle and lists it with manifest metadata', async () => {
    ...
    expect((await stat(join(dir, 'logs', 'multiai.log'))).isFile()).toBe(true);
