...
import { MULTIAI_HOME } from '../config';
import { discoverLogFiles, readLogs } from '../lib/log-reader';
import { readSessionDetail } from '../lib/session-store';

const SESSION_LOG_REL = ['logs', 'multiai.log'] as const;
const GLOBAL_LOG_REL = ['logs', 'global', 'multiai.log'] as const;
const HOME_GLOBAL_LOG_REL = ['logs', 'multiai.log'] as const;

export function logsRoute(home: string = MULTIAI_HOME): Hono {
  const r = new Hono();
  r.get('/:id/logs', async (c) => {
    ...
    // it is captured under the session dir (logs/global/multiai.log); for a
    // live local session it lives at <MULTIAI_HOME>/logs/multiai.log
    ...
    // Either log may have rotated (multiai.log.1, .2, …); discover the active
